 ...
 * ofrece igual (`RN-AC-010`). La escritura —fila y auditoría— es de {@link CourseClassifier}, que
 * comparte con el alta del curso desde el 25-09-2026.
 *
...
public class ClassifyCourseService {
  ...
  static final String ENTIDAD_FILA = CourseClassifier.ENTIDAD_FILA;
  ...
  public CourseDetailResponse classify(UUID courseId, ClassifyCourseRequest peticion) {
    ...
    // El `entity_id` de la auditoría es el del CURSO: la fila es suya y no
    // tiene identificador, y es por el curso por donde se pregunta «qué le hicieron».
    clasificador.clasificar(curso.getId(), categoria);
