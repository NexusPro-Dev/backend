...
### 15.1.1 La zona horaria de la operación

**Todo el sistema opera en la zona de Colombia** (`America/Bogota`), decidido el 01-09-2026 por el responsable del proyecto.

**Qué significa exactamente, porque no es «se guarda así»:** el almacenamiento **no cambia** —todo instante se persiste como `timestamptz`, que es UTC con desplazamiento y no admite ambigüedad—. Lo que la zona decide es **dónde se corta el día**, y eso solo importa donde una fecha o un horario se convierten en una decisión de negocio:

| Dónde | Qué decide |
|---|---|
| El código de un movimiento | El `AAAAMMDD` que lleva dentro (`RN-MV-017`) |
| El cierre mensual de comisiones | Cuándo son «las 00:00 del primero» (`RN-MV-029`) |
| La vigencia de una tarifa | Ya resuelto de otra forma: `commission_rates` usa `date` justamente para no tener que decidirlo (`requirements/cm.md` §7.1) |

**Por qué no UTC**, que era la alternativa y es lo que hace hoy el único trabajo programado del sistema: un movimiento hecho a las **23:30 en Bogotá** llevaría en su código **el día siguiente**, y el cierre del mes ocurriría a las **19:00 del día 31** — con las ventas de esas cinco horas cayendo en el mes siguiente sin que nadie lo hubiera decidido.
...
### 15.2.1 Y cómo ESCRIBE un módulo en otro — cierre de D-26

Hasta el 01-09-2026 la pregunta no se había hecho: las cuatro lecturas cruzadas son **de solo lectura**, y esta sección lo declara como norma con un argumento explícito —se devuelven modelos de lectura y nunca entidades, **precisamente para no dar con qué escribir**—.

`MV` la obliga: un **depósito confirmado tiene que habilitar la cuenta** de un cliente, y `users` es de `SP`.

**La norma es la misma que para leer: el dueño del dato publica, el consumidor invoca.** `SP` expone la **operación concreta** —«habilitar cuenta por depósito», «aplicar upgrade comprado»— en su capa `application`, con sus reglas intactas dentro de `SP`, y `MV` la llama.

| Qué se exige | Por qué |
|---|---|
| **Una operación por caso**, nunca una genérica | «Cambiar el estado de un usuario» sería una **llave maestra**: cualquier módulo movería cuentas por cualquier motivo, y las reglas de `RF-SP-028` quedarían del lado de quien llama |
| **Las reglas viven en el dueño** | La operación valida lo suyo. `MV` no sabe —ni debe— qué hace que una cuenta pueda habilitarse |
| **Síncrona y en la misma transacción** | Si la escritura falla, **falla la operación entera**. Nadie queda a medias: no hay depósito confirmado con cuenta retenida |
| **Sigue sin viajar la entidad** | Lo que cruza son parámetros y un resultado, no un agregado JPA |

**Se descartó el evento de dominio**, que era la alternativa seria: desacopla de verdad y deja que otros módulos reaccionen al mismo hecho, pero **la habilitación deja de ser inmediata** y aparece la pregunta de qué pasa si nadie lo atiende — una cuenta retenida **sin que nada haya fallado visiblemente**, que es la clase de avería que nadie reporta porque nadie sabe que ocurrió.

**Lo que esto NO abre**: `SP` no gana un endpoint, ni una operación que algún actor suyo pida. Gana un método publicado que existe **porque otro módulo lo necesita**, exactamente con el mismo reparto que D-25 fijó para las lecturas — y la tarea que lo escribe pertenece al requerimiento que lo necesita.
...
| 0.28.0 | 01-09-2026 | **D-26 se cierra el mismo día que se abrió**, por decisión del responsable del proyecto, y **§15.1.1 fija la zona horaria de la operación**. (1) **Cómo escribe un módulo en otro**: la norma es **la misma que para leer** —el dueño del dato publica y el consumidor invoca—, con **una operación por caso y nunca una genérica**, **síncrona y en la misma transacción**, y con las reglas viviendo en el dueño. §15.2.1 lo desarrolla. Se descartó el **evento de dominio**, que era la alternativa seria: desacopla de verdad, pero la habilitación deja de ser inmediata y aparece la pregunta de qué pasa si nadie lo atiende — **una cuenta retenida sin que nada haya fallado visiblemente**, que es la clase de avería que nadie reporta porque nadie sabe que ocurrió. Y se descartó la **escritura genérica**, que sería una llave maestra dejando las reglas de `RF-SP-028` del lado de quien llama. (2) **Todo el sistema opera en `America/Bogota`**. El almacenamiento no cambia —`timestamptz` sigue siendo UTC con desplazamiento— y lo que la zona decide es **dónde se corta el día**, que solo importa donde una fecha se vuelve decisión de negocio: el `AAAAMMDD` dentro del código de un movimiento y el instante del cierre mensual de comisiones. **Con UTC**, un movimiento de las 23:30 en Bogotá llevaría el día siguiente en su código y el mes cerraría a las 19:00 del 31, con esas cinco horas de ventas cayendo al mes siguiente sin que nadie lo decidiera. Queda escrito que `ExpiredTokenPurgeJob` **sigue en UTC y no hay que cambiarlo**: purgar tokens caducados no es una decisión de negocio — la zona gobierna **las operaciones que producen fechas visibles**, no todo lo que tiene un reloj. | Responsable del proyecto |
