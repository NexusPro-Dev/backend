...
| `YOUTUBE_API_KEY` | No | Clave de la YouTube Data API v3 con la que Academia lee la duración de una lección de video de YouTube (`RN-AC-017`, 25-09-2026). **Sin ella, toda lección de YouTube exige la duración a mano**; Vimeo no necesita credencial |
...
| Membresía vigente de una persona | Cuál es su nivel **hoy**, o que no tiene | `RF-PM-007`, `RF-MV-001` |
| Producto por **código**, con su destino y su vigencia | Qué membresía concede un producto y por cuántos días | `RF-SP-045` |
| Productos **para vender, por lote** | Precio, moneda, tipo, vigencia y membresía destino de cada uno, para **copiarlos** en la venta | `RF-MV-001`, `RF-MV-002` |
| **Si un producto está en la oferta** de una persona, por lote | Lo que `RF-PM-007` ya resuelve. `MV` **no lo recalcula**: dos definiciones de «lo que alguien puede comprar» divergirían, y la copia atrasada seguiría vendiendo lo que la otra ya no ofrece | `RF-MV-001`, `RF-MV-002` |
| El cliente: **estado y vendedor vigente** | Si puede comprar y **de qué vendedor cuelga**, que es a quién se atribuirá la venta | `RF-MV-001`, `RF-MV-002` |
...
Declararlo otra vez habría creado **la segunda definición de «vigente»**, que es precisamente lo que esta sección existe para impedir un nivel más abajo. Y deja una lección que conviene que no se pierda: **antes de publicar una lectura, hay que mirar si el módulo dueño ya la publica** — la norma de «una interfaz por lectura» facilita añadir puertos, y eso mismo hace fácil añadir el que ya está.

**El 18-09-2026 la tabla ganó la primera fila que responde sobre AUTORIZACIÓN y no sobre un dato**, y conviene decir por qué cabe aquí sin abrir una grieta: `AC` condiciona quién puede figurar como instructor a que porte un permiso de `SP` (`RN-AC-006`), y las tres formas de saberlo eran leer `user_roles` —lo que §7 de `modules.md` prohíbe—, que `SP` publicara la lista de permisos de la persona, o que respondiera **sobre uno**. Se eligió la tercera por la misma regla 2 de abajo: **la regla se queda con su dueño**. Con la lista, cualquier consumidor podría escribir su propio «¿tiene tal permiso?» y, con el tiempo, su propio «¿qué puede hacer?» — y ese es el código que un día diverge de `RN-SEG-010` sin que nada falle. Con un booleano sobre un código no hay nada que reimplementar. Y el predicado de «portar» no se escribió dos veces: es una constante que `JpaEffectivePermissions` y el puerto comparten.
...
| Dónde | Qué decide |
|---|---|
| El corte de un día en cualquier fecha de negocio | Qué día es «hoy» para lo que se declare por fecha |
...
**Lo que esto NO habilita.** Son lecturas. Aplicar un upgrade sobre la membresía de una persona es una **escritura** sobre `user_products` —`user_memberships` hasta el 23-09-2026—, con `RN-SP-018` de por medio, y sigue sin existir: `requirements/pm.md` §1.4 lo deja fuera del alcance, y el día que la compra lo necesite será otra decisión y otro puerto.
...
|---|---|---|---|
| **Conceder lo comprado** (`MembershipGrant`) | Escribe en `user_products` lo que la persona pasa a tener —el producto, la línea que lo entregó y la vigencia desde el instante indicado— y, **si lo comprado concede nivel**, cierra además la membresía vigente e inserta la nueva; audita en `SP`. **Ampliada el 23-09-2026** (`RN-SP-056`, `RN-MV-036`): hasta ese día solo escribía membresías, y la línea de un bot entregado no dejaba constancia de posesión en ninguna parte. El nombre se conserva a propósito —lo pide `RF-MV-003` y está citado en tripletas aprobadas—, aunque hoy describe menos de lo que hace | `SP`, en su capa `application` | `RF-MV-003` |
