...
**Ninguno.** La columna `status` y su restricción `ck_roles_status` se crean en `V4__create_roles.sql` (`RF-SP-001`).
...
**Respuesta `200`** — el rol con su estado actualizado.
...
| `403` | Sin `roles:update`, o el rol está entre los del actor (`EX-002`) | `RN-SEG-011` |
| `404` | El rol no existe o está eliminado (`EX-003`) | `EX-003` |
| `409` | El rol es de sistema o es el rol raíz (`EX-001`) | `RN-SEG-012` |
...
`RN-SEG-011` es aquí más que una formalidad: sin ella, un administrador podría desactivar su propio rol y quedarse sin acceso en la misma petición. Se verifica igual que en el resto del módulo, contra los códigos de rol del token y solo sobre los asignados directamente.
...
**`shared/security`** es donde este requerimiento tiene su efecto real. La resolución de permisos consulta el estado del rol, de modo que la invalidación debe alcanzar tanto a la entrada del rol como a cualquier resultado derivado que lo incluya.
...
|---|---|---|
| La invalidación no alcanza a los resultados derivados y el rol sigue concediendo | **Alto** | `CA-SP-050` debe probarse resolviendo permisos de un portador real, no comprobando la columna |
