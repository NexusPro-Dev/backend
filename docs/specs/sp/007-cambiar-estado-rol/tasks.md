...
| `T-07` | `api/RoleController`: añade `PATCH /api/v1/roles/{id}/status` con el permiso `roles:change-status`, devolviendo `RoleResponse` | `T-04`, `T-06` | Prueba de API: `200` con el estado actualizado; el `409` del rol de sistema cita `RN-SEG-012` y el del raíz, `RN-SEG-007` | Hecha |
