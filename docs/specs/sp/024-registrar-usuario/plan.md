...
### 2.3.ter `V57__todo_usuario_con_membresia.sql` — segunda enmienda del 05-09-2026

**El mismo día y sobre lo anterior**: el responsable del proyecto decidió que **toda persona tenga membresía**, y que quien no reciba una arranque en la más baja. `RN-SP-018` se reescribe de «todo consumidor» a «todo usuario», y `RN-SP-013` y `RN-SP-015` quedan **retiradas**.

**Esta migración RELLENA y no altera nada.** No hay columna nueva, ni restricción nueva:
...
| `fk_user_products_movement_detail` | `(movement_detail_id) → movement_details(id)` **`ON DELETE CASCADE`** | De qué línea salió. Es una clave foránea de `SP` que apunta a `MV`, y **no invierte la dependencia**: es el mismo caso que `client_sellers.first_movement_id`. `SP` no lee esa tabla; guarda el identificador que la orden de **D-26** le entrega. **`ON DELETE CASCADE` y no `RESTRICT`, que es la unica de las cuatro que no lo lleva** (23-09-2026): la posesion es **derivada** de la linea, y una fila que apunta a una linea que ya no esta no dice de donde salio lo que alguien tiene. En produccion la clausula **no se dispara nunca** —una venta no se edita ni se borra (`RN-MV-001`), solo avanza de estado—, de modo que `RESTRICT` protegia aqui un caso que no ocurre a cambio de obligar a cada suite de pruebas a aprenderse un orden de borrado. La asimetria con el producto es la que importa: aquel si va con `RESTRICT`, porque nadie puede hacer desaparecer un producto mientras alguien lo tenga. |
...
| `application` | `AssignableDocumentType` | **Nuevo, 08-09-2026** | Puerto de lectura del tipo de documento a asignar: devuelve si existe y si está activo, **distinguiendo los dos**, igual que {@code AssignableCountry}. Es un puerto de `SP` hacia su propio catálogo y **no el repositorio de `RF-SP-051`**: el alta no lee el catálogo entero para comprobar una fila. Lo reutilizan `RF-SP-027` y `RF-SP-045` |
...
| `400` | Tipo o número de documento ausente (`EX-010`, `RN-SP-035`) | `VAL-015` |
| `400` | Número de documento con formato inadmisible | `VAL-016` |
| `400` | Teléfono ausente o con formato inadmisible (`RN-SP-037`) | `VAL-017` |
...
| `409` | El tipo de documento existe pero está **inactivo** (`EX-010`) | `RN-SP-035` |
| `409` | El par tipo+número **ya lo tiene otra persona**, vigente o eliminada (`EX-010`) | `RN-SP-035` |
...
| `422` | El tipo de documento **no existe** en el catálogo (`EX-010`) | `EX-010` |
...
5.ter **Existencia y estado del tipo de documento, y unicidad del par** (`EX-010`). El tipo se lee **con bloqueo compartido**, por el mismo motivo que el país. La unicidad se consulta **para el mensaje** y la garantiza `uq_users_document` en la escritura, igual que el nombre de usuario: entre la lectura y el `INSERT` hay una ventana que dos altas simultáneas atraviesan, y el adaptador traduce esa violación al mismo `409` distinguiéndola **por nombre de restricción**.
...
Cinco decisiones, y la primera es la que hay que entender antes que las demás:

- **LAS SEIS COLUMNAS NACEN NULABLES, y dos de ellas son obligatorias en la API.** Es la asimetría deliberada con `country_id` (§2.6), que sí nació `NOT NULL`, y la diferencia está en **qué significa el relleno**. Un país de relleno es una afirmación neutra: poner «Colombia» al superadministrador no dice nada falso sobre nadie. Un **número de documento** de relleno es otra cosa — es una afirmación sobre la **identidad** de una persona, y **cualquier valor que se invente es falso**. Lo mismo el teléfono. `V22` siembra un superadministrador que no tiene ni uno ni otro, y la semilla de desarrollo crea decenas de personas que tampoco. De modo que el esquema **admite la ausencia** —que es la verdad sobre esas filas— y **la API la prohíbe** en toda alta nueva. La consecuencia se acepta entera: **existen y seguirán existiendo personas sin documento**, y todo consumidor tiene que contemplarlo; lo que no puede ocurrir es que se creen más.
- **`ck_users_document_pair` es la restricción que más fácil se olvida y la que más barata sale.** Sin ella caben dos estados que no significan nada: un número sin decir de qué documento es, y un tipo sin número. Con las dos columnas nulables, **es lo único que impide media identidad**.
- **`uq_users_document` es parcial sobre el nulo y TOTAL sobre los eliminados.** No lleva `deleted_at IS NULL`, y esa es la parte que importa: `RN-SP-035` no libera el documento al eliminar, con el mismo criterio que `uq_users_username` — reutilizarlo permitiría que la actividad de dos personas quedara bajo la misma identidad. El `WHERE document_number IS NOT NULL` **no es una excepción a la regla**, es lo que permite que convivan las filas anteriores a esta migración: sin él, dos personas sin documento colisionarían entre sí.
- **La unicidad va sobre el PAR y no sobre el número.** Dos catálogos distintos pueden numerar igual, y un índice sobre `document_number` solo rechazaría documentos legítimos.
