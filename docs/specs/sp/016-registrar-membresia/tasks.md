...
| `T-19` | Migración `V38__add_membership_color.sql`: columna `color varchar(6)` nullable, relleno de las filas existentes con valores distintos entre sí, `SET NOT NULL`, `ck_memberships_color_format` y `uq_memberships_color`, **en ese orden** (`plan.md` §2) | — | `mvn flyway:info` la lista aplicada y una prueba comprueba que la columna es `NOT NULL`, que un valor con `#` se rechaza y que dos membresías no pueden compartir color | Hecha |
| `T-20` | `domain/models/Membership`: campo `color`, **recortado y normalizado a mayúsculas** en el constructor, con el formato validado en el dominio y no solo en el DTO | `T-19` | Prueba unitaria: `1e88e5` se guarda como `1E88E5`; `#1E88E5`, `1E88E` y `1E88E5F` se rechazan | Hecha |
| `T-21` | `application`: `RegisterMembershipRequest` y `RegisterMembershipCommand` ganan `color` obligatorio; `MembershipResponse`, `MembershipItem` y `MembershipDetailItem` lo devuelven | `T-20` | El alta exige el campo y la respuesta lo trae en mayúsculas | Hecha |
| `T-22` | Traducir la violación de `uq_memberships_color` **por nombre de restricción** a `VAL-009`, distinguiéndola de la de código y la de nombre | `T-21` | Prueba de integración: el color repetido produce su propio rechazo y no el de código o nombre en uso | Hecha |
| `T-23` | Pruebas de API de `CA-SP-487` a `CA-SP-489`, y documentación OpenAPI del campo en los tres endpoints de membresía | `T-21`, `RF-SP-017 · T-11`, `RF-SP-018 · T-12` | La suite cubre los tres criterios y el contrato publicado declara `color` en la petición y en las tres respuestas | Hecha |
