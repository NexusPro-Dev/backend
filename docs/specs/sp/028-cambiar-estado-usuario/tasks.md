...
| `T-10` | Revocación de sesiones **dentro** de la transacción de negocio, y publicación del corte **fuera y después del commit** | `T-06`, `T-09` | Prueba de integración: forzando el fallo de la revocación, el estado **tampoco** cambia; forzando el fallo del commit, ningún token queda cortado | **Hecha** — 26-08-2026 |
...
| `T-13` | `api`: `ChangeUserStatusRequest` con el motivo condicional en los dos sentidos y rechazo de propiedades desconocidas; `UserStatusResponse`; y `PATCH /api/v1/users/{id}/status` con el permiso `users:change-status` | `T-08` | Prueba de API: `PENDIENTE` devuelve `400`; un cuerpo con `lockedUntil` devuelve `400` por campo desconocido; el motivo al reactivar devuelve `400` con `VAL-006` | **Hecha** |
...
| `T-15` | Prueba **concurrente** de dos desactivaciones simultáneas de **dos superadministradores distintos**, con transacciones reales | `T-05`, `T-13` | Una `200` y una `409` con `RN-SP-001`, y **queda al menos un portador activo**. Es la prueba que distingue bloquear el conjunto de bloquear la fila | **Hecha** — 26-08-2026, en `UserConcurrencyIT`. **Destapó el defecto que describe**: ver §4.ter |
...
| 7 | Quien tiene `users:change-status` puede desactivar cualquier cuenta **y** liberar bloqueos. Aceptado en `spec.md` §14, resolución 1: separarlo sería un permiso nuevo y un requerimiento nuevo | 22-08-2026 | Responsable del proyecto | Abierto |
...
| 5 | ~~`T-09` (`AccessRevocationRegistry`) queda **Pendiente**, y con ella la mitad de `T-10`~~ — **cerrada el 26-08-2026** | Lo que la mantenía abierta era cómo se propaga el corte entre instancias. Se resuelve **sin decidirlo**: el corte vive en memoria detrás del puerto `AccessRevocationPublisher`, y sustituirlo por un canal compartido no toca ningún caso de uso. La decisión sigue pendiente y ahora bloquea solo el despliegue de una segunda instancia (bloqueo 2), no el requerimiento | Mientras estuvo abierta, **el token de acceso ya emitido seguía valiendo hasta quince minutos** tras desactivar la cuenta, eliminarla o restablecer su contraseña. Hoy se corta en la petición siguiente, y `AccessRevocationIT` lo verifica por los cuatro caminos. Tres hallazgos al construirlo, en §4.ter |
...
## 4.ter Lo que apareció al construir el corte — 26-08-2026

Tres cosas que el plan no anticipaba. Ninguna cambia la decisión de §7; las tres cambian **cómo** se implementa.

| # | Hallazgo | Qué se hizo |
|---|---|---|
| 1 | **El plan dice «el filtro de autenticación compara el `iat`», y no hace falta un filtro.** Un token cortado no es el de alguien sin permiso: es un token que **ya no vale**, igual que uno caducado | Se implementa como `OAuth2TokenValidator<Jwt>` sobre el `JwtDecoder`, junto al validador de vigencia. Produce el mismo `401` por el mismo camino y, sobre todo, **corta antes de que `JwtActorConverter` resuelva los permisos contra la base**. Un filtro posterior habría pagado esa consulta para descartar la petición justo después |
| 2 | **El segundo del corte es indistinguible.** El `iat` de un JWT va en **segundos enteros**: dentro del segundo en que se revoca no hay forma de saber si un token nació antes o después. Lo destapó la prueba de `RF-SP-037`: quien cambia su contraseña y vuelve a entrar en el acto recibía un `401` con el token recién emitido | **No se elige a qué lado caen los empates: se quita la ambigüedad.** Cerrarlos mata tokens legítimos y abrirlos deja vivos quince minutos los que debían morir — las dos opciones son malas. En cambio `AccessTokenIssuer` **sella** el token que emite con `iat` igual al corte cuando este es posterior al reloj: si estamos emitiendo es que la revocación ya ocurrió y esa persona acaba de probar quién es, de modo que ahí no hay nada ambiguo. El adelanto es de menos de un segundo, la expiración se sigue contando desde el reloj —no regala vida— y ningún validador lo mira: `JwtTimestampValidator` comprueba `exp` y `nbf`, y este token no declara `nbf` |
| 3 | **La siembra del arranque falla en silencio si se lee mal el tipo.** `queryForList` devuelve lo que el driver decida —un `Timestamp` para `timestamptz`, no un `OffsetDateTime`—, y el molde muere con un `ClassCastException` que el `catch` del arranque convierte en un registro **vacío** | Se lee con `RowMapper` y `getObject(..., Class)`. El defecto lo destapó la prueba del reinicio, que es la única que mira dentro del registro: por HTTP todo seguía pasando, porque el corte del caso de uso sí se anotaba. Es exactamente el fallo silencioso que la siembra existe para evitar, escondido dentro de la siembra |

## 4.quater `T-15` encontró lo que su propio texto anunciaba — 26-08-2026

`T-05` estaba marcada **Hecha** desde el 24-08-2026 y su verificación se cumplía: la traza mostraba el `SELECT … FOR UPDATE` sobre el conjunto y ordenado. **Y aun así la regla no se cumplía.** Dos desactivaciones simultáneas de superadministradores distintos devolvían `200` las dos y dejaban el sistema con **cero** portadores activos.

**Por qué el bloqueo no bastaba.** `FOR UPDATE OF ur` bloquea las filas de `user_roles`, y el razonamiento escrito en el adaptador era que la segunda transacción esperaría y, al soltarse, PostgreSQL reevaluaría la condición sobre la versión ya confirmada. Eso es cierto **solo cuando la primera modifica la fila bloqueada**:

| Requerimiento | Qué toca | ¿Funcionaba? |
|---|---|---|
| `RF-SP-029`, `RF-SP-031` | **Borran** la asignación | **Sí** — la reevaluación la ve desaparecer y el conteo baja |
| `RF-SP-028` (este) | Solo `users.status` | **No** — la fila bloqueada no cambió, y el `users` del `JOIN` se lee de la instantánea propia, donde la cuenta sigue activa |

Las dos transacciones contaban dos portadores, las dos concluían que sobraba uno.

**Cómo se corrigió.** Una **compuerta**: la fila del rol raíz en `roles`, tomada con `FOR UPDATE` **antes** de contar. Una sola fila conocida de antemano, de modo que no hay orden que acordar ni abrazo mortal posible. Y el conteo va en una **sentencia aparte**: en `READ COMMITTED` cada sentencia toma su instantánea, y la que corre ya con la compuerta en la mano ve lo que la otra confirmó.

**Se descartó bloquear también las filas de `users`** (`FOR UPDATE OF ur, u`), que resuelve la reevaluación y **crea un abrazo mortal**: cada transacción ya tiene bloqueada la fila de su objetivo —el caso de uso la toma en el paso 2— y pediría la del objetivo de la otra.

**La lección es sobre la verificación, no sobre el código.** `T-05` pedía comprobar *que la traza muestra el bloqueo*, y eso se puede cumplir con un bloqueo que no sirve. Lo que prueba la regla es ejecutarla concurrentemente, que es lo que `T-15` hace — y su propio texto ya lo decía: «es la prueba que distingue bloquear el conjunto de bloquear la fila».
...
- [ ] Todas las tareas en estado `Hecha`. — falta `T-15`, y ocho en curso. `T-09` y `T-10` quedaron hechas el 26-08-2026.
...
- [x] Los endpoints nuevos declaran su permiso. — `users:change-status`.
