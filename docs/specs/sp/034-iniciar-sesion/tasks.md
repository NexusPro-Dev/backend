...
| 5 | `T-08` (`RateLimiter`) y `T-12` (`MustChangePasswordFilter`) quedan **Pendientes** | El primero necesita decidir dónde vive el estado de la ventana —en memoria no sobrevive a más de una instancia—, y el segundo no puede escribirse sin `RF-SP-037`, que es el único endpoint que debe seguir accesible con la marca puesta | El `429` de `EX-001` **no se produce ni se documenta**, y el `mcp` viaja en el token y se devuelve en la respuesta, pero **no restringe nada todavía**. `T-17` queda por tanto en curso: el contrato no publica el `429` porque no existe |
...
- [ ] Todas las tareas en estado `Hecha`. — faltan `T-08`, `T-12` y `T-15`, y ocho en curso.
- [ ] Todos los criterios de aceptación con prueba automatizada en verde. — `CA-SP-293`, `CA-SP-300`, `CA-SP-377` y `CA-SP-379` sin prueba.
