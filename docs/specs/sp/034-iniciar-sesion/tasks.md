...
| `T-08` | `application/RateLimiter` e `InMemoryRateLimiter`: ventana deslizante por credencial y por origen | — | Prueba de API: superar el límite devuelve `429`; la ventana se reabre al pasar | Hecha |
...
| 9 | El flujo gana un **paso 5**: releer la cuenta con su fila bloqueada antes de emitir la sesión | Lo pidió `RF-SP-029` · `T-12`, cuya prueba concurrente destapó que una sesión podía nacer **después** de que una eliminación barriera las sesiones de esa cuenta, y sobrevivir a ella. El paso 4 lo decide sobre una instantánea donde la cuenta sigue viva | Un bloqueo de fila por inicio de sesión **correcto**. Va en el camino de éxito y **no al principio** a propósito: tomarlo antes de comprobar la contraseña le añadiría al rechazo un tiempo que depende de si la cuenta existe, que es la fuga por temporización que `EX-001` existe para cerrar |
