...
| `T-04` | Revocación de **todas** las sesiones con motivo `ACCESO_RETIRADO`, **incluida la que ejecutó el cambio**, dentro de la transacción, más la publicación del corte de tokens de acceso | `T-02` | Prueba de integración: ningún refresh token vigente queda; el token de acceso de la sesión que hizo el cambio deja de admitirse | **Hecha** — la revocación el 24-08-2026; **el corte, el 26-08-2026**: hasta entonces la tarea estaba marcada hecha con solo la mitad construida, y la segunda frase de su verificación no era cierta (§4.bis · 5) |
...
| `T-06` | **Exceptuar esta ruta en `MustChangePasswordFilter`** de `RF-SP-034` | — | Prueba de API: con `mcp` en verdadero, esta ruta responde y **cualquier otra** es negada. Sin la excepción, la cuenta queda sin salida | **Hecha** — 26-08-2026, en `MustChangePasswordIT` |
...
|---|---|---|---|---|
| 1 | Ninguna tarea es ejecutable hasta que `RF-SP-034` exista: aporta la verificación de contraseña, `LockoutPolicy`, `refresh_tokens` y el filtro que `T-06` debe exceptuar | 24-08-2026 | Responsable técnico | **Cerrado** — el filtro existe desde el 26-08-2026; el resto, desde el 24-08-2026 |
...
| 3 | ~~`T-06` —exceptuar esta ruta en el filtro de cambio obligatorio— queda **Pendiente**~~ — **cerrada el 26-08-2026** | El filtro ya existe: `RF-SP-034` · `T-12` se implementó ese día | Mientras estuvo abierta, **la marca de cambio obligatorio no restringía nada**: quien la tenía usaba el resto de la API con normalidad. Esta ruta es hoy la excepción que da salida a la cuenta, y `MustChangePasswordIT` recorre el ciclo entero —retenida, cambia, libre— en lugar de comprobar solo que la ruta responde |
...
| 5 | `T-04` estuvo marcada **Hecha** desde el 24-08-2026 con **solo la mitad construida** | Se revocaban los refresh tokens y no se publicaba el corte de tokens de acceso, porque `AccessRevocationPublisher` no existía: `RF-SP-028` · `T-09`, que lo aporta, seguía **Pendiente**. La tarea se cerró mirando lo que sí se había hecho | Su verificación decía «el token de acceso de la sesión que hizo el cambio deja de admitirse» y **era falsa**: seguía admitiéndose hasta quince minutos, y ninguna prueba lo comprobaba. Corregido el 26-08-2026 al implementar el registro; `AccessRevocationIT` lo verifica ahora de extremo a extremo. **La lección es de método, no de código:** marcar hecha una tarea cuyo segundo verbo depende de otro requerimiento abierto convierte la lista de tareas en algo que no se puede leer |
...
- [ ] Todas las tareas en estado `Hecha`. — `T-06` quedó hecha el 26-08-2026, al existir el filtro. Quedan tres en curso.
