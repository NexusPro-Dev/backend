...
Exige `users:change-own-password` desde el 21-09-2026 (`RF-SP-062`); hasta entonces no había permiso asociado más allá de estar autenticado (`requirements/sp.md` §6.1). Nadie cambia la contraseña de otro por este camino.
...
- La persona está autenticada y porta `users:change-own-password` — **hasta el 21-09-2026 sin permiso** (`RF-SP-062`).
