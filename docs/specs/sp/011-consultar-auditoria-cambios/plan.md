...
**El actor se devuelve como identificador, no como nombre.** Con los usuarios dentro de `SP`, resolver el nombre sería un `LEFT JOIN` a `users` y no una llamada a otro módulo, de modo que la objeción técnica desapareció. Se mantiene la decisión por dos razones que no dependen de dónde vivan los usuarios. La primera: `spec.md` §6.2 dice «actor» sin exigir su nombre, y el valor probatorio del registro está en el identificador, que no cambia nunca; un nombre es una foto del momento en que se consulta, no del momento en que ocurrió el evento, y en un registro de auditoría esa diferencia importa. La segunda: `users` la crea `RF-SP-024`, y aunque se haya adelantado en el orden de implementación por `RF-SP-003`, atar este endpoint a esa tabla añade una dependencia que hoy no necesita. Añadir `actorName` más adelante es aditivo y no rompe a ningún cliente.
...
- ~~**No se devuelve el nombre del actor**~~ · **Revertido el 28-08-2026**: el actor llega resuelto con su `username` —inmutable, `RN-SP-016`— y su nombre actual, y el `actorId` sigue siendo el dato probatorio. Sigue sin devolverse el nombre de la entidad afectada, por lo dicho en §3.
...
| `RF-SP-024` | Ninguna dependencia hoy. Si algún requerimiento posterior pide el nombre del actor, será un `LEFT JOIN` a `users` con `deleted_at` incluido —un usuario eliminado sigue siendo el autor de lo que hizo— y una decisión sobre qué mostrar cuando ese usuario ya no exista |
...
| ~~Resolver el nombre del actor con un `LEFT JOIN` a `users`~~ · **ADOPTADA el 28-08-2026** | Se descartó porque `spec.md` §6.2 no lo pedía y porque el nombre es una foto del momento de la consulta. Lo segundo **sigue siendo cierto**, y por eso lo que se devuelve como identidad es el `username`, que es **inmutable** (`RN-SP-016`); el nombre completo va declarado como **actual** y no como evidencia. La previsión de esta misma fila se cumplió: fue **aditivo** y no rompió a ningún cliente |
...
| La pantalla muestra el UUID del actor en lugar de su nombre | Bajo | Consecuencia aceptada de no devolver lo que `spec.md` no pide (§3). El identificador es el dato probatorio; el nombre es presentación y llegará con el requerimiento que lo pida, sin romper el contrato |
