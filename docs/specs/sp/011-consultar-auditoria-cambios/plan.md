...
| Resolver el nombre del actor con un `LEFT JOIN` a `users` | Técnicamente barato desde que los usuarios están en `SP`, pero `spec.md` §6.2 no lo pide y el nombre es una foto del momento de la consulta, no del evento. El identificador es el dato probatorio. Añadirlo después es aditivo |
