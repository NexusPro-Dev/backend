...
| Enmendada | 18-09-2026 — **el cliente sale de `user_supervisors`** (`RN-SP-028` revertida, `RN-SP-049`): el registro escribe la fila `REGISTRO` de **`client_sellers`**, con la venta del enlace como `first_movement_id`, y ese vendedor es su principal para siempre. `CA-SP-513`, `CA-SP-525` y `CA-SP-526` quedan **invertidos** en `CA-SP-711` a `CA-SP-713`; `T-02` deja de existir. Lo construye `RF-SP-059` |
...
| `CA-SP-513` | **Invertido el 18-09-2026** en `CA-SP-711`. Decía: «el cliente queda a cargo del vendedor en `user_supervisors`, en una fila vigente» |
| `CA-SP-525` | **Invertido el 18-09-2026** en `CA-SP-712`. Decía: «un vendedor con clientes a cargo no se puede desactivar ni eliminar sin reasignarlos» |
| `CA-SP-526` | **Invertido el 18-09-2026** en `CA-SP-713`. Decía: «`GET /users/{id}/team` devuelve a los clientes junto al equipo» |
| `CA-SP-711` | El cliente queda vinculado al vendedor como **`REGISTRO`** en `client_sellers`, con **la venta del enlace** como `first_movement_id`, y **`user_supervisors` no recibe ninguna fila**; la línea de esa venta lleva al mismo vendedor |
| `CA-SP-712` | Un vendedor que registró clientes **se puede desactivar y eliminar** sin reasignar nada: la cartera no cuenta para `RN-SP-022`, y sus filas `REGISTRO` quedan |
| `CA-SP-713` | `GET /users/{id}/team` del vendedor **no devuelve a sus clientes**; los devuelve `GET /users/me/sellers` desde el lado del cliente (`RF-SP-059`) |
...
| 1.6.0 | 21-09-2026 | **El cliente sale de `user_supervisors`**, por decisión del responsable del proyecto (`RN-SP-028` revertida, `RN-SP-049` enmendada; `requirements/sp.md` v1.63.0). El registro por enlace deja de colgar al cliente en la tabla de mando y escribe la fila `REGISTRO` de `client_sellers`, con la venta del enlace como `first_movement_id`; ese vendedor es su principal **y no se cambia**. **Mismo contrato**: ni la petición ni la respuesta cambian un campo. `CA-SP-513`, `CA-SP-525` y `CA-SP-526` se invierten en `CA-SP-711` a `CA-SP-713`; `RN-SP-020` deja de aplicar y `RN-SP-022` deja de alcanzar a la cartera; `T-02` deja de existir. El código lo cambia `RF-SP-059` (`T-08`), que además trae la migración `V20` que mueve lo que este endpoint escribió hasta hoy. — redactada el 18-09-2026 en `feature/vendedores-de-un-cliente` e integrada sobre `feature/academia` el 21-09-2026 con el número renumerado; `RF-SP-060` de aquella rama pasa a `RF-SP-061` porque `RF-SP-060` nació en `feature/academia` el 19-09-2026 | Responsable del proyecto |
