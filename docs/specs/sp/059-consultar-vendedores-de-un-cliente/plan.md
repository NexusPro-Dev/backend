...
- **`/me/sellers`: `@PreAuthorize("hasAuthority('users:read-own-sellers')")` desde el 21-09-2026** (`RF-SP-062`). Hasta entonces, sin `@PreAuthorize` y con la ausencia declarada en `EndpointPermissionsIT` como autenticada sin permiso. El actor sale del token; no hay identificador que validar ni existencia que comprobar.
