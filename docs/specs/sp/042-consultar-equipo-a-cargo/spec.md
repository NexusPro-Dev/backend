...
| Enmendada | 18-09-2026 — **la cartera sale del equipo**: el cliente deja de colgar de `user_supervisors` (`RN-SP-028` revertida) y el equipo vuelve a ser solo fuerza comercial. `CA-SP-625` queda **invertido** en `CA-SP-710`; `roles` y su filtro se conservan; «los clientes de un vendedor» pasa a `RF-SP-061` (renumerado el 21-09-2026; nació como `RF-SP-060`). Lo construye `RF-SP-059` |
...
| `CA-SP-625` | **Invertido el 18-09-2026** en `CA-SP-710`. Decía: «un cliente de la cartera llega con su rol a la vista y no en nulo, que es lo que permite distinguirlo de un vendedor sin rol» — y desde `V20` no hay cliente que llegue |
| `CA-SP-710` | Consultar el equipo de un vendedor que registró clientes devuelve **solo** a sus vendedores directos: los clientes **no aparecen**, y `roles=CLIENTE` devuelve el equipo vacío con `200` |
