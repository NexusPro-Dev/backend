...
| `T-04` | `api/UserController`: `GET /api/v1/users/{id}/team` con `users:read`, `page` y `size` | `T-03` | Prueba de API: `size` por encima del máximo devuelve `400`. **La verificación cambió el 10-09-2026**: exigía que ningún filtro se aplicara, y el filtro por rol sí se aplica desde `T-15`; los demás siguen sin aplicarse | **Hecha** |
