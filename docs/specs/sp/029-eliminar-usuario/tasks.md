...
| `T-12` | Pruebas **concurrentes** con transacciones reales: eliminación contra inicio de sesión, y dos eliminaciones de superadministradores distintos | `T-06`, `T-10` | Nunca queda una sesión viva sobre una cuenta eliminada; y queda al menos un portador activo del rol raíz | **Hecha** — 26-08-2026, en `UserConcurrencyIT` |
...
### `T-12` destapó una sesión que sobrevivía a la eliminación — 26-08-2026

Eliminar una cuenta mientras alguien inicia sesión con ella dejaba **una sesión viva sobre una cuenta que ya no existe** — exactamente lo que la verificación de esta tarea declara imposible.

**La secuencia.** El inicio de sesión comprueba el estado de la cuenta en su paso 4, sobre **su instantánea**, donde sigue viva. Mientras tanto la eliminación marca el borrado, revoca las sesiones vigentes y confirma. El inicio, que ya había decidido que la cuenta podía entrar, inserta su refresh token **después** de ese barrido: nace fuera de él y nadie vuelve a mirarlo.

**Por qué no se veía.** Es intermitente por construcción: si la eliminación confirma antes de que el inicio tome su instantánea, el inicio ve la cuenta eliminada y rechaza. La prueba pasó aislada y falló en la suite completa, que es donde el reparto de hilos cambia.

**Cómo se corrigió.** `LoginService` vuelve a leer la cuenta **con su fila bloqueada** justo antes de emitir la sesión, y confirma que sigue pudiendo entrar. Al soltarse el bloqueo, PostgreSQL reevalúa la fila sobre la versión ya confirmada, de modo que el borrado se ve.

**Va en el camino de éxito y no al principio**, y esa asimetría es deliberada: tomar el bloqueo antes de comprobar la contraseña le añadiría al rechazo un tiempo que depende de si la cuenta existe — la fuga por temporización que `RF-SP-034` · `EX-001` existe para cerrar. El coste es un bloqueo de fila por inicio de sesión correcto, que es una operación poco frecuente comparada con los rechazos.
