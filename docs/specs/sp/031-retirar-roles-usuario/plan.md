...
| `application` | `TeamMembershipRetirement` | **Consumido — enmienda del 23-09-2026** | Puerto que publica `teams` (`RF-SP-070`): cierra la pertenencia vigente de la persona si la hay, y **no falla si no la hay**. Se invoca cuando el retiro deja a la persona sin el rol comercial de mayor rango, **en la misma transacción** y con la misma correlación (`RN-SP-055`). El sentido del cruce es el admisible: `users` consume un puerto que `teams` publica, y no al revés |
