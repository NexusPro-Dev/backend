...
| `T-02` | `SaleLinesRequest` en `application`: los nueve parámetros, con los estados y las fechas **como texto** para que su `400` viaje junto a los demás | — | `CA-MV-174` | Pendiente |
...
| `T-05` | `SaleLineResponse` y `ListSaleLinesService`: los cinco `400` **juntos**, la página y el total acotado. `@Transactional(readOnly = true)` | `T-04` | `CA-MV-164`, `CA-MV-166` a `CA-MV-174` | Pendiente |
...
|---|---|---|---|---|
| 1 | **`V37` espera a que `RF-MV-016` integre su `V36`.** Su migración deja el catálogo en 134 y la guarda de la mía lo exige ahí. Acordado con esa sesión el 23-09-2026: su rama se mezcla primero. Si el orden se invierte, esta tripleta **renumera** a `V36` y permiso 134 | 23-09-2026 | Responsable técnico | **Abierto** |
| 2 | **Los fixtures tendrán que poner `type_status_id`** en las ventas que insertan a mano, en cuanto `RF-MV-016` añada esa columna (`NOT NULL`, sin `DEFAULT`). No afecta al código de producción, solo a `SaleLinesIT`, y se resuelve al integrar con la subconsulta que esa sesión dejó escrita | 23-09-2026 | Responsable técnico | **Abierto** |
