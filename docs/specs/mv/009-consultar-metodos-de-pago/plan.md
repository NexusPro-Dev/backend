...
| Enmendado | 09-09-2026 — `RN-MV-024`: la ruta pasa a ser **pública**. Cambia §5 entera, el `401` de §4 desaparece y entran dos componentes que este plan no tenía: `SecurityConfig` y `RateLimitFilter` (Art. I.7) |
...
| `shared/security` | `SecurityConfig` | **Modificado** (09-09-2026) | La ruta entra en `CATALOGOS_PUBLICOS`, **casada por método** |
| `shared/security` | `RateLimitFilter` | **Modificado** (09-09-2026) | Cuarto cubo de la política `public-catalog`, ya existente |
...
| Estado | Cuándo |
|---|---|
| `200` | **Siempre**, con sesión o sin ella (`RN-MV-024`) |
| `429` | Al superar la cota de la política `public-catalog`: 120 por minuto y por origen, con cubo propio |

**No hay `403`, y su ausencia es la decisión de autorización** (§5). **No hay `404`**: un catálogo vacío es una lista vacía. **Y desde el 09-09-2026 tampoco hay `401`**: la ruta es pública, de modo que el único rechazo que puede devolver es el `429` de la cota — que no dice nada de quién pregunta.
...
**Ninguna: la ruta es pública desde el 09-09-2026** (`RN-MV-024`). Hasta ese día era «autenticado y sin permiso», y el porqué de que no llevara permiso **sigue valiendo entero**, de modo que se conserva abajo: es el argumento que impide que alguien le ponga uno al reabrir esta sección.

### 5.1 Se abre en `SecurityConfig` y **casada por método**

Entra en `CATALOGOS_PUBLICOS` —la segunda lista, la que casa por verbo— y **no** en `RUTAS_PUBLICAS`. Hoy `/api/v1/payment-methods` solo responde a un `GET`, de modo que las dos listas se comportarían igual; se elige la que ata el método porque **el día que el catálogo se administre por API** —`requirements/mv.md` §5.3 lo tiene aplazado, no descartado— la ruta entera ya estaría abierta a nivel de filtro y el alta pasaría a responder `403` en lugar de `401`. Es exactamente el razonamiento que `RN-SP-041` escribió para `/api/v1/countries`, con la diferencia de que allí el `POST` ya existía y aquí todavía no: **se declara ahora, cuando cuesta una línea, y no el día que alguien lo descubra**.

**No hay `@PreAuthorize` que retirar**, y esa es la diferencia con la apertura de los tres catálogos de `SP`. Aquel cambio dejó `countries:read`, `document-types:read` y `brokers:read` **sembrados y sin endpoint que los exija**; aquí no sobra ningún permiso porque este endpoint nunca exigió uno.

### 5.2 La cota de tasa entra sola: es el cuarto catálogo público

`RateLimitFilter` gana la ruta en su propia lista de catálogos públicos y **reutiliza la política `public-catalog`** —120 por minuto y por origen, sin penalización—, que ya existe desde el 08-09-2026. No se declara una política nueva **porque la naturaleza es la misma**: es una ruta pública que consulta la base en cada llamada, y lo que se corta es el bucle. No es la cota del hotlink: aquí no hay nada que sondear, porque la respuesta es idéntica para todo el mundo y no depende de ningún dato que quien pregunta pueda variar.

**Cubo propio y no compartido**, como los otros tres: el ámbito es la ruta, de modo que agotar el de países no deja al mismo formulario sin métodos de pago.

### 5.3 Por qué no lleva `movements:read` — el argumento de 04-09-2026, intacto

Ese permiso gobierna **ver ventas** —quién compró qué y por cuánto—, y hoy está reservado al superadministrador (`requirements/mv.md` §6.1). Exigirlo aquí dejaría **sin métodos de pago a todo el mundo salvo uno**, y con ello la pantalla de compra propia (`RF-MV-002`), que no exige ningún permiso, quedaría sin poder pintar su selector.

Es el mismo razonamiento por el que `RF-PM-007` no exige `products:read`: son dos preguntas y dos actores.

### 5.4 Lo que hace que abrirlo sea barato no está en esta sección

Está en la consulta. La respuesta lleva **los dos ejes** desde el mismo día —`is_active` y `visibility = 'PUBLICO'`—, de modo que el anónimo recibe **exactamente lo que recibía el autenticado**. Si el predicado tuviera uno solo, abrir la ruta publicaría el pago gratuito, y con él la forma de anotar como gratuita una venta que no lo es (`RN-MV-022`). **La seguridad de este cambio la sostiene `RN-MV-023`, no el filtro**, y por eso `CA-MV-052` lo comprueba **sin token** en lugar de confiar en que lo de siempre siga funcionando.
...
**Y el 09-09-2026, con la apertura de la ruta, tres más** —también aplicadas en el mismo pase, y ninguna toca el esquema:

| Documento | Enmienda |
|---|---|
| `requirements/mv.md` v0.14.0 | Nace `RN-MV-024`; §4.1 deja `RF-MV-009` **sin permiso y sin sesión**; §6 declara que no sobra ningún permiso y §7.4 anota que `visibility` pasa a decidir **qué ve cualquiera** |
| `security.md` | La tabla de rutas públicas de §6 gana `GET /api/v1/payment-methods`, **solo en `GET`**, con el motivo escrito al lado |
| `requirements.md` | La matriz recoge que la consulta es pública, y con qué cota |

**En el código, ahora sí hay impacto fuera de `MV`**: `SecurityConfig` y `RateLimitFilter` son de `shared/security`. Ninguno de los dos aprende nada de métodos de pago —les entra una ruta en una lista que ya existía—, y esa es la razón de que no haga falta política de tasa nueva (§5.2).
...
| **Dejar la ruta cerrada y que el registro por enlace no ofrezca método de pago** | Es lo que había, y obliga al formulario a enviar `movement.paymentMethodId` sin poder pintar de dónde elegirlo — o a que el frontend lleve los códigos escritos a mano, que es el acuerdo por fuera del contrato que el Art. VIII.7 prohíbe y que este requerimiento existía para quitar |
| **Un segundo endpoint público** (`/api/v1/public/payment-methods`) | Dos rutas, dos entradas en el contrato y dos sitios donde arreglar lo mismo, para devolver **exactamente la misma respuesta**: la que ya salía estaba acotada por los dos ejes. La duplicidad solo se justificaría si el catálogo público fuera un subconjunto del privado, y no lo es |
| **Abrir la ruta entera** en `RUTAS_PUBLICAS`, sin casar el método | Hoy daría igual —solo hay `GET`—, y por eso es la alternativa que más fácil se toma. Se descarta por lo que pasaría el día que el catálogo se administre por API: el alta quedaría abierta a nivel de filtro y respondería `403` en lugar de `401`, que es decir «existe y no puedes» en vez de «identifícate» |
| **Abrirla sin cota de tasa** | Es una ruta pública que consulta la base en cada llamada, como el refresco y como los otros tres catálogos. Sin cubo, el bucle sale gratis |
| **Añadir un parámetro para que el autenticado vea más** —lo inactivo, o lo `INTERNO`— | Daría dos catálogos con dos verdades sobre lo mismo, y el selector del registro podría discrepar del de la pantalla de compra. `RN-MV-023` ya decidió que lo `INTERNO` no sale bajo ninguna petición, y `CA-MV-049` lo prueba contra **la ausencia del parámetro** y no solo contra la respuesta |
...
| 4 | **Que el predicado pierda un eje** y la ruta pública empiece a publicar el pago gratuito | Es el riesgo que nace con la apertura, y el único que empeora al abrirla: antes un descuido así lo veía un autenticado; ahora lo ve cualquiera. Se mitiga con `CA-MV-052`, que comprueba **sin token** que no salen ni lo desactivado ni lo `INTERNO` |
| 5 | **Que alguien vuelva a cerrar la ruta** al revisar la lista de rutas públicas, y el formulario de registro se quede sin selector | `CA-MV-033` está **invertido y no borrado**: afirma que sin token se responde `200`. Cerrar la ruta falla ahí, y no en producción. Es lo que `CA-SP-143` y `CA-SP-606` hicieron el día antes |
| 6 | **Que se copie la cota del hotlink** creyendo que aquí también hay algo que sondear | La respuesta es idéntica para todo el mundo y no depende de ningún dato que quien pregunta pueda variar: no hay oráculo. La política que aplica es `public-catalog`, que ya existe, y §5.2 dice por qué |
...
| **Sin token** | Integración | `CA-MV-033`: `200`. **Invertido el 09-09-2026**; era `401` |
| Lo mismo con token y sin él | Integración | `CA-MV-051`: las dos respuestas se comparan **entre sí**, no contra una lista escrita a mano |
| **Los dos ejes, para el anónimo** | Integración | `CA-MV-052`: sin token no salen ni el desactivado ni el `INTERNO`. Cubre el riesgo 4 |
