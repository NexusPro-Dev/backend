...
| Versión | 0.5.0 |
| Estado | **Descartado el 15-09-2026** — la tasa de rol nace con su producto (`RN-CM-021`) |
...
| 0.5.0 | 15-09-2026 | **Descartado.** La tasa de rol **nace con su producto** (`RN-CM-021`, [`requirements/cm.md`](../../../requirements/cm.md) v0.14.0 §5.4), de modo que no hay nada que asociar: lo que esta operación comprobaba —producto vivo, un rol por producto, el tope, el gratuito— se comprueba en el alta (`RF-CM-001` v1.1.0). Sus rutas se retiran del contrato y `product_commission_rates` se borra en `V94`. La tripleta se conserva como historia; el número queda consumido. | Responsable del proyecto |
