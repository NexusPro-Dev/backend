...
**Y desde v0.3.0, una cosa más**: dónde vive la regla de que una tasa declara **una forma y solo una** (`RN-CM-016`). Es la primera regla del módulo que **rige sobre dos agregados a la vez** —el catálogo por rol y la tasa personalizada—, y la decisión de este plan es que **viva una sola vez**. Ver §3.1.
...
### 2.3 `V49` — el valor fijo

**Una sola migración para las dos tablas.** `commission_rates` y `user_commission_rates` reciben exactamente los mismos tres cambios, y separarlas en dos migraciones dejaría un estado intermedio en el que una de las dos piezas admite el valor fijo y la otra no — un estado que **nadie querría desplegar** y que sin embargo existiría en el historial.

Sobre cada una de las dos:

| Cambio | Detalle |
|---|---|
| `rate_type varchar(20) NOT NULL` | La forma declarada. Ver abajo por qué se añade **con** valor por defecto y se le quita después |
| `percentage` pasa a **admitir nulo** | Deja de ser obligatoria: solo la lleva la forma proporción |
...
**`ck_*_percentage` hay que rehacerla, y es el error fácil de esta migración.** La que existe desde `V44` dice `percentage >= 0 AND percentage <= 100`. Con la columna ya nula, ese `CHECK` **evalúa a nulo y por tanto acepta la fila** —el mismo defecto que `V48` documentó sobre `ck_deletion_reason`—, de modo que seguiría estando ahí sin comprobar nada de las filas de tipo `FIJO`. No es un fallo: es lo que se quiere. Pero **si no se reescribe explícitamente, nadie que la lea sabrá si eso se decidió o se pasó por alto**, y se reescribe como `percentage IS NULL OR (percentage >= 0 AND percentage <= 100)` con la rama nula **delante**, siguiendo el criterio que `V48` ya usó.

!!! danger "`rate_type` se añade con `DEFAULT 'PORCENTAJE'` y el valor por defecto se RETIRA acto seguido"

    El relleno es **exacto y no una suposición**: hasta hoy la única forma que existía era el porcentaje, de modo que toda fila anterior a `V49` es de tipo `PORCENTAJE` por definición. A diferencia de `V48`, aquí **no hay nada que borrar**.

    Lo que no puede quedarse es el valor por defecto. Si permanece, una inserción que **omita la forma** obtiene `PORCENTAJE` en silencio — que es exactamente lo que `spec.md` §6.1 acaba de decidir que no debe pasar: la forma se **declara**, y una petición que no la declare tiene que ser rechazada, no completada.

    `ALTER COLUMN rate_type DROP DEFAULT` es, por eso, **la línea más importante de `V49`**, y la que un despliegue apresurado se dejaría.

## 3. Componentes afectados

| Capa | Componente | Nuevo / Modificado | Responsabilidad |
|---|---|---|---|
| `domain/models` | `CommissionValue` | **Nuevo (v0.3.0)** | La forma y su valor, **juntos y validados a la vez**. Ver §3.1 |
| `domain/models` | `CommissionRateType` | **Nuevo (v0.3.0)** | `PORCENTAJE` \| `FIJO` |
| `domain/models` | `CommissionRate` | **Rehecho** | El agregado, reducido a rol y **un `CommissionValue`** |
...
`RN-CM-016` es la primera regla del módulo que **rige sobre dos agregados**: la tasa de rol y la personalizada la cumplen igual, y las dos podrían romperla por separado. La respuesta es un **objeto embebido** que las dos incrustan, con la comprobación en su constructor.
...
2. **`RF-CM-005` devuelve un objeto en lugar de un número.** La comisión resuelta pasa a ser «esta forma, este valor», que es lo único que significa algo (`spec.md` §6.2).
3. **La instantánea de auditoría gana un campo sin que nadie lo decida dos veces**, porque la arma el agregado a partir del mismo objeto.
...
El cuerpo gana `rateType` y `fixedAmount`, y **`percentage` deja de ser obligatorio**. Es un cambio **incompatible** del contrato publicado, y conviene llamarlo por su nombre: una petición que hoy funciona —solo `roleId` y `percentage`— **pasará a devolver `400`** por no declarar la forma.

!!! warning "Se rompe a propósito, en lugar de dejar que la forma ausente signifique porcentaje"

    La alternativa compatible existe y es tentadora: sin `rateType`, suponer `PORCENTAJE`. Nadie tendría que cambiar nada.

    **Y reintroduce exactamente el defecto que `V49` se molesta en quitar del esquema** (§2.3): la forma volvería a deducirse en lugar de declararse, y una petición equivocada —la que quiso enviar un importe y se equivocó de campo— quedaría **aceptada como porcentaje** en vez de rechazada.

    El coste de romper es bajo y acotado: el módulo no tiene consumidores fuera del propio proyecto, y el `400` **dice qué falta**. El coste de no romper es una petición mal escrita que se registra como una tasa válida y **se descubre liquidando**.

**`fixedAmount` no lleva moneda en el contrato**, y la documentación publicada tiene que decirlo. Es el único importe de dinero de toda la API que se declara sin su moneda; sin una frase que lo explique, quien lea el contrato supondrá que falta un campo.
...
**En la documentación, cuatro enmiendas ya aplicadas** en el mismo pase que la v0.2.0 de este plan:
...
**Y dos más para la v0.3.0, aplicadas ANTES que este plan** y no en el mismo pase — es el orden que el responsable del proyecto fijó el 02-09-2026: primero el documento del módulo, después el modelo de datos, después las tripletas, y el código al final.

| Documento | Enmienda |
|---|---|
| `requirements/cm.md` v0.7.0 | §1.1.1 «Las dos formas de declarar una comisión»; nacen `RN-CM-016`, `RN-CM-017` y `RN-CM-018`; §7.1, §7.2 y §7.4 con las tres columnas y las seis restricciones |
| `modelo-datos.md` v0.17.0 | Las veintiuna tablas siguen escritas, **pero cuatro columnas están diseñadas y sin escribir**; la asimetría con `RN-PM-007`; y qué le hace a `RN-CM-008` que la copia pase de un número a tres cosas |

**Sigue sin haber impacto en el código de otros módulos.** Y conviene decir que **no lo hay a pesar de tratarse de dinero**: un importe fijo de comisión **no consulta `currencies`** ni resuelve nada contra `PM`, precisamente porque no lleva moneda (`RN-CM-017`). La frontera de D-25 no se toca. **Lo que sí crece es la deuda con la liquidación**, y crece fuera de este módulo: será ella quien tenga que juntar el importe con la moneda del producto.
...
| **Deducir la forma de qué campo venga lleno**, sin `rate_type` | Funciona mientras llegue exactamente uno. Con los dos llenos o ninguno **no se puede decir cuál de los dos errores es**, y el `CHECK` no podría nombrarlo. Argumentado en `cm.md` §7.1 y en `spec.md` §6.1 |
| **Dejar el `DEFAULT 'PORCENTAJE'`** después del relleno de `V49` | Una inserción que omita la forma la obtendría en silencio, que es justo lo que se acaba de decidir que no pase. Ver §2.3 |
| Suponer `PORCENTAJE` cuando falta `rateType` **en la API**, para no romper el contrato | La misma deducción, un nivel más arriba. Compra compatibilidad al precio de aceptar como válida una petición equivocada. Ver §4 |
| **Un solo campo `value` más el tipo**, en vez de dos columnas | Obligaría a `numeric(14,4)` para un porcentaje que cabe en `numeric(5,2)`, y **perdería el `CHECK` de cero a cien**: no se puede acotar una columna cuyo rango depende del valor de otra. Dos columnas dejan que cada forma conserve su propia restricción |
| Tres atributos sueltos en el agregado, sin `CommissionValue` | La regla solo sería cierta **entre asignaciones**. Ver §3.1 |
| **Una moneda propia en la tasa**, con coincidencia exigida al asociar | Descartada en `cm.md` v0.7.0 y no aquí: **la tasa personalizada no tiene producto con el que coincidir**, y se quedaría sin forma de expresarse |
| Un tope superior para `fixed_amount` | Sería un número inventado. La tasa no conoce el precio del producto —y al registrarla no hay ninguno—, de modo que el único tope defendible es el de la venta, y ese lo comprueba la liquidación. `RN-CM-018` |
...
| 5 | **`ALTER COLUMN rate_type DROP DEFAULT` se quede fuera de `V49`** | Es el fallo silencioso de esta migración: todo seguiría funcionando y la forma habría dejado de ser obligatoria. Lo cubre una prueba propia — ver §11 |
| 6 | **`ck_*_percentage` se quede como está** y deje de comprobar nada sin que nadie lo note | Se reescribe explícitamente con la rama nula delante. Ver §2.3 |
| 7 | Un cliente del contrato se rompa al dejar de bastar `percentage` | Aceptado y buscado. El módulo no tiene consumidores externos y el `400` dice qué falta. Ver §4 |
...
| **Alta en valor fijo** | Integración | `CA-CM-079`: y la respuesta trae **la forma junto al valor** |
...
| **`rate_type` no tiene valor por defecto** | Integración | Ver abajo |

**Falta la prueba que ninguna de las anteriores cubre**, y es la del riesgo 5. Todas ellas pasan por la API, que **siempre envía la forma**, de modo que ninguna se entera de si la columna conserva su `DEFAULT`. La comprobación tiene que ser **contra el esquema**: un `INSERT` directo que omita `rate_type` **debe fallar**.

Es una prueba fea —habla `SQL` en lugar de negocio— y es la única capaz de fallar cuando la línea que importa se cae de la migración. El módulo ya tiene precedente de pruebas así: `CA-CM-075` comprueba algo que el esquema **habría aceptado sin ello**.

**No hay prueba concurrente en este requerimiento**, y su ausencia sigue siendo una afirmación: no queda ninguna regla que dos altas simultáneas puedan burlar. `RN-CM-016` tampoco es una de ellas — **es una regla dentro de una fila**, no entre filas, y por eso un `CHECK` la cierra del todo. Las que sí quedan se prueban en `RF-CM-006` y `RF-CM-007`.
