...
- **Corregir a importe fijo una tasa asociada a un producto de PRECIO CERO:** existe desde el 08-09-2026, porque `RN-PM-006` dejó de exigir «mayor que cero» (`V67`, `requirements/pm.md` §5.2.4). La conversión `fixed_amount ÷ precio` **dividiría entre cero**, y la resolución no necesita ninguna regla nueva: un producto que no cobra nada **no puede pagar ningún importe fijo**, de modo que cualquier valor fijo mayor que cero pasa del 100 % y la corrección se rechaza **entera** —como con cualquier otro producto que se pasara—; uno de cero ocupa cero. Corregir a **porcentaje** no se ve afectado: no divide por nada.
