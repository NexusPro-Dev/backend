...
|---|---|---|---|---|
| `T-01` | Puerto `CommissionRateQueryRepository` con los filtros de `plan.md` §4 | `RF-CM-001` · `T-01` | Compila; los filtros nulos significan «sin filtro» | `Pendiente` |
| `T-02` | Adaptador con **una sentencia** y los tres `LEFT JOIN` que resuelven rol, producto y persona | `T-01` | Rol, producto y persona llegan resueltos sin consulta adicional | `Pendiente` |
| `T-03` | El **predicado en un solo método**, incluido el filtro por fecha como pertenencia | `T-02` | `valid_from <= fecha AND (valid_to IS NULL OR valid_to >= fecha)` | `Pendiente` |
| `T-04` | El **grado** calculado en el modelo de lectura | `T-02` | Los cuatro grados salen de qué columnas vienen nulas | `Pendiente` |
| `T-05` | `ListCommissionRatesService`, de solo lectura | `T-03` | `@Transactional(readOnly = true)` | `Pendiente` |
| `T-06` | DTO de filtros con `VAL-006` y `VAL-011` | `T-05` | Fecha mal formada y paginación fuera de límites | `Pendiente` |
| `T-07` | `GET /api/v1/commission-rates` con la envoltura paginada del sistema | `T-05`, `T-06` | `200` con total y orden; `403` sin permiso | `Pendiente` |
| `T-08` | Pruebas de los criterios de `spec.md` §12 | `T-07` | `CA-CM-014` a `CA-CM-022` |  `Pendiente` |
...
| `T-11` | Documentación OpenAPI del endpoint | `T-07` | Los seis parámetros y la envoltura | `Pendiente` |
| `T-12` | Actualizar la matriz de `docs/requirements.md` | `T-08` | La fila de `RF-CM-002` refleja el estado | `Pendiente` |
