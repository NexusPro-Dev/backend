...
|---|---|---|---|---|
| `T-01` | `application/UpdatePackageRequest` con `Patchable` en los tres corregibles y `Patchable<Object>` en `code` y `currencyId`; `informaAlgo()`, `traeInmutables()` | `RF-PM-017 · T-07` | Unitaria de deserialización: los tres estados por campo | **Hecha el 15-09-2026** |
| `T-02` | `ProductPackage.update(name, description, scope, ahora)`: recorta, valida, rechaza el nulo en nombre y alcance, vacía la descripción, devuelve el mapa de cambios y avanza `updatedAt` solo si no está vacío | `RF-PM-017 · T-03` | Unitaria: cada campo, nulo de descripción, sin cambios | **Hecha el 15-09-2026** |
| `T-03` | `ProductPackageRepository.existsAliveNameForOther(name, id)` | `RF-PM-017 · T-05` | Integración: el propio nombre no choca; el de otro vivo sí; el de un retirado no | **Hecha el 15-09-2026** |
| `T-04` | `domain/service/UpdatePackageService`: inmutables (`VAL-004`) y «al menos uno» (`VAL-005`) antes de leer; paquete vivo con bloqueo (`EX-001`); nombre libre (`EX-002`); aplicar; auditar solo si cambió; relectura del detalle | `T-01`, `T-02`, `T-03`, `RF-PM-019 · T-03` | `CA-PM-285`, `CA-PM-288`, `CA-PM-290` | **Hecha el 15-09-2026** |
| `T-05` | `PackageController`: `PATCH /api/v1/packages/{id}`, `@PreAuthorize("hasAuthority('packages:update')")` | `T-04` | La ruta entra en `EndpointPermissionsIT` | **Hecha el 15-09-2026** |
...
|---|---|---|---|---|
| 1 | Depende de `RF-PM-017` y `RF-PM-019` | 15-09-2026 | Responsable técnico | **Cerrado** el 15-09-2026 |
