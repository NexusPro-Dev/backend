...
| `T-25` | **Los enlaces, corregibles en bloque** (`plan.md` §4): `UpdateProductRequest` cambia `Patchable<String> videoUrl` por `Patchable<List<ProductLinkRequest>> links` —contando en `informaAlgo`—; `UpdateProductService` comprueba el tipo repetido sobre el cuerpo (`VAL-015`), construye los `ProductLink` con las validaciones que ya escribió `RF-PM-001` `T-43` y **reemplaza el conjunto en dos sentencias** —borrar los tipos que no vienen, insertar o reescribir los que sí—; el diff de auditoría lleva `links` con `before` y `after` como **un valor**, y **enviar el mismo conjunto no registra evento**; y la prosa de la `@Operation` dice con todas las letras que **la colección que llega es la que queda** y que un tipo que no viene **se borra** | `T-24`, `RF-PM-001` `T-43` | `CA-PM-225` a `CA-PM-227` reescritos y `CA-PM-389` a `CA-PM-393` en `ProductUpdateIT`. **El contrato regenerado declara `links` en `UpdateProductRequest` y ningún `videoUrl`** | **Hecha el 22-09-2026** |
