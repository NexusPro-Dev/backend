...
| `RN-PM-032` | Un producto puede enlazar un video — **se corrige y se quita**, en los dos tipos, con la misma comprobación de forma que en el alta. **Desde el 22-09-2026 se corrige dentro de `links`** | `requirements/pm.md` §5.1 |
| `RN-PM-048` | **Los enlaces viven aparte, y hay uno por tipo** — aquí, que se corrigen **en bloque**: la colección que llega es la que queda, y las mismas comprobaciones que en el alta | `requirements/pm.md` §5.1 |
| `RN-PM-049` | **El identificador se pega al final, y administración ve el crudo** — aquí, que lo que se envía es **lo crudo**, exactamente lo que `RF-PM-002` y `RF-PM-003` devolvieron | `requirements/pm.md` §5.1 |
...
| Enlaces | No | **El conjunto entero** de enlaces del producto. **Sustituye al «enlace del video» el 22-09-2026** | **Se corrige EN BLOQUE, y ahí se aparta de todo lo demás de esta operación**: los otros campos se corrigen **uno a uno** y aquí la colección **viaja entera o no viaja**. **Ausente: no toca ningún enlace.** **Nula o vacía: los quita todos** —es el vaciado explícito de siempre, convertido en colección—. **Con entradas: es el conjunto que queda**, de modo que un tipo que estaba y no viene **se borra**, uno que viene y no estaba **se crea**, y uno que viene y estaba **se reescribe**. Mismas comprobaciones que en el alta: tipo conocido, tipo no repetido, dirección obligatoria y con forma, identificador acotado, y la incompatibilidad entre identificador y `?` o `#` (`RN-PM-048`, `RN-PM-049`) |
...
| `VAL-009` | Formato de la dirección de un enlace. **Decía «del enlace del video» hasta el 22-09-2026**; vale igual para los dos tipos, y el error **nombra el índice y el tipo** del enlace que lo incumple | La dirección del enlace debe ser una dirección absoluta http o https, sin espacios y de hasta 500 caracteres. |
| `VAL-014` | **Tipo de enlace obligatorio y dentro del dominio** (22-09-2026) | El tipo del enlace es obligatorio y debe ser uno de los admitidos. |
| `VAL-015` | **Tipo de enlace repetido** (22-09-2026), comprobado **sobre el cuerpo recibido y antes de escribir** | Un producto no puede declarar dos enlaces del mismo tipo. |
| `VAL-016` | **Dirección del enlace obligatoria** (22-09-2026). Quitar un enlace es **no incluirlo en la colección**, no enviarlo vacío | La dirección del enlace es obligatoria. |
...
| `CA-PM-225` | El sistema **corrige el enlace de tipo `VIDEO_PRESENTACION`** —también en un producto de tipo **bot**— y el evento registra el conjunto anterior y el nuevo. **Reescrito el 22-09-2026**: hasta ese día el enlace era el campo `videoUrl` |
| `CA-PM-226` | El sistema **quita todos los enlaces** con `links` nula **o vacía**, el producto queda sin ninguno, y enviar **el mismo conjunto que ya tenía no registra evento**. **Reescrito el 22-09-2026** |
| `CA-PM-227` | El sistema rechaza una dirección **sin forma de URL absoluta `http` o `https`** con `VAL-009`, **nombra el enlace por su índice y su tipo**, y **no aplica ninguno** de los demás cambios enviados. **Reescrito el 22-09-2026**: hasta ese día nombraba `videoUrl` |
| `CA-PM-389` | **`links` ausente no toca ningún enlace**: se corrige el nombre de un producto que tiene los dos, y los dos siguen ahí, **sin evento de enlaces** |
| `CA-PM-390` | **La colección que llega es la que queda**: un producto con `VIDEO_PRESENTACION` y `CUPON_BOT` que recibe solo `CUPON_BOT` **pierde el video**; uno que recibe un tipo que no tenía **lo gana**; y uno que recibe el mismo tipo con otra dirección **lo reescribe** |
| `CA-PM-391` | El sistema **rechaza dos enlaces del mismo tipo** con `VAL-015` y **no aplica ningún cambio**, ni de enlaces ni de los demás campos |
| `CA-PM-392` | El sistema rechaza **un tipo desconocido** con `VAL-014`, **un enlace sin dirección** con `VAL-016` y **un identificador con espacios o de más de 100 caracteres** con `VAL-017` |
| `CA-PM-393` | El sistema rechaza **un identificador sobre una dirección que lleva `?` o `#`** con `VAL-018`; **la misma dirección sin identificador se admite**, y **quitar el identificador de un enlace que ya lo tenía sobre una dirección limpia también** |
