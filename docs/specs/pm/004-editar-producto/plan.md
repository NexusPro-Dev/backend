...
- **La portada no se corrige por aquí, y `coverImageUrl` en el cuerpo se ignora**: `UpdateProductRequest` no tiene el campo, y Jackson descarta lo desconocido como con cualquier otro. No se rechaza como los inmutables (`VAL-006`) porque no es un campo del producto que alguien pueda creer corregible por JSON: es un archivo, y la ruta correcta está en la prosa de la `@Operation`. **La respuesta sí lo trae**: `ProductDetailResponse` gana `coverImageUrl` (`RF-PM-014` §4).
