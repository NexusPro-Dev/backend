...
| Enmendada el | 22-09-2026 — **cada fila trae `links`, y `videoUrl` desaparece** (`RN-PM-048` a `RN-PM-050`): los enlaces **crudos**, con su identificador, y **esta es una de las dos únicas lecturas donde se ve el `CUPON_BOT`**. Ver §15 |
...
| `RN-PM-032` | **Un producto puede enlazar un video, y el enlace sale en toda lectura** — cada fila lo trae, **dentro de `links` desde el 22-09-2026** | `requirements/pm.md` §5.1 |
| `RN-PM-048` | **Los enlaces viven aparte, y hay uno por tipo** — aquí, que cada fila trae su colección, **vacía** cuando el producto no declara ninguno | `requirements/pm.md` §5.1 |
| `RN-PM-049` | **El identificador se pega al final, y administración ve el crudo** — aquí, que esta lectura devuelve **la dirección tal cual y el identificador en su campo**, sin componer, porque es lo que `RF-PM-004` espera recibir de vuelta | `requirements/pm.md` §5.1 |
| `RN-PM-050` | **El cupón del bot es entrega, no material de venta** — aquí, **la excepción**: esta lectura **sí** lo trae, con `RF-PM-003`, porque es donde se administra | `requirements/pm.md` §5.1 |
...
| Dato | Descripción |
|---|---|
| Productos | Identificador, tipo, nombre, descripción, **icono**, **sus enlaces** —los dos tipos, crudos y con su identificador externo (22-09-2026; era «el enlace del video»)—, **la dirección de la portada**, **los dos precios** con su moneda, **vigencia en días**, estado y —en los upgrades— la membresía destino con su nombre y su nivel |
...
| `CA-PM-223` | Cada fila devuelve **`links`** con lo que el producto declaró, tal cual se guardó, y **presente y vacía** en los productos que no declaran ninguno. **Reescrito el 22-09-2026**: hasta ese día comprobaba `videoUrl` presente y nulo |
| `CA-PM-385` | Una fila con los **dos** enlaces los devuelve **los dos**, `CUPON_BOT` incluido: es la lectura de administración, y es donde se administran. El identificador externo viaja **en su campo y sin pegar**, con la dirección cruda |
| `CA-PM-386` | Una página con **veinte productos con enlaces** no dispara una consulta por fila: los enlaces de la página se leen **en una sola sentencia** más, y el total no crece con el tamaño de la página |
...
| 0.11.0 | 22-09-2026 | **Cada fila trae `links`, y `videoUrl` desaparece** (`RN-PM-048` a `RN-PM-050`, [`requirements/pm.md`](../../../requirements/pm.md) v0.43.0 §5.2.14). Los enlaces del producto dejan de ser un campo de la fila y pasan a ser **una colección**, con hasta uno por tipo. **Esta lectura es, con `RF-PM-003`, una de las dos únicas donde se ve el `CUPON_BOT`** (`RN-PM-050`): no filtra por tipo, al revés que la oferta y los dos hotlinks, porque es **donde se administra** y esconderle a administración lo que publica sería dejarla sin poder revisarlo. **Y los devuelve crudos** (`RN-PM-049`): la dirección tal cual y el identificador externo en su campo, **sin pegar**, porque quien usa esta lectura es quien va a mandar el `PATCH` de `RF-PM-004` y tiene que recibir lo que luego enviará. Lo que cambia en la consulta: los enlaces **ya no salen del `SELECT` del listado** —están en otra tabla—, de modo que entra **una segunda sentencia** con `IN` sobre los identificadores de la página ya resuelta, con la forma que la conversión estrenó. Se descartó el `JOIN`: multiplicaría las filas del producto por sus enlaces y dejaría al `LIMIT` contando filas que no son productos. `CA-PM-223` se **reescribe** —comprobaba `videoUrl` presente y nulo— y nacen **`CA-PM-385`** y **`CA-PM-386`**, el segundo para que la segunda sentencia no se convierta un día en una por fila sin que nadie lo note. Enmienda de Art. I.7. | Responsable del proyecto |
