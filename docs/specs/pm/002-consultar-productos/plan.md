...
- **`links` viaja en cada fila** (22-09-2026, `RN-PM-048`; **era `videoUrl`, un campo de la fila, hasta ese día**) y **tampoco es un filtro**. Lo que cambia respecto del campo viejo es que **ya no sale del `SELECT` del listado**: los enlaces están en otra tabla, de modo que `ProductRow` pierde `videoUrl` y el servicio hace **una segunda sentencia** —`WHERE product_id IN (:ids)` sobre los identificadores de la página ya resuelta— y los reparte en memoria por producto.
- **Una sentencia más por página, y ninguna por fila.** Es la forma que esta consulta ya usa para la conversión (§4): el coste no depende del tamaño de la página, y un `JOIN` contra `product_links` habría sido la alternativa obvia y peor — multiplicaría las filas del producto por sus enlaces y obligaría a deduplicar en memoria justo lo que la paginación acaba de acotar, con `LIMIT` contando filas que no son productos. **`CA-PM-386` es lo que impide que eso se convierta en un `N+1` sin que nadie lo note.**
- **Los enlaces se devuelven CRUDOS** (`RN-PM-049`): `ProductLinkResponse` con la dirección tal cual y `externalId` en su campo, **sin componer**. Es una lectura de administración —la misma que devuelve `purchasePrice`— y quien la usa es quien va a mandar el `PATCH` de `RF-PM-004`. **Y trae el `CUPON_BOT`** (`RN-PM-050`): esta consulta **no filtra por tipo**, al revés que la oferta y los dos hotlinks, y es deliberado — el filtro por tipo de aquellas es lo que impide regalar la prestación, y aquí no hay nada que impedir.
...
| Los enlaces en cada fila | API | Un producto con los dos enlaces y otro sin ninguno en la misma página: el primero los trae **crudos y con su identificador**, `CUPON_BOT` incluido; el segundo, `links` **presente y vacía** (`CA-PM-223`, `CA-PM-385`) |
| Los enlaces no son un `N+1` | Integración | Veinte productos con enlaces en una página: **una sentencia** para la página y **una** para sus enlaces, y el total **no crece** con el tamaño de la página (`CA-PM-386`) |
