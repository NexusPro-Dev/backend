...
| `T-42` | **La tabla de los enlaces, y la columna que se va** (`plan.md` §2.10): migración `V35__pm_enlaces_de_producto.sql` — crea `product_links` con `pk_product_links (product_id, type)`, los cuatro `CHECK` —incluido el cruzado `ck_product_links_id_sin_consulta`— y `fk_product_links_product` sin `ON DELETE`; **copia cada `video_url` no nula** a una fila `VIDEO_PRESENTACION`; y solo entonces borra `ck_products_video_url_format` y la columna. Los tres pasos **en el mismo archivo**, porque Flyway lo aplica en una transacción y separarlos dejaría el mismo enlace en dos sitios sin decir cuál manda | `T-41` | Sobre un esquema con productos **con y sin** video: tantas filas `VIDEO_PRESENTACION` como direcciones había, ninguna para los que no tenían, y `products.video_url` **ya no existe**. `SchemaIT` y el arranque de la suite | **Hecha el 22-09-2026** |
| `T-43` | **El enlace como modelo de dominio**: `ProductLinkType` —`VIDEO_PRESENTACION`, `CUPON_BOT`— y `ProductLink` con clave compuesta, que **recorta** la dirección y el identificador, comprueba `VAL-017`, `VAL-021`, `VAL-022` y el cruzado `VAL-023`, y sabe **resolverse** (`RN-PM-049`): dirección sin barra final + `/` + identificador, y **tal cual** cuando no lo hay. `ProductLinkRepository` con la lectura de un producto, la de un **lote** y el reemplazo del conjunto. **La composición vive aquí y en ningún otro sitio**: seis lecturas la usarán, y escrita dos veces un día dejarían de coincidir | `T-42` | Unitarias sobre `ProductLink`: las cuatro validaciones y **la resolución en sus cuatro casos** —con identificador, sin él, con barra final —que no se duplica—, y sin ella | **Hecha el 22-09-2026** |
| `T-44` | **El alta declara los enlaces**: `ProductLinkRequest` y `ProductLinkResponse`; `RegisterProductRequest`/`Command` cambian `videoUrl` por `links`, y `ProductResponse` también; `RegisterProductService` comprueba **el tipo repetido sobre el cuerpo y antes de escribir** (`VAL-020`), construye los `ProductLink` y los persiste en la misma transacción que el producto; la respuesta los devuelve **crudos y sin componer**; y la instantánea de auditoría de `Product.create` lleva los enlaces y **deja de llevar `video_url`** | `T-43` | `CA-PM-219` a `CA-PM-222` reescritos y `CA-PM-380` a `CA-PM-384` en `ProductsIT` | **Hecha el 22-09-2026** |
| `T-45` | **La prosa y el contrato**: la `@Operation` del alta dice qué es cada tipo de enlace, que **hay uno por tipo**, que el identificador **se pega al final** y que con identificador la dirección no admite `?` ni `#`; `ProductLinkResponse` lleva `@Schema(name = ...)` explícito para que no se funda con ningún registro homónimo de otro módulo; y el contrato se regenera. **La prosa no se regenera sola** — el esquema sí | `T-44` | `OpenApiContractIT`, y el `diff` de `docs/api/openapi.json` revisado: `links` en seis respuestas, `videoUrl` en ninguna | **Hecha el 22-09-2026** |
...
| Criterio | Tareas |
|---|---|
| `CA-PM-219` a `CA-PM-222` (reescritos), `CA-PM-380` a `CA-PM-384` | `T-42`, `T-43`, `T-44` |
