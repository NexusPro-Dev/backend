...
| Producto | Código, tipo, nombre, descripción, icono, **el enlace del video**, **la dirección de la portada**, vigencia en días |
...
| `CA-PM-238` | La respuesta lleva **`coverImageUrl`** sin token, con la forma `/api/v1/product-images/{uuid}` cuando hay portada y **presente y nulo** cuando no; la dirección **responde `200` sin token** en `RF-PM-016`; y la respuesta sigue sin llevar `purchasePrice` ni el identificador del producto |
