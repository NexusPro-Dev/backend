...
| `RN-PM-048` | **Los enlaces viven aparte, y hay uno por tipo** — aquí, que el producto del hotlink trae su colección, **vacía** cuando no publica ninguno | `requirements/pm.md` §5.1 |
| `RN-PM-049` | **El identificador se pega al final** — aquí, que el hotlink publica el enlace **RESUELTO**: quien abre un hotlink no administra nada | `requirements/pm.md` §5.1 |
| `RN-PM-050` | **El cupón del bot es entrega, no material de venta** — aquí, **con el peso que tiene en una ruta sin token**: publicarlo sería regalar por un enlace repartido lo que la compra concede. El tipo va **en el predicado** | `requirements/pm.md` §5.1 |
| `RN-PM-033` | **La portada es un archivo y se publica por su identificador** — el hotlink publica su dirección sin token, y es la única lectura en la que **la dirección y la imagen se sirven las dos sin credencial**; lo que eso significa está en `pm.md` §5.2.9 | `requirements/pm.md` §5.1 |
...
| Producto | Código, tipo, nombre, descripción, icono, **sus enlaces publicables** —el video, **resuelto**; nunca el cupón (22-09-2026; era «el enlace del video»)—, **la dirección de la portada**, vigencia en días |
...
| `CA-PM-229` | La respuesta lleva **`links`** sin token, y **presente y vacía** cuando el producto no publica ninguno — y sigue sin llevar `purchasePrice`. **Reescrito el 22-09-2026**: hasta ese día comprobaba `videoUrl` presente y nulo |
| `CA-PM-396` | Un producto que declara **los dos** enlaces se publica **solo con el video**: el `CUPON_BOT` **no aparece en el cuerpo**, comprobado sobre la respuesta entera. Es el criterio que impide regalar la prestación **a cualquiera y sin token** |
