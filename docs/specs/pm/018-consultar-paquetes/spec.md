...
La envoltura de página del sistema, y en `content` cada paquete con: `id`, `code`, `name`, `currency`, `scope`, `status`, **`itemCount`**, `listPrice`, `price`, `savings`, `exchange`, `offerable`, `createdAt`, `deletedAt` (`NON_NULL`).
