...
| `T-07` | `interfaces`: `GET /api/v1/products/available`, **sin permiso**, solo autenticado | `T-05` | Responde a un actor autenticado sin ninguna autoridad (`CA-PM-065`) | **Hecha** |
...
- [x] Toda escritura emite su evento de auditoría, en la transacción que corresponde. **No aplica: este requerimiento no escribe nada** (`plan.md` §7).
- [x] Los endpoints nuevos declaran su permiso. **Este declara que no lleva**, con su motivo, en la lista blanca de `EndpointPermissionsIT`: la excepción queda escrita donde alguien la lee, en lugar de pasar inadvertida.
