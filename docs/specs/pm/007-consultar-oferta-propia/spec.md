...
| Enmendada el | 22-09-2026 — **`links` sustituye a `videoUrl`, RESUELTOS y sin el `CUPON_BOT`** (`RN-PM-048` a `RN-PM-050`): el video sigue publicándose y el cupón **no**, porque es entrega y no material de venta. Ver §15 |
...
| Enmendada el | 15-09-2026 — **la oferta filtra por alcance por primera vez**: solo `TIENDA` y `AMBOS` (`RN-PM-019` reescrita). Ver §15 |
| Enmendada el | 15-09-2026 — **`packages` queda construida**, el alcance del paquete filtra igual, y cuesta una sentencia. Ver §15 |
| Enmendada el | 16-09-2026 — **`RN-PM-044` se reescribe**: un paquete lleva un upgrade como máximo (`RN-PM-046`), y se ofrece a quien tiene el origen de **su** upgrade. Sin cambio de comportamiento ni de contrato. Ver §15 |
| Enmendada el | 16-09-2026 — **cada paquete de `packages` trae su propia `coverImageUrl`** (`RN-PM-045`, `RF-PM-028`). Ver §15 |
| Enmendada el | 16-09-2026 — **cada paquete de `packages` trae `validFrom` y `validTo`, y uno fuera de su vigencia no aparece** (`RN-PM-047`). Ver §15 |
...
| `RN-PM-032` | **Un producto puede enlazar un video, y el enlace sale en toda lectura** — la oferta lo publica: es lo que se quiere que vea quien compra. **Dentro de `links` desde el 22-09-2026** | `requirements/pm.md` §5.1 |
| `RN-PM-048` | **Los enlaces viven aparte, y hay uno por tipo** — aquí, que cada producto ofrecible trae su colección, **vacía** cuando no publica ninguno | `requirements/pm.md` §5.1 |
| `RN-PM-049` | **El identificador se pega al final** — aquí, que la oferta publica el enlace **RESUELTO**: quien mira la oferta lo abre, no lo edita | `requirements/pm.md` §5.1 |
| `RN-PM-050` | **El cupón del bot es entrega, no material de venta** — aquí, que la oferta **no lo publica**, y el filtro por tipo va **en el predicado** de la consulta, no después | `requirements/pm.md` §5.1 |
...
| `RN-PM-044` | **(Desde el 15-09-2026, reescrita el 16-09-2026)** El upgrade del paquete decide a quién se ofrece — `RN-PM-011` aplicada al paquete; sin upgrade, a todo el mundo | `requirements/pm.md` §5.1 |
| `RN-PM-046` | **(Desde el 16-09-2026)** Un paquete lleva un upgrade como máximo — lo garantiza `RF-PM-023`, y es lo que hace que aquí baste mirar **uno** | `requirements/pm.md` §5.1 |
| `RN-PM-047` | **(Desde el 16-09-2026)** Fuera de su vigencia el paquete se oculta — aquí, **no aparece**, y las fechas viajan en los que sí | `requirements/pm.md` §5.1 |
...
| Dato | Descripción |
|---|---|
| Productos ofrecibles | Identificador, código, tipo, nombre, descripción, **sus enlaces publicables** —el video, **resuelto**; nunca el cupón (22-09-2026; era «el enlace del video»)—, **la dirección de la portada**, **el precio** con su moneda y **vigencia en días**. **El precio no se ajusta por nivel**: dos personas ven el mismo importe para el mismo producto |
...
| **`packages`** (desde el 15-09-2026) | La **tercera colección**, envuelta como las otras dos: los paquetes que hoy se pueden ofrecer **a esta persona**. Cada uno con código, nombre, descripción, **`coverImageUrl`** —la portada del paquete, desde el 16-09-2026, presente y nula cuando no hay—, **`validFrom` y `validTo`** —la vigencia, desde el 16-09-2026, el fin presente y nulo cuando es indefinido, para que quien compra sepa hasta cuándo vale—, moneda, `items` —el producto en **la misma forma que los productos de la oferta**, `discount` y **`priceInPackage`**—, `listPrice`, `price`, `savings` y `exchange` de `price`. **Sin `purchasePrice` en ningún nivel** (`RN-PM-043`). Ordenados por **fecha de alta** |

**Qué paquetes entran (desde el 15-09-2026).** Un paquete aparece si **se puede ofrecer hoy** —activo, vivo, con descripción, con dos productos al menos, **dentro de su vigencia** (`RN-PM-047`, desde el 16-09-2026: hoy no es anterior a `validFrom` ni posterior a `validTo`), y **todos** ellos activos y vivos (`RN-PM-039`, `RN-PM-040`)— **y** si esta persona puede comprarlo entero: **su upgrade sale de la membresía del actor** (`RN-PM-044`, que es `RN-PM-011` aplicada al paquete; desde el 16-09-2026 no hay más que uno, `RN-PM-046`), y un paquete **solo de bots** se ofrece a todo el mundo, como los bots — también a quien no tiene membresía. **El alcance del paquete filtra como el del producto** (desde el 15-09-2026, con los cuatro valores): entran `TIENDA` y `AMBOS`; `HOTLINK` y `NINGUNO` no. **El alcance de sus productos no filtra dentro del paquete**: el canal lo decide el paquete. **Un paquete que no se puede ofrecer no aparece, y nada lo dice**: quien tiene que saberlo lo sabe por el detalle administrativo (`RF-PM-019`), con `offerableReason`.
...
| ~~`CA-PM-124`~~ | ~~El sistema ofrece los productos de los dos alcances: uno de `TIENDA` y otro de `HOTLINKS` aparecen los dos, porque la escala es acumulativa y esta consulta no filtra por ella~~ **Invertido el 15-09-2026**: la escala dejó de existir y la oferta filtra (`CA-PM-351`) |
...
| `CA-PM-228` | Cada producto de la oferta devuelve **`links`** con su video, y **presente y vacía** cuando no publica ninguno — **sin que `purchasePrice` aparezca**, que sigue ausente (`CA-PM-160`). **Reescrito el 22-09-2026**: hasta ese día comprobaba `videoUrl` presente y nulo |
| `CA-PM-394` | Un producto que declara **los dos** enlaces se ofrece **solo con el video**: el `CUPON_BOT` **no aparece en el cuerpo**, comprobado sobre la respuesta entera y no solo sobre los campos esperados. Y un producto que **solo** declara cupón se ofrece con `links` **vacía**, no ausente |
| `CA-PM-395` | El enlace llega **resuelto**: un video con identificador externo se publica con el identificador **pegado al final**, y uno cuya dirección termina en `/` **no duplica la barra**. Sin identificador, la dirección viaja **tal cual** |
...
| `CA-PM-336` | La oferta **no devuelve** un paquete inactivo, retirado, con menos de dos productos, ni con un producto **inactivo o retirado** dentro, **y lo vuelve a devolver** cuando ese producto se reactiva; el alcance **del paquete** filtra —`TIENDA` y `AMBOS` entran, `HOTLINK` y `NINGUNO` no— y el de **sus productos** no |
...
| `CA-PM-338` | Con tres paquetes ofrecibles en dos monedas distintas, el número de sentencias sube **en una como máximo** respecto de la oferta sin paquetes: ni una por paquete ni una por moneda |
| `CA-PM-351` | El sistema ofrece **solo los productos de alcance `TIENDA` y `AMBOS`**: un `HOTLINK` y un `NINGUNO` activos y desde la membresía del actor **no aparecen** —ni en `upgrades` ni en `services`—, y la respuesta sigue publicando el `scope` de cada uno |
| `CA-PM-378` | Cada paquete de `packages` devuelve **`validFrom` y `validTo`**, el fin presente y nulo; y un paquete activo con todo en regla cuya vigencia **empieza mañana** o **terminó ayer** **no aparece**, mientras que uno que **termina hoy** sí (16-09-2026) |
| `CA-PM-370` | Cada paquete de `packages` devuelve **su propia `coverImageUrl`** —la del paquete, no la de sus productos— con la forma `/api/v1/product-images/{uuid}` cuando tiene portada, y **presente y nula** cuando no, sin que el número de sentencias suba (16-09-2026) |
...
| 0.14.0 | 15-09-2026 | **La oferta filtra por alcance por primera vez** (`RN-PM-019` reescrita, [`requirements/pm.md`](../../../requirements/pm.md) v0.35.0 §5.2.11): solo `TIENDA` y `AMBOS`. Hasta hoy **no podía** —bajo la escala acumulativa los dos valores llegaban a la tienda—; con cuatro valores explícitos, `HOTLINK` y `NINGUNO` no son de la tienda. **`CA-PM-124` se invierte** y lo sustituye `CA-PM-351`. El predicado es la única línea que cambia en la sentencia. | Responsable del proyecto |
| 0.15.0 | 15-09-2026 | **La colección `packages` queda construida** (`RF-PM-019` `T-11`/`T-12`; `PackageOfferIT`), y dos precisiones de Art. I.7 al construirla: **el alcance del paquete sí filtra** —`TIENDA` y `AMBOS` entran, con los cuatro valores de [`requirements/pm.md`](../../../requirements/pm.md) v0.35.0—, aunque el de sus productos no; y **cuesta una sentencia y no dos**, porque los paquetes y sus líneas vienen en la misma proyección pública que el hotlink del paquete (`CA-PM-338` reescrita: «en una como máximo»). `ProductOfferIT` sube su techo de sentencias de cuatro a cinco. | Responsable técnico |
| 0.16.0 | 16-09-2026 | **Cada paquete de `packages` trae su propia `coverImageUrl`** (`RN-PM-045`, [`requirements/pm.md`](../../../requirements/pm.md) v0.37.0 §5.2.12), con el mismo trato que la de cada producto: material de venta, sin consulta más, presente y nula cuando no hay. `CA-PM-370`. Enmienda que construye `RF-PM-028` (Art. I.7). | Responsable del proyecto |
| 0.17.0 | 16-09-2026 | **`RN-PM-044` se reescribe** ([`requirements/pm.md`](../../../requirements/pm.md) v0.38.0 §5.2.10): un paquete lleva **un upgrade como máximo** (`RN-PM-046`, comprobado en `RF-PM-023`), y la oferta lo enseña a quien tiene el origen de **ese** upgrade. **Ni el comportamiento ni el contrato cambian**: el filtro ya miraba que todos los upgrades salieran de la membresía del actor, y con uno solo mira ese. Cambia la letra de §5 y §6. | Responsable del proyecto |
| 0.18.0 | 16-09-2026 | **Cada paquete de `packages` trae `validFrom` y `validTo`, y uno fuera de su vigencia no aparece** (`RN-PM-047`, [`requirements/pm.md`](../../../requirements/pm.md) v0.39.0 §5.2.13): la vigencia es un motivo más de `RN-PM-039`, decidido por el mismo `PackageOfferability` con **hoy** en UTC, y las fechas viajan porque quien compra tiene que saber hasta cuándo vale — material de venta, no un costo. `CA-PM-378`. Se construye con `RF-PM-019` `T-13`, como las demás enmiendas de `packages`. | Responsable del proyecto |
| 0.13.0 | 22-09-2026 | **`links` sustituye a `videoUrl`: resueltos, y sin el `CUPON_BOT`** (`RN-PM-048` a `RN-PM-050`, [`requirements/pm.md`](../../../requirements/pm.md) v0.43.0 §5.2.14). La oferta sigue publicando el video **por el mismo motivo por el que lo publicaba** —es material de venta, y al revés que el precio de compra sí se selecciona— y **no publica el cupón del bot**, que es su contrario exacto: lo que la compra concede, y enseñarlo antes de cobrar lo regala. **El filtro por tipo va en el predicado de la consulta y no en Java**: filtrarlo en memoria funcionaría igual hasta el día que alguien reutilice la proyección, y ese día el fallo sería el de `RN-PM-050` — no falla, publica. La segunda defensa es la de siempre en este requerimiento, una **ausencia**: `OfferItem` no tiene dónde poner un cupón, como no tiene dónde poner `purchasePrice`. **Y los publica resueltos** (`RN-PM-049`), con el identificador externo ya pegado, porque quien mira la oferta abre el enlace y no lo edita; la composición vive en `ProductLink` y no en el DTO ni en la consulta, de modo que las seis lecturas que la usan no pueden discrepar. `CA-PM-228` se **reescribe** y nacen **`CA-PM-394`** —el cupón no aparece en el cuerpo entero— y **`CA-PM-395`** —el resuelto en sus tres formas, incluida la barra final que no se duplica—. Enmienda de Art. I.7. | Responsable del proyecto |
