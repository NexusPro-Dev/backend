...
- **El filtro por tipo va EN EL PREDICADO, y no después en Java** (`RN-PM-050`). La consulta de los enlaces de la oferta lleva `AND type <> 'CUPON_BOT'` —o, lo que es lo mismo escrito del derecho, `type = 'VIDEO_PRESENTACION'`—, de modo que **el cupón no sale nunca de la base**. Filtrarlo en memoria funcionaría igual **hasta el día que alguien reutilice la proyección**, y el fallo de ese día es el de `RN-PM-050`: no falla, publica. Es la misma decisión que `RN-PM-024` sostiene con una ausencia — `OfferItem` **no tiene dónde poner** un cupón.
- **Y llegan RESUELTOS** (`RN-PM-049`): el identificador externo **ya pegado**, porque quien mira la oferta abre el enlace y no lo edita. La composición la hace `ProductLink` —la misma función que usarán los dos hotlinks y `RF-MV-014`—, y no el DTO ni la consulta: escrita en cada sitio, un día dejarían de coincidir.
...
| **Los enlaces publicables** | API | Un producto con enlaces y otro sin ninguno en la misma oferta: `links` con el video y `links` **vacía**, y en la misma respuesta **sigue sin haber `purchasePrice`** (`CA-PM-228`) |
| **El cupón no sale** | API | Un producto con **los dos** enlaces: el cuerpo entero **no contiene** el `CUPON_BOT` ni su dirección (`CA-PM-394`) |
| **El enlace resuelto** | API | Con identificador —pegado al final—, con dirección terminada en `/` —**una sola barra**— y sin identificador —tal cual— (`CA-PM-395`) |
