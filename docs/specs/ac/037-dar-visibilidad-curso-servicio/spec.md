...
| `CA-AC-219` | **Enmienda de `RF-AC-008` (`RN-AC-015`), reescrita el 25-09-2026**: un curso `ACTIVO`, con las dos descripciones y un módulo activo con lección activa con contenido, **se ofrece sin llaves** —es de todos— y **sigue ofreciéndose** al recibir su primer servicio, que cambia a quién se abre; el motivo «sin membresías ni servicios» ya no existe |
...
| 0.3.0 | 25-09-2026 | **Construida** (`V43`, `CourseProductVisibilityIT` (10), `CourseOfferabilityTest`). Dos precisiones: **`CourseOfferability` recibe la suma de las dos llaves** —membresías y servicios— y su cuarto motivo se llama `SIN_LLAVES`; y **`CA-AC-222`** se comprueba con `products:update`, que no habilita. El servicio retirado después sigue en `products` del detalle, y su fila en la instantánea del retiro. Las suites que comparaban el texto viejo del cuarto motivo se actualizaron. | Responsable técnico |
| 0.4.0 | 25-09-2026 | **Enmienda por decisión del responsable del proyecto** (`ac.md` §5.2.12, `RN-AC-015` reescrita): **un curso sin membresías ni servicios es de todos** y las llaves dejan de ser motivo de la ofrecibilidad, que queda en **cuatro** —retirado, inactivo, sin descripción, sin módulo ofrecible—. `CA-AC-219` se reescribe. | Responsable técnico |
