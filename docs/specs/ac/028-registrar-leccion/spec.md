...
| 0.6.0 | 25-09-2026 | **La enmienda de §5.2.11 está construida**: `LessonVideoDurationIT` (`CA-AC-233` a `CA-AC-235`), `VideoLinkTest` y `ProviderVideoDurationLookupTest`. El reconocimiento del enlace y la consulta viven en `shared/video` (`VideoLink`, `ProviderVideoDurationLookup`); la lección los usa por `LessonDurationReader`, que convierte el fallo en `EX-003` sobre `durationSeconds`. **Ninguna prueba sale a internet**: la suite apunta los proveedores a un puerto local que rechaza la conexión. | Responsable técnico |
