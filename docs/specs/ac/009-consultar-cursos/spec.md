...
| 0.3.0 | 25-09-2026 | **Enmienda de `RF-AC-016`, construida** (Art. I.7): `categories` de cada fila son reales y `categoryId` acota —**solo por una categoría viva**: filtrar por una retirada devuelve vacío, porque ninguna fila la enseñaría—. **Tres** sentencias fijas por página (`CA-AC-048`). Probado en `CourseListIT` y `CourseClassificationIT` (`CA-AC-128`). | Responsable técnico |
