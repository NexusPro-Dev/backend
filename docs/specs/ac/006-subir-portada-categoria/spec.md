...
| 0.2.0 | 25-09-2026 | **Construida** (`V44`, `AcademyCoverIT`), adelantada del bloque 5 porque la portada del curso la necesita (`ac.md` §5.2.9). `ImageSignature` y `CambioDePortada` viven en `shared/images` y `ImageSignature.validar` lleva los tres `VAL`; `ProductImage.de` delega, con las suites de imágenes de `PM` en verde. La secuencia vive en `CoverUploader` desde el primer día, que es lo que el plan de `RF-AC-014` pedía extraer. Sin enmiendas de comportamiento. **Un pendiente de permisos**: `course-categories:update` gobierna desde hoy la corrección y la portada, y la regla de un permiso por operación (`RN-SEG-014`) lo detectó; entra en `REPARTO_PENDIENTE` de `EndpointPermissionsIT` con su motivo, porque `V28` no sembró un código de portada para `AC`: el tramo 3 de `RF-SP-060` crea `course-categories:set-cover` y `courses:set-cover`, como `products:set-cover`. | Responsable técnico |
