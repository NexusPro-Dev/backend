...
| 0.2.0 | 25-09-2026 | **Construida** (`CourseClassificationIT` (11), que reúne también `RF-AC-017` y las enmiendas). `V42` y no `V25`: el número se asignó al construir. Tres precisiones que la spec no fijaba: **la categoría se lee sin bloquear** con una lectura propia (`findAliveById`), porque `findAliveByIdForUpdate` la habría bloqueado contra §14.1; **el filtro `categoryId` del listado solo acota por una categoría viva** —filtrar por una retirada es vacío—, porque ninguna fila la enseña en `categories` y el resultado no cuadraría con la fila; y **el detalle del curso cuesta una sentencia más** —sus categorías—, con lo que `CA-AC-054` pasa de dos a tres sin módulos. `offerable` de los cursos de la categoría viaja y es falso siempre hasta `RF-AC-020` (`tasks.md` §4, bloqueo 2). | Responsable técnico |
