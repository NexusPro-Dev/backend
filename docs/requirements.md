...
| Requerimientos con `plan.md` aprobado | 29 |
| Requerimientos implementados | 0 |

Los 42 corresponden al módulo `SP`, que absorbió los usuarios, sus roles y su acceso al retirarse `USR` (`modules.md` v0.9.0). **Los cuarenta y dos tienen `spec.md` y los cuarenta y dos han superado la primera compuerta** del Art. I.6: treinta y ocho el 21-08-2026, y `RF-SP-041`, `RF-SP-039`, `RF-SP-042` y `RF-SP-040` el 22-08-2026. No queda ninguna pregunta abierta en el módulo. **Seis requerimientos superaron la segunda compuerta el 22-08-2026** —`RF-SP-024` a `RF-SP-029`, todo el submódulo de Usuarios—, con lo que **restan trece `plan.md`**: de `RF-SP-030` a `RF-SP-042`, y uno de ellos —el de `RF-SP-040`— no puede escribirse hasta cerrar **D-23**.
