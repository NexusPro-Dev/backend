...
**`package_id` dice de qué paquete salió la línea, y es nulo si el producto se compró suelto** (16-09-2026). Comprar un paquete será **un movimiento con una línea por producto**, todas con el mismo `package_id` y cada una con el descuento que el paquete le declara (`RN-PM-036`, `RN-PM-037`); es una **referencia y no una copia**, con el criterio de `RN-MV-002`: el paquete no desaparece —se retira—, y lo que sí puede cambiar de él (el descuento de cada producto, `RF-PM-024`) queda congelado en la línea y en sus descuentos. Comprar paquetes sigue siendo otro requerimiento; hoy solo nace la columna, y ninguna entrada la escribe.
