...
**Y desde el 22-09-2026 un producto ENLAZA, y sus enlaces viven aparte.** Lo que hasta hoy era una columna —la dirección de un video— pasa a ser una fila de `product_links` con su **tipo**, y los tipos son dos: **`VIDEO_PRESENTACION`**, el video que presenta el producto, y **`CUPON_BOT`**, la dirección donde quien ya compró un bot registra la cuenta que ese bot le da. Cada enlace lleva además un **identificador externo** opcional —un dato de un sistema ajeno que NEXUS guarda y devuelve sin interpretar— que **se pega al final de la dirección** al publicarla. Y el tipo decide **dónde se ve**: el video es material de venta y sale donde salía; el cupón **es entrega**, y solo aparece en el registro de lo comprado cuando la línea está `ENTREGADA` (§5.2.14).
...
- **Enlazar un producto**: hasta **un enlace por tipo** —el video que lo presenta y el cupón del bot—, con su dirección y un identificador externo opcional, declarados al registrar y corregidos al editar. **No estrenan endpoint ni permiso**, y **cada tipo se publica donde le toca** (§5.2.14).
...
| `RN-PM-032` | **Un producto puede enlazar un video, y el enlace sale en toda lectura** | Al registrar, al editar y siempre que se consulte un producto, con token o sin él | Todo producto —**de los dos tipos**— puede declarar **la dirección de un video** que lo presenta: una **URL absoluta `http` o `https`, sin espacios y de hasta 500 caracteres**, de cualquier dominio. Es **opcional**, se corrige y **se quita** (`RF-PM-004`), su ausencia significa «no tiene video» y **no condiciona la activación**. **Es un enlace, no un archivo**: el sistema comprueba su forma y **no lo sigue** —no comprueba que exista, no lo descarga, no lo incrusta—. Las **cuatro** lecturas lo devuelven, **incluido el hotlink sin token**: es material de venta, no un costo (§5.2.8). **Desde el 22-09-2026 no es una columna: es el enlace de tipo `VIDEO_PRESENTACION`** de `product_links` (`RN-PM-048`) y viaja dentro de `links`. Lo que esta regla decide —su forma, que sea opcional, que valga en los dos tipos y que salga en las cuatro lecturas— **no cambia ni una coma**; cambia dónde se guarda y cómo se lee, y con ello la forma del nulo: donde había un campo **presente y nulo** ahora hay una **colección sin esa entrada** (§5.2.14) | Media |
...
| `RN-PM-048` | **Los enlaces de un producto viven aparte, y hay uno por tipo** | Al registrar, al editar y en toda lectura que publique enlaces | Un producto tiene **hasta un enlace por tipo** en `product_links`, y los tipos son **dos**: `VIDEO_PRESENTACION` —el video que lo presenta (`RN-PM-032`)— y `CUPON_BOT` —la dirección donde quien ya compró registra la cuenta que el bot le da—. Cada fila lleva una **dirección obligatoria**, con la forma de siempre —`http` o `https`, sin espacios, hasta 500 caracteres, de cualquier dominio— y un **identificador externo opcional** de hasta 100 caracteres sin espacios, que es un dato **de un sistema ajeno**: NEXUS lo guarda y lo devuelve, y **no lo interpreta**. Los dos tipos valen en los **dos tipos de producto**, son **opcionales**, **no condicionan la activación** y **no estrenan endpoint ni permiso**: se declaran en el alta y se corrigen en la edición (`RF-PM-001`, `RF-PM-004`). Repetir un tipo en la misma petición **se rechaza** —la clave es la pareja producto-tipo, y el esquema no admite el segundo—, y un tipo desconocido también (§5.2.14) | Alta |
...
**`RN-PM-050` — el cupón solo se publica a quien lo compró y ya lo tiene entregado.** Es crítica por las dos formas en que incumplirla **no falla**. Publicarlo de más —en la oferta, en el hotlink, en el catálogo público— **regala la prestación**: el cupón *es* lo que se vendió, de modo que enseñarlo sin token convierte una venta en un enlace que cualquiera reparte, y nadie ve un error porque todas las peticiones responden `200`. Publicarlo antes de tiempo —en una línea `PENDIENTE` o `RETENIDA`— **entrega sin autorización**, que es el defecto que `RN-MV-021` describe y que aquí entraría por la puerta de atrás: no por confirmar mal una venta, sino por seleccionar una columna de más en una lectura. Lo que la sostiene no es una restricción sino una **ausencia**, como en `RN-PM-024`: las proyecciones de la oferta y del hotlink **no tienen dónde poner** un `CUPON_BOT`, y la de `RF-MV-014` lo selecciona **con el estado de la entrega en el predicado** y no después, en Java (§5.2.14).
...
!!! info "Enmendada el 22-09-2026: todo esto sigue siendo verdad, pero el video ya no es una columna"

    Las cuatro respuestas de esta sección **no se revisan**: el video se ve en las cuatro lecturas, se valida solo en su forma, vale en los dos tipos y es opcional. Lo que cambia es **dónde vive** — una fila de `product_links` con tipo `VIDEO_PRESENTACION` (`RN-PM-048`)— y, con ello, **la forma del nulo**: donde había un campo `videoUrl` presente y nulo ahora hay una colección `links` **sin esa entrada**, que es igual de distinguible y no obliga a inventar una fila vacía. El párrafo de arriba sobre el nulo es el único de esta sección que el cambio deja atrás, y §5.2.14 dice por qué.
...
### 5.2.14 Los enlaces del producto — 22-09-2026

**Decisión del responsable del proyecto.** Un producto deja de tener **un** enlace en una columna y pasa a tener **enlaces en una tabla**, cada uno con su tipo. Nacen dos: `VIDEO_PRESENTACION` —el video de §5.2.8, que no cambia de significado— y `CUPON_BOT` —la dirección donde quien ya compró un bot registra la cuenta que ese bot le da—. Lo que abrió la petición fue el cupón; lo que obligó a mover el video fue que **la segunda cosa de la misma forma es la que enseña que la forma era una tabla**. Seis cosas se preguntaron antes de escribir una línea:

| Pregunta | Decisión | Lo que se descartó, y por qué |
|---|---|---|
| **¿Una columna más, o una tabla?** | **Una tabla anexa, `product_links`**, y `video_url` se muda a ella | *Dos columnas más en `products`* —`coupon_url` y `coupon_external_id`— era lo más barato hoy y lo más caro cada vez: el tercer tipo de enlace sería otra migración, otro par de campos en cuatro respuestas y otra rama en el alta y en la edición. La tabla lo absorbe con una fila. *Dejar el video donde estaba y que la tabla guarde solo lo nuevo* — el video y el cupón son **la misma cosa** —una dirección con un tipo—, y guardarlos en dos sitios obliga a leerlos de dos maneras y a decidir dos veces cada regla que los toque |
| **¿Cuántos por producto?** | **Uno por tipo**, y el tipo va en la clave | *Varios del mismo tipo* — nadie ha pedido dos videos ni dos cupones, y admitirlo obliga a decidir **cuál se pinta** en cada lectura, que es una decisión sin dueño. El día que haga falta, la clave deja de ser la pareja y entra un orden; hoy sería una libertad que solo produce ambigüedad |
| **¿Dónde se declara el tipo?** | **En un `CHECK`**, como `type`, `scope` e `implementation` | *Una tabla catálogo administrable, con su CRUD y sus permisos* — un tipo de enlace **no es un dato**: cada uno trae consigo dónde se publica y qué significa, y las dos cosas son código. Un catálogo dejaría crear `CUPON_LOTERIA` desde la interfaz sin que nadie hubiera escrito qué hace el sistema con él, y la fila existiría sin que ninguna lectura la mirara |
| **¿Endpoints propios?** | **No: viajan dentro del alta y de la edición** | *`POST`, `PATCH` y `DELETE /products/{id}/links` con tres permisos* — es la forma de la portada (`RF-PM-014`, `RF-PM-015`), y allí está justificada porque la portada **son bytes** y no cabe en un JSON. Un enlace es texto: cabe, y quien registra un producto ya declara en la misma petición su icono, su precio y su alcance. Tres rutas nuevas serían tres permisos nuevos (`RN-SEG-015`) para algo que no es una operación aparte, sino un campo más de lo que ya se registra |
| **¿Dónde se ve el cupón?** | **Solo en lo comprado, y solo entregado** (`RN-PM-050`) | *Donde el video, en las cuatro lecturas* — sería **la prestación regalada**: quien abre un hotlink obtendría sin pagar el enlace que la compra concede. *En cuanto el pago se confirma* — entregaría lo comprado **sin la autorización** que `RN-MV-021` exige de un producto manual, que es el defecto que aquel requerimiento existe para impedir |
| **¿Qué es el identificador externo?** | **Un dato de un sistema ajeno: opcional, guardado tal cual y sin interpretar** | *Un identificador nuestro* — no lo es: lo emite quien aloja el bot o el video, NEXUS no lo genera, no lo valida contra nada y no lo usa como clave. *Obligatorio en el cupón* — hay cupones que son un enlace y nada más, y exigirlo dejaría sin registrar los que no lo tienen. *Único entre productos* — que dos productos apunten al mismo cupón puede ser un descuido o puede ser lo querido, y el esquema no sabe cuál de las dos cosas es |

#### El enlace resuelto, y por qué administración ve el crudo

**El identificador se pega al final de la dirección** (`RN-PM-049`): la dirección guardada sin su barra final, más `/`, más el identificador. `https://t.me/MiBot` con `CUPON_ORO_2026` se publica `https://t.me/MiBot/CUPON_ORO_2026`; sin identificador, la dirección sale **tal cual**. Es una regla de una línea a propósito: el sistema no analiza la dirección, no la normaliza, no sabe qué plataforma hay al otro lado y **no compone nada más**.

**Y administración ve el crudo.** `RF-PM-002` y `RF-PM-003` devuelven la dirección **como se guardó**, con su identificador al lado y en su propio campo; las demás lecturas devuelven **el resuelto**, que es el enlace que alguien abre. El motivo no es de gusto: quien edita un producto recibe del `GET` lo que va a mandar en el `PATCH`, y si administración viera el resuelto, corregir el enlace obligaría a **deshacer la composición a mano** o a que el `PATCH` aceptara una forma distinta de la que el `GET` enseña. Es la misma razón por la que el precio de compra se ve donde se corrige.

!!! warning "Con identificador, la dirección no puede llevar `?` ni `#`"

    Pegar un segmento de ruta detrás de una cadena de consulta —`https://t.me/MiBot?start=hola` más `/CUPON`— produce un enlace **roto que responde `200`**: la plataforma del otro lado recibe un parámetro que no entiende y enseña su página genérica, de modo que el fallo no se ve en ninguna traza y aparece semanas después como «el cupón no funciona». Por eso la incompatibilidad se rechaza **al escribir el enlace** y no al publicarlo, con un `CHECK` que la declara en el esquema (`ck_product_links_id_sin_consulta`): el único sitio donde hay alguien delante para corregirlo es el `POST` o el `PATCH` que lo escribe.

    Un enlace **sin** identificador sí admite `?` y `#`: nada se le pega, de modo que nada se rompe. La restricción es **cruzada**, como `ck_products_type_target`, y no una prohibición general sobre la columna.

#### Lo que cuesta, escrito entero

**`products.video_url` desaparece, y con ella `videoUrl` del contrato.** La migración copia cada dirección no nula a una fila de `product_links` con tipo `VIDEO_PRESENTACION` **antes** de borrar la columna y su `CHECK`, de modo que ningún producto pierde su video; las filas nuevas nacen con el `created_at` de la migración, que es cuando la fila existe, y no con el del producto, que sería una fecha inventada para una fila que no existía. Lo que sí cambia es **el contrato de seis lecturas**: donde había un campo `videoUrl` presente y nulo ahora hay una colección `links`, y el frontend que pinta el video tiene que leerla. Se aceptó a sabiendas: conservar `videoUrl` además de `links` habría dejado **la misma dirección publicada en dos sitios**, que es la clase de duplicado que un día deja de coincidir.

**Y es la primera vez que este módulo publica algo en función de una compra.** El cupón no lo lee ninguna lectura de `PM`: lo lee `RF-MV-014`, que es de `MV` y sabe si la línea está entregada. `PM` sigue sin saber qué se ha vendido —la frontera de §1.4 no se mueve—, y lo que hace es lo mismo que hizo con la implementación (§5.2.2): **declarar el dato y dejar que el otro módulo decida cuándo enseñarlo**. `MV` lo lee por la interfaz de solo lectura que `PM` ya publica, sin una consulta cruzada entre módulos.

!!! warning "Lo que se publica sigue siendo una dirección que alguien escribió"

    Vale lo que §5.2.8 aceptó para el video, y vale **más** para el cupón: el sistema comprueba la forma y no sigue el enlace, de modo que un cupón roto, caducado o que lleve a otro sitio se publica igual — y aquí quien lo recibe **ya pagó**. La confianza sigue estando en `products:create` y `products:update`, no en el dato. La diferencia con el video es que el error del video se ve al mirarlo y el del cupón se ve cuando alguien reclama, y por eso la única defensa que el sistema puede dar es la de arriba: rechazar en el alta lo que él mismo sabe que va a romper.
...
| Prioridad | **Crítica** |
| Reglas aplicables | `RN-PM-024`, `RN-PM-032`, `RN-PM-033`, `RN-PM-048`, `RN-PM-049`, `RN-PM-050` |
...
**Cada fila trae sus enlaces** (`RN-PM-048`, 22-09-2026), en una colección `links` **vacía** en los productos que no declaran ninguno. Hasta el 22-09-2026 era un campo `videoUrl` presente y nulo; ahora son **los dos tipos**, y esta es —con `RF-PM-003`— **una de las dos únicas lecturas donde se ve el `CUPON_BOT`** (`RN-PM-050`), porque es donde se administra. **Vienen crudos**, con su identificador externo al lado y sin componer, que es lo que `RF-PM-004` espera recibir de vuelta (`RN-PM-049`). **No es un filtro** ni una consulta por fila: los enlaces de toda la página se leen **en una sola sentencia** y se reparten en memoria, como la conversión.
...
**Y desde el 14-09-2026 devuelve el enlace del video** (`RN-PM-032`), que **desde el 22-09-2026 viaja en `links`** junto al cupón del bot si lo hay (`RN-PM-048`): **crudos, con su identificador y vacía la colección** cuando no hay ninguno. Es —con `RF-PM-002`— **la otra lectura donde el `CUPON_BOT` se ve** (`RN-PM-050`), y también en un producto retirado: quien administra tiene que poder revisar lo que se entregó. **Y la dirección de la portada**, `coverImageUrl` (`RN-PM-033`), con el mismo trato: presente y nula, sin consulta más, y también en un producto retirado.
...
**Los enlaces se corrigen en bloque** (`RN-PM-048`, 22-09-2026), y ahí se apartan de todo lo demás de esta operación: los otros campos se corrigen **uno a uno** —lo que no viaja no se toca—, y `links` viaja **entero o no viaja**. La colección que llega **es la que queda**: un tipo que estaba y no viene **se borra**, uno que viene y no estaba **se crea**, y uno que viene y estaba **se reescribe**. **`links` ausente no toca ningún enlace**; **`links` vacía los quita todos**, que es el nulo explícito de siempre convertido en colección y sigue siendo **una orden**. Se eligió el bloque y no tres operaciones sueltas porque la petición ya enseña el conjunto completo y compararlo con lo guardado es una sentencia por tipo, mientras que un `PATCH` que corrigiera un enlace y dejara los demás obligaría a nombrarlos uno a uno con un verbo por caso. Se corrige **en los dos tipos de producto**, sin la condición cruzada del icono, y con **las mismas comprobaciones que en el alta**: forma de la dirección, tope del identificador, tipo conocido, tipo no repetido y la incompatibilidad entre identificador y cadena de consulta (`RN-PM-049`). Corregirlos **no condiciona la activación** y queda en la auditoría como cualquier otro cambio.
...
| Prioridad | Alta |
| Reglas aplicables | `RN-PM-009`, `RN-PM-011`, `RN-PM-019`, `RN-PM-020`, `RN-PM-024`, `RN-PM-032`, `RN-PM-033`, `RN-PM-045`, `RN-PM-047`, `RN-PM-048`, `RN-PM-049`, `RN-PM-050` |
| Depende de | `RF-PM-001` |
...
**Publica el enlace del video** (`RN-PM-032`, 14-09-2026), en la colección `links` desde el 22-09-2026 y **resuelto** (`RN-PM-049`): el identificador externo ya pegado al final, porque quien mira la oferta abre el enlace y no lo edita. Es lo contrario del precio de compra: material de venta, que existe para que lo vea quien compra, y por eso **sí se selecciona** aquí. **El cupón del bot no** (`RN-PM-050`): la proyección de la oferta **no tiene dónde ponerlo** y la consulta **no lo selecciona**, porque publicarlo aquí sería entregar sin cobrar lo que la compra concede. **Y publica la dirección de la portada** (`RN-PM-033`), `coverImageUrl`, por lo mismo y con el mismo trato — la imagen la sirve `RF-PM-016` sin token, de modo que quien ve la oferta puede pintarla sin una segunda credencial.
...
| Prioridad | Alta |
| Reglas aplicables | `RN-PM-009`, `RN-PM-019`, `RN-PM-021`, `RN-PM-022`, `RN-PM-024`, `RN-PM-032`, `RN-PM-033`, `RN-PM-048`, `RN-PM-049`, `RN-PM-050` |
...
**Publica el enlace del video, sin token** (`RN-PM-032`, 14-09-2026): la dirección que administración escribió, **resuelta** desde el 22-09-2026 y dentro de `links` (`RN-PM-048`, `RN-PM-049`), y la colección **vacía** cuando no hay ninguno. El sistema no la sigue ni la valida más allá de su forma, y lo que eso significa en una ruta pública está escrito en §5.2.8. **Y no publica el cupón del bot** (`RN-PM-050`): es la lectura donde publicarlo sería más grave —sin token y por un enlace que se reparte a propósito—, de modo que la consulta **filtra por tipo en el predicado** y no después.
...
!!! note "Los enlaces no estrenan ruta, y es una decisión, no un olvido"

    Esta tabla **no cambia** el 22-09-2026. Los enlaces de un producto (`RN-PM-048`) entran por `POST /api/v1/products` y se corrigen por `PATCH /api/v1/products/{id}`, dentro del cuerpo, y por eso **no estrenan permiso**: con un permiso por operación (`RN-SEG-015`), tres rutas nuevas habrían sido tres permisos nuevos para algo que no es una operación aparte sino un campo más de lo que ya se registra. La portada tiene rutas propias porque **son bytes** y no caben en el JSON del alta (§5.2.9); un enlace es texto y cabe. **Lo que sí cambia es el cuerpo** de esas dos peticiones y el de seis respuestas, donde `videoUrl` deja su sitio a `links` (§5.2.14).
...
| `product_links` | Los enlaces de un producto, **uno por tipo**: el video que lo presenta y el cupón del bot, con su dirección y su identificador externo (22-09-2026, §10.7). **Recoge lo que era `products.video_url`** | Este módulo |
...
**`video_url` estuvo aquí entre el 14-09-2026 y el 22-09-2026**, `varchar(500)` y nula, con `ck_products_video_url_format`. **`V35` la migra a `product_links` y la borra** con su restricción (§10.7): el video pasó a ser un enlace **con tipo**, y lo que esta tabla guardaba de él —su forma, su tope de quinientos, que no hubiera cadena vacía— lo guarda ahora la tabla anexa **sin cambiar ni un umbral**. La regla que lo decide, `RN-PM-032`, tampoco cambió: cambió dónde vive el dato (§5.2.14).
...
| ~~`ck_products_video_url_format`~~ | `video_url IS NULL OR video_url ~ '^https?://[^[:space:]]+$'`. **Se va con su columna el 22-09-2026 en `V35`**: la comprobación no desaparece, se muda a `ck_product_links_url_format`, que la hace **sin la rama `IS NULL`** porque allí la dirección es obligatoria (§10.7) | `RN-PM-032` |
...
### 10.7 `product_links` — los enlaces del producto (22-09-2026)

| Campo | Tipo | PK | FK | Nullable | Default | Entidad relacional |
|---|---|---|---|---|---|---|
| `product_id` | `uuid` | **Sí** | Sí | No | — | `products` |
| `type` | `varchar(30)` | **Sí** | No | No | — | — |
| `url` | `varchar(500)` | No | No | No | — | — |
| `external_id` | `varchar(100)` | No | No | Sí | — | — |
| `created_at` | `timestamptz` | No | No | No | `now()` | — |
| `updated_at` | `timestamptz` | No | No | No | `now()` | — |

**La clave es la pareja producto-tipo, y no hay `id` propio**, como en `product_package_items` y por lo mismo: la fila **no es una entidad**, es el valor de un hueco que el producto tiene. Un identificador propio permitiría dos filas del mismo tipo sin que el esquema dijera nada, que es justo lo que `RN-PM-048` prohíbe, y obligaría a que las correcciones lo nombraran para algo que ya se nombra con el tipo. La contrapartida está escrita: el día que un producto necesite **dos** videos, la clave deja de ser la pareja y hay que decidir qué los ordena.

**Y no hay `deleted_at`**, también como en `product_package_items`: quitar un enlace **lo borra**. No es contenido de nadie —al revés que una reseña, que se retira y se cuenta—, no se referencia desde ninguna venta y su rastro está donde está el de todo lo demás que se corrige: en la auditoría del producto, con su antes y su después. Un borrado lógico aquí obligaría a que las seis lecturas y el índice único llevaran el predicado `WHERE deleted_at IS NULL` para no publicar un enlace retirado.

**`url` es `NOT NULL` y `external_id` admite nulo.** Una fila sin dirección no significa nada —no hay «enlace sin enlace»—, de modo que quitar el video es **borrar la fila** y no vaciar una columna; y esa es la diferencia con la columna que esta tabla reemplaza, donde el nulo era el estado «no tiene video». El identificador sí es opcional, y su nulo significa «este enlace no lleva ninguno», que es el caso corriente del video.

**Los topes son los de siempre y no se estrechan**: quinientos para la dirección, el mismo que tenía `products.video_url` y por el mismo motivo —un enlace que no cabe ahí no es uno que nadie vaya a escribir a mano—, y cien para el identificador, que es un código que alguien copia de otra plataforma. Subir cualquiera de los dos es un `ALTER` de solo metadatos en PostgreSQL; estrecharlos es una reescritura, y por eso se eligen anchos.

#### Restricciones exigidas en el esquema

| Restricción | Sobre | Regla que implementa |
|---|---|---|
| `pk_product_links` | `PRIMARY KEY (product_id, type)` | `RN-PM-048`. **La unicidad es la clave**, no un índice aparte: un producto tiene un enlace por tipo, y el segundo no entra. Por ser una restricción y no un índice parcial **admitiría `DEFERRABLE`**, y no se declara así: la colisión de dos altas simultáneas debe morder en el `INSERT` para que el caso de uso la traduzca, como en `RF-SP-019` |
| `ck_product_links_type` | `type IN ('VIDEO_PRESENTACION','CUPON_BOT')` | `RN-PM-048`. Los tipos son código, no catálogo (§5.2.14). Sin `DEFAULT`: un enlace sin tipo declarado sería el sistema decidiendo dónde se publica |
| `ck_product_links_url_format` | `url ~ '^https?://[^[:space:]]+$'` | `RN-PM-032`, `RN-PM-048`. Es `ck_products_video_url_format` mudada, **sin la rama `IS NULL`**: allí la columna admitía nulo y aquí es obligatoria, de modo que el `CHECK` no puede evaluar a `NULL` y no hay permiso que escribir explícito. Comprueba **la forma y nada más**; que el enlace resuelva a algo no es cosa del esquema (§5.2.8) |
| `ck_product_links_external_id_format` | `external_id IS NULL OR external_id ~ '^[^[:space:]]{1,100}$'` | `RN-PM-048`. La rama `IS NULL` va **delante y explícita**, como en el icono y en la vigencia: un `CHECK` que evalúa a `NULL` **acepta** la fila, y el permiso debe ser deliberado. Prohíbe la cadena vacía y los espacios —un identificador con un espacio partiría el enlace al pegarlo— y **nada más**: qué forma tiene el código de otra plataforma no lo sabe este sistema |
| `ck_product_links_id_sin_consulta` | `external_id IS NULL OR url !~ '[?#]'` | `RN-PM-049`. **Cruzada**, como `ck_products_type_target`: sin identificador la dirección admite `?` y `#` porque no se le pega nada; con identificador no, porque el segmento iría detrás de la cadena de consulta y el enlace saldría roto **respondiendo `200`** (§5.2.14) |
| `fk_product_links_product` | `product_id` → `products(id)` | El enlace es de un producto que existe. **Sin `ON DELETE`**: el producto no se borra físicamente nunca (`RN-PM-010`), de modo que no hay cascada que declarar, y declararla sugeriría que sí lo hace |

#### Lo que no se declara en el esquema

| Regla | Por qué no | Cómo se verifica |
|---|---|---|
| `RN-PM-049` — el enlace resuelto | Es **una lectura, no una restricción**: la dirección guardada es la cruda, y la resuelta se compone al publicarla. Una columna con lo resuelto sería la copia que un día deja de coincidir, como el precio del paquete (`RN-PM-036`) | En las lecturas de venta, con identificador y sin él, y con la dirección terminada en `/` y sin terminar: las dos publican **una sola** barra. Y en las de administración, comprobando que devuelven **la cruda** |
| `RN-PM-050` — dónde se ve cada tipo | Ninguna restricción puede declarar que una fila no se publique. Lo sostiene que las proyecciones de la oferta, del hotlink y de su catálogo **filtren por tipo en el predicado** y que `RF-MV-014` exija además el estado de la entrega | En la oferta y en los dos hotlinks, con un producto que tiene los dos enlaces: sale el video y **no** el cupón, comprobando el cuerpo entero y no solo los campos esperados. Y en `RF-MV-014`, con la línea `PENDIENTE`, `RETENIDA` y `ENTREGADA` |
| El tipo repetido en la misma petición | La clave lo impide, pero **el error que produce es de base de datos**: un `23505` que sale como `500` si nadie lo traduce | El caso de uso lo comprueba **antes de escribir**, sobre el cuerpo recibido, y responde `400` nombrando el tipo repetido. Prueba del alta y de la corrección con dos `VIDEO_PRESENTACION` |
| El tipo desconocido | El `CHECK` lo rechazaría, y también tarde | Lo rechaza la deserialización del tipo enumerado, con `400`. Prueba con un tipo inventado en el alta y en la corrección |
...
| 0.42.0 | 21-09-2026 | **`packages:buy` entra en el catálogo con recurso `packages`** (`RF-SP-062`, `RN-SEG-015`; [`requirements/mv.md`](mv.md) v0.30.0 §6): gobierna `POST /packages/{code}/purchases` (`RF-MV-012`), que hasta hoy se atendía con solo el token. Es una operación de `MV` sobre un recurso de `PM`, y se documenta allí; aquí queda anotado para que el recuento de códigos `packages:` no descuadre: **doce**, once de este módulo y uno de `MV`. | Responsable técnico |
| 0.43.0 | 22-09-2026 | **Un producto deja de tener UN enlace en una columna y pasa a tener ENLACES en una tabla, cada uno con su tipo.** Por decisión del responsable del proyecto, con **seis respuestas preguntadas antes de escribir** (§5.2.14). Nace **`product_links`** (§10.7) con la pareja `(product_id, type)` como clave —la unicidad **es** la clave, como en `product_package_items`, y por lo mismo: la fila no es una entidad sino el valor de un hueco del producto—, y con ella **`RN-PM-048`**, **`RN-PM-049`** y **`RN-PM-050`**, la última **crítica**. Los tipos son **dos y van en un `CHECK`**, no en un catálogo administrable: cada uno trae consigo dónde se publica y qué significa, y las dos cosas son código. **`VIDEO_PRESENTACION`** recoge lo que era `products.video_url`, que `V35` **migra fila a fila y borra** junto a `ck_products_video_url_format`; `RN-PM-032` **no cambia ni una coma** de lo que decide —forma, opcionalidad, los dos tipos de producto, las cuatro lecturas—, cambia dónde vive el dato y con ello la forma del nulo: donde había un campo `videoUrl` presente y nulo hay ahora una colección `links` **sin esa entrada** (§5.2.8, enmendada). **`CUPON_BOT`** es nuevo y es **la primera cosa de este módulo que se publica en función de una compra**: la dirección donde quien ya compró un bot registra la cuenta que ese bot le da, que **no sale del catálogo público, ni de la oferta, ni de los dos hotlinks** —enseñarla antes de cobrar es la prestación regalada— y sale **solo** en `RF-MV-014` y **solo en la línea `ENTREGADA`** (`RN-MV-030`), porque publicarla en una `PENDIENTE` sería entregar sin la autorización que `RN-MV-021` exige. Cada enlace lleva un **identificador externo opcional** —dato de un sistema ajeno, guardado y devuelto **sin interpretar**— que **se pega como último segmento de la dirección** al publicarla (`https://t.me/MiBot` + `CUPON_ORO_2026` = `https://t.me/MiBot/CUPON_ORO_2026`), sin duplicar una barra final; **administración ve el crudo** y todo lo demás el resuelto, porque quien edita tiene que recibir del `GET` lo que va a mandar en el `PATCH`. Y con identificador la dirección **no puede llevar `?` ni `#`** (`ck_product_links_id_sin_consulta`): pegar un segmento detrás de una cadena de consulta da un enlace roto que responde `200`, de modo que se rechaza donde hay alguien delante para corregirlo. **No estrenan ruta ni permiso**: viajan dentro de `POST /products` y `PATCH /products/{id}`, y en la corrección van **en bloque** —la colección que llega es la que queda; ausente no toca nada, vacía los quita todos—, al revés que los demás campos, que se corrigen uno a uno. Lo que cuesta, escrito: **el contrato de seis lecturas cambia**, porque `videoUrl` deja su sitio a `links`, y se acepta a sabiendas —conservar los dos habría dejado la misma dirección publicada en dos sitios—. Enmienda las tripletas de `RF-PM-001` a `RF-PM-004`, `RF-PM-007`, `RF-PM-008`, `RF-PM-026` y `RF-PM-027`, y la de `RF-MV-014` ([`requirements/mv.md`](mv.md) v0.35.0) (Art. I.7). | Responsable del proyecto |
