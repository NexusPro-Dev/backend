...
| `fk_user_products_movement_detail` | `user_products(movement_detail_id)` → `movement_details(id)`, `ON DELETE RESTRICT` — **desde el 23-09-2026**. Es una clave foránea de `SP` que apunta a una tabla de `MV`, y **no invierte nada**: es el mismo caso que `client_sellers.first_movement_id`, que ya señala a `movements` desde el 16-09-2026. `SP` no lee `movement_details` ni sabe qué es una venta; guarda el identificador que la orden de **D-26** le entrega, y del resto se encarga el motor |
