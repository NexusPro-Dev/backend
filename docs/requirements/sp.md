...
| `RN-SP-014` | Una membresía vigente por usuario | Al conceder una membresía | Un usuario tiene como mucho **una membresía vigente**, y **conserva todas las que tuvo**: conceder otra **cierra la anterior e inserta una fila nueva**, no reescribe la que había. La asignación admite una **fecha de fin opcional**: sin ella es indefinida, y con ella deja de estar **vigente** al pasar. **La vigencia se evalúa al consultarla, no la retira ningún proceso** (`RN-MV-020`), de modo que una membresía vencida conserva su fila —y su plaza— hasta que se renueve o se retire, sin conceder nivel alguno. **Terminar por vencimiento y terminar por sustitución son dos hechos distintos y se guardan por separado**: `ends_at` dice hasta cuándo se pagó y `closed_at` cuándo se cerró de verdad, de modo que «pagó hasta el 30 y se la reemplazaron el 12» es una pregunta que el sistema sabe responder. Enmendada el 05-09-2026: hasta entonces la tabla llevaba **una sola fila por persona** y asignar sustituía con un `UPDATE`, sin dejar rastro del nivel anterior. **Enmendada de nuevo el 23-09-2026**: la fila ya no vive en `user_memberships` sino en `user_products` —una membresía es **una cosa que se posee** entre otras (`RN-SP-056`)— y es la que lleva `membership_id` poblado; y **solo se concede comprándola** (`RF-MV-003`, `RN-MV-020`) o al registrarse, donde nace el suelo de `RN-SP-018`. **El contenido de la regla no cambia**: sigue habiendo una vigente, sigue conservándose todo lo que se tuvo, y `ends_at` y `closed_at` siguen respondiendo preguntas distintas. Lo que cambia es que la restricción que la declara pasó a ser **parcial** —solo mira las filas con nivel—, porque en la misma tabla conviven ahora cosas de las que sí se pueden tener dos a la vez | Alta |
...
| `RN-SP-018` | Todo usuario tiene membresía | Siempre | **Toda persona tiene una membresía vigente**, y no solo los consumidores. Quien no recibe una al registrarse **arranca en la más baja**, que es la de código `BECA` — sembrada por `V46`, imposible de borrar (`RN-SP-008`) y única por `uq_memberships_code`. El estado «usuario sin nivel» **no existe para nadie**. En consecuencia: el alta **ya no exige indicar membresía** (`RF-SP-024`) y concede el suelo, y asignar roles **tampoco la admite** (`RF-SP-030`). **Enmendada el 23-09-2026**: cambiarla ya no es `RF-SP-032` —retirado junto con `RF-SP-033`—, sino **comprarla** (`RF-MV-003`); y como nadie la retira a mano, **no queda vuelta al suelo**: el suelo se concede una vez, al registrarse, y a partir de ahí la membresía que vence **sigue abierta y deja de conceder** hasta que se compre otra, que es lo que `RN-SP-014` ya decía y ahora es el único final posible. Bajar de nivel a alguien dejó de ser algo que el sistema sepa hacer, y es deliberado: lo era a mano. **Reescrita el 05-09-2026**; hasta entonces decía «todo consumidor tiene membresía» y ataba el nivel al rol, con `RN-SP-013` y `RN-SP-015` sosteniendo las dos mitades de esa atadura | **Crítica** |
...
!!! important "`RN-SP-034` — por qué el país es una columna de `users` y no una tabla puente"
    ...
    Las otras dos cosas que una persona «tiene» en este módulo viven en tablas propias, y conviene decir por qué esta no. `user_products` es un **historial** porque lo que se posee se concede por un periodo, vence y se sustituye (`RN-SP-014`, `RN-SP-056`); `user_supervisors` lo es porque el mando cambia y hay que poder responder quién estaba a cargo **entonces** (`RN-SP-021`).
...
| ~~`RF-SP-032`~~ | ~~Asignar membresía a un usuario~~ **Descartado el 23-09-2026** (`RN-SP-056`): el nivel **se compra** (`RF-MV-003`) y deja de fijarse a mano; su ruta y `users:assign-membership` se retiran del contrato. El número queda consumido | — | — | **Descartado** |
| ~~`RF-SP-033`~~ | ~~Devolver la membresía de un usuario al suelo~~ **Descartado el 23-09-2026** (`RN-SP-056`), con `RF-SP-032` y por lo mismo: si nadie fija el nivel a mano, nadie lo retira a mano. **No queda vuelta al suelo** — la membresía vencida sigue abierta y deja de conceder (`RN-SP-014`). El número queda consumido | — | — | **Descartado** |
...
| Depende de | `RF-SP-024`, `RF-SP-052`, `RF-PM-001`, `RF-MV-001` |
...
**Y desde el 17-09-2026 publica su primera ESCRITURA** (**D-26**, cerrada ese día por el responsable del proyecto; [`architecture.md` §15.2](../architecture.md)): **`MembershipGrant`**, «conceder el nivel comprado», que `MV` invoca al confirmar el pago de una venta con un upgrade (`RF-MV-003`, `RN-MV-020`). **Cierra la vigente e inserta la nueva** (`RN-SP-014`), con la vigencia contada desde el instante que la orden indica y sin fecha de fin cuando lo comprado no caduca, deja el suelo intacto (`RN-SP-018`) y escribe su asiento en `audit_change_log` como `user_products`. **Enmendada el 23-09-2026**: hasta ese día hacía «exactamente lo que `RF-SP-032` hace a mano», y ese requerimiento está retirado — la operación publicada dejó de ser el reflejo de una operación manual para quedarse como **el único camino** por el que alguien cambia de nivel. Y la orden lleva dos datos más, porque lo que escribe es una **posesión** y no solo una membresía: **qué producto** se entregó y **qué línea** lo entregó (`RN-SP-056`). Una línea sin upgrade escribe su fila igual, con `membership_id` nulo, y entonces la operación no toca nivel alguno. **Lo que no hace es decidir si conceder**: eso lo decide `MV` antes de llamar (`RN-MV-029`), y por eso la operación no compara niveles. Corre **en la transacción de quien la llama**: si conceder falla, la venta no se confirma, y al revés. La escribe `RF-MV-003` en paquetes de `SP`, por lo mismo que las lecturas: ningún actor pide «publicar una escritura» como comportamiento.
...
**Y desde el 26-09-2026 publica LO QUE UNA PERSONA TIENE VIGENTE** (D-25): **`CurrentProductsLookup`**, «¿qué productos tiene hoy esta persona?», **un conjunto de identificadores de producto**, sin fechas ni filas. Lo pide `AC` para el aula (`RF-AC-033` a `RF-AC-035`): un curso se abre a quien tiene vigente uno de sus servicios (`RN-AC-020`), y `AC` no lee `user_products`. **Vigente es lo mismo que en `CurrentMembershipLookup`**: una fila de `user_products` con producto, **empezada, sin fin pasado y sin cerrar** (`RN-SP-056`). **Devuelve un conjunto y no responde sobre un producto**, al revés que `PermissionHolderLookup`, porque el catálogo del aula decide `accessible` para decenas de cursos: una llamada por catálogo y no una por curso. Lo que no cruza —cuándo vence, de qué venta salió— es lo que permitiría a otro módulo rehacer la comparación que este puerto existe para no repetir. La escribió `RF-AC-033` · `T-03` en paquetes de `SP`, como las demás.
...
| `user_products` | Historial de **lo que cada persona posee** —bots y membresías, una fila por cosa y por periodo (`RN-SP-056`)—; la que lleva `membership_id` es la que concede nivel, y de esas la abierta es la actual (`RN-SP-018`). **Desde el 23-09-2026**; hasta entonces se llamaba `user_memberships` y solo guardaba niveles | `SP` |
...
| `pk_user_products` | **Clave primaria**: `user_products(id)` — un identificador propio, porque la tabla guarda **todo lo que alguien tuvo** y `user_id` se repite. **Desde el 05-09-2026**; hasta entonces la clave iba sobre `user_id` y era ella quien declaraba `RN-SP-014` |
| `fk_user_products_user` | `user_products(user_id)` → `users(id)`, `ON DELETE RESTRICT` |
| `fk_user_products_membership` | `user_products(membership_id)` → `memberships(id)`, `ON DELETE RESTRICT`. **Pasa a admitir nulo el 23-09-2026**: la lleva poblada solo lo que concede nivel, y lo que se posee sin concederlo —un bot— la deja vacía |
| `fk_user_products_product` | `user_products(product_id)` → `products(id)`, `ON DELETE RESTRICT` — **desde el 23-09-2026**. Admite nulo, y `ck_user_products_origen` es quien dice cuándo se acepta que lo sea |
| `fk_user_products_movement_detail` | `user_products(movement_detail_id)` → `movement_details(id)`, **`ON DELETE CASCADE`** — **desde el 23-09-2026**. Es una clave foránea de `SP` que apunta a una tabla de `MV`, y **no invierte nada**: es el mismo caso que `client_sellers.first_movement_id`, que ya señala a `movements` desde el 16-09-2026. `SP` no lee `movement_details` ni sabe qué es una venta; guarda el identificador que la orden de **D-26** le entrega, y del resto se encarga el motor. **`ON DELETE CASCADE` y no `RESTRICT`, que es la única de las cuatro que no lo lleva** (23-09-2026): la posesión es **derivada** de la línea, y una fila que apunta a una línea que ya no está no dice de dónde salió lo que alguien tiene. En producción la cláusula **no se dispara nunca** —una venta no se edita ni se borra (`RN-MV-001`), solo avanza de estado—, de modo que `RESTRICT` protegía aquí un caso que no ocurre a cambio de obligar a cada suite de pruebas a aprenderse un orden de borrado. La asimetría con el producto es la que importa: aquel sí va con `RESTRICT`, porque nadie puede hacer desaparecer un producto mientras alguien lo tenga. |
| `uq_user_products_linea` | **Único**: `user_products(movement_detail_id)` — una línea produce **como mucho una** posesión, y es lo que hace la entrega **idempotente** (`RN-SP-056`). Una confirmación repetida no puede duplicar lo que alguien tiene, y no hay que comprobarlo antes de insertar: comprobarlo sería una carrera, y este es el mismo recurso que `uq_user_roles_vendedor` usa para `RN-SP-025` |
| `ck_user_products_origen` | `user_products(product_id IS NOT NULL OR membership_id IS NOT NULL)` — **desde el 23-09-2026**. Una fila que no dice **qué** se tiene ni **qué nivel** concede no describe nada. Deja pasar los dos casos legítimos de `product_id` nulo —el suelo de `RN-SP-018` y la semilla fundacional— sin abrir la puerta a la fila vacía |
| `ck_user_products_periodo` | `user_products(ends_at IS NULL OR ends_at > started_at)` |
| `ck_user_products_cierre` | `user_products(closed_at IS NULL OR closed_at >= started_at)` — un cierre anterior al comienzo no es un caso de negocio. Va con `>=` y no con `>` a propósito: **conceder y cerrar en la misma transacción producen el mismo instante**, y prohibirlo haría fallar la operación normal de `RF-MV-003` |
| `ck_user_products_validity_days` | `user_products(validity_days IS NULL OR validity_days > 0)` — **desde el 23-09-2026**. Nulo es «no caduca»; cero o negativo sería una vigencia que **nace vencida**, y lo que entrega esa columna es una copia de lo vendido, no un cálculo |
| `uq_user_products_membresia_abierta` | **Índice único parcial**: `user_products(user_id) WHERE closed_at IS NULL AND membership_id IS NOT NULL` — **una sola membresía abierta por persona**, y es esta la que carga `RN-SP-014` en el día a día. **La condición ganó su segunda mitad el 23-09-2026, y sin ella el cambio entero no se sostendría**: con la tabla abierta a los productos, un único sobre `(user_id) WHERE closed_at IS NULL` impediría tener **un bot y una membresía a la vez**, que es el caso corriente y no un error. Su otro trabajo es el de siempre y no es menor: `RF-SP-025` y `RF-SP-026` cruzan esta tabla con un `LEFT JOIN`, y sin él ese cruce multiplicaría filas — el listado de usuarios repetiría personas. Obliga a que conceder **cierre siempre** la anterior, incluso si ya estaba vencida |
| `ex_user_products_membresia_sin_solape` | **`EXCLUDE USING gist` parcial**: `user_id WITH =`, `tstzrange(started_at, COALESCE(LEAST(ends_at, closed_at), 'infinity')) WITH &&`, con `WHERE membership_id IS NOT NULL` — impide que **dos periodos de nivel se pisen**, que es lo que el índice de arriba **no** puede decir: aquel habla de filas abiertas, y dos membresías **cerradas** con fechas solapadas lo satisfarían sin problema. `LEAST` da el fin **real** —vence o la cierran, lo que ocurra antes— y devuelve la que no sea nula; el `COALESCE` cubre el caso en que las dos lo son, que es la membresía indefinida y viva. **Los dos hacen falta y ninguno sobra**: el único parcial no ve el historial y el `EXCLUDE` no ve dos filas abiertas que no se solapan —una vencida y otra nueva—. **Parcial desde el 23-09-2026**, por lo mismo que el de arriba: dos bots comprados el mismo mes se solapan y eso es lo normal. Mismo patrón y misma extensión que `ex_commission_rates_sin_solape` (`V44`), y por el mismo motivo que allí: comprobarlo con un `SELECT` previo es una carrera |
...
### 10.12 Campos principales — `user_products`
...
| `membership_id` | `uuid` | No | Sí | **Sí** | — | `memberships` |
| `movement_detail_id` | `uuid` | No | Sí | Sí | — | `movement_details` |
...
**Se llamaba `user_memberships` y desde el 23-09-2026 es `user_products`** (`RN-SP-056`), por decisión del responsable del proyecto. **El cambio no es de nombre.** La tabla respondía «qué nivel tiene esta persona» y ahora responde **«qué tiene esta persona»**; una membresía deja de ser lo único que se puede tener y pasa a ser **un caso** de ello: el que concede nivel. Lo que se gana es que **lo comprado existe en alguna parte**. Hasta ese día una venta de un bot confirmada y entregada **no dejaba ninguna fila** que dijera que alguien lo tiene: «mis productos» (`RF-MV-014`) se reconstruía en cada consulta recorriendo las líneas de venta y recalculando el vencimiento, de modo que el sistema sabía **qué se vendió** y no **qué se posee**, que no son la misma pregunta en cuanto algo se anula o se cancela antes de tiempo.

**Es un historial, no una foto.** Cada fila es **algo que alguien tuvo**, con su periodo. Conceder otra membresía **cierra la que había e inserta una nueva**; nada se sobrescribe. Hasta el 05-09-2026 la tabla tenía `user_id` como clave primaria y `RF-SP-032` sustituía con un `UPDATE` — el nivel anterior no se podía reconstruir, y esa deuda estaba escrita y aceptada. Dejó de estarlo por decisión del responsable del proyecto.

**`ends_at` y `closed_at` responden preguntas distintas, y por eso son dos columnas.** `ends_at` es **hasta cuándo se pagó** —nula, indefinida—; `closed_at` es **cuándo dejó de tenerse**. Una membresía de treinta días que se reemplaza el día doce termina con `ends_at` en el día treinta y `closed_at` en el doce, y las dos cosas son ciertas: se pagó un mes y se usó menos de medio. Con una sola columna esa diferencia se pierde, y con ella la respuesta a un reclamo. **Desde el 23-09-2026 la columna puede decir una segunda cosa, y es la que pidió el cambio**: que algo **se canceló antes de tiempo**. Para una membresía sustituida ya lo decía; lo que no existía era poder preguntarlo **de un bot**, que hasta ese día no tenía fila donde escribirlo. **Hoy nadie la escribe por esa vía y conviene decirlo**: anular (`RF-MV-008`) solo alcanza a una venta `PENDIENTE`, que no entregó nada, y cancelar algo ya entregado **no es una operación del sistema**. La columna deja el hecho declarable el día que esa operación exista; lo que no se hace es inventarla ahora.

**Tres columnas nuevas, y ninguna es adorno.** `product_id` dice **qué** se tiene. `validity_days` es **la copia de lo que se vendió** —los mismos días que la línea copió del producto (`RN-PM-015`)—, y está aquí por lo mismo que está en la línea: el catálogo cambia, y sin la copia «hasta cuándo» dejaría de ser verificable el día que alguien edite el producto. `movement_detail_id` dice **de qué línea salió**, y con su único convierte la entrega en **idempotente**.

**`product_id` nulo NO es un hueco del diseño: es lo que no se compra.** Hay exactamente dos cosas que se poseen sin venta, y las dos son la misma regla: el **suelo** que `RN-SP-018` concede al registrarse (`RF-SP-024`, `RF-SP-045`) y la fila del superadministrador que siembra la semilla fundacional. Ninguna de las dos tiene un producto que señalar, y **inventarle uno sería peor**: un «producto BECA» en el catálogo sería comprable, aparecería en la oferta y habría que excluirlo de todas partes a mano. `ck_user_products_origen` es lo que impide que esa nulidad se extienda a una fila que no sea ni una cosa ni un nivel.

**La fila abierta única y el no-solape se volvieron PARCIALES, y es la consecuencia mayor del cambio.** Las dos restricciones decían «por persona» y ahora dicen «por persona, **entre lo que concede nivel**». El motivo es que el invariante nunca fue sobre la tabla sino sobre el nivel: nadie tiene dos niveles a la vez, pero **tener un bot y una membresía a la vez es lo normal**, y dos bots comprados el mismo mes se solapan sin que nada esté mal. Dejar las restricciones como estaban habría convertido la tabla nueva en una tabla que **no admite dos productos**, que es justo lo contrario de lo que se pedía.

**Fijar el nivel a mano ya no existe.** `RF-SP-032` y `RF-SP-033` quedaron **retirados el 23-09-2026** con sus dos permisos: el nivel **se compra** (`RF-MV-003`, `RN-MV-020`) o se recibe al registrarse, y no hay una tercera puerta. Lo que aquellos hacían sigue ocurriendo —cerrar la vigente, abrir la nueva, no dejar a nadie sin nivel—, pero dentro de la operación publicada de **D-26**, que es ahora quien lo hace y el único sitio donde está escrito. **Retirar la membresía tampoco es ya una operación**, y con ella se fue la única vuelta al suelo: la vencida ocupa su plaza **sin conceder nada** hasta que se compre otra.

`RF-SP-024` la creó (`V20__create_user_memberships.sql`), `V56` la convirtió en historial y **`V38` la renombra y la abre a los productos**. Es un `RENAME` y no una tabla nueva: conserva los datos, las claves foráneas y la auditoría ya escrita, y las filas que existían quedan con `product_id` nulo, que es **exactamente lo que son** — niveles concedidos sin compra. Migrarla copiando a una tabla nueva habría obligado a decidir qué producto inventarles.
...
!!! warning "Las siete nacen **nulables en el esquema** y dos de ellas son **obligatorias en la API**, y la asimetría es deliberada"
    ...
    teléfono. `V22` sembraba un superadministrador que no tenía ni uno ni otro, y la semilla de
    desarrollo crea decenas de personas que tampoco.

    **El superadministrador dejó de ser uno de esos casos el 23-09-2026**, por decisión del
    responsable del proyecto: nace con `CC`, número `12345678910` y teléfono `3001234567`.
    **El argumento de arriba no se cae, se acota**, y conviene ver dónde: «cualquier valor que
    se invente es falso» vale sobre **una persona**, y el superadministrador no lo es — es la
    cuenta con la que el sistema arranca, y su identidad documental es un **marcador de
    instalación**, no la afirmación de que exista alguien con esa cédula. Para las decenas de
    personas de la semilla de desarrollo la regla sigue intacta, y para toda alta por la API
    también. **La ausencia sigue siendo representable en el esquema** por las filas anteriores,
    que es lo que este apartado defiende.
...
| 1.84.0 | 23-09-2026 | **`user_memberships` pasa a ser `user_products`: la tabla deja de guardar niveles y pasa a guardar LO QUE CADA PERSONA TIENE**, por decisión del responsable del proyecto, y con ella **nace `RN-SP-056`**. Una membresía deja de ser lo único poseíble para ser **un caso** de lo poseído: el que lleva `membership_id`. **Dos requerimientos se RETIRAN y no se sustituyen** —`RF-SP-032` y `RF-SP-033`, con `users:assign-membership` y `users:revoke-membership`, y el catálogo de permisos baja de **135 a 133**—: el nivel **se compra** (`RF-MV-003`) o se recibe al registrarse, y fijarlo a mano deja de ser una puerta del sistema. **`RN-SP-014` y `RN-SP-018` se enmiendan sin cambiar de contenido** (Art. I.7): sigue habiendo una membresía vigente por persona y sigue sin existir nadie sin nivel; lo que cambia es que las dos restricciones que lo declaran —`uq_user_products_membresia_abierta` y `ex_user_products_membresia_sin_solape`— pasan a ser **parciales sobre las filas con nivel**, porque en la misma tabla conviven ahora cosas de las que sí se pueden tener dos a la vez. **Tres columnas nuevas**: `product_id` —nulo **solo** en lo que no se compra, con `ck_user_products_origen` cerrando el hueco—, `validity_days` copiado de lo vendido y `movement_detail_id`, único, que hace la entrega **idempotente**. **Y `closed_at` queda habilitada para decir «se canceló antes de tiempo»** de cualquier cosa poseída y no solo de una membresía sustituida — hecho que hoy **ninguna operación escribe**, porque anular solo alcanza a lo `PENDIENTE`, y que se deja declarable sin inventar la operación que falta. La operación publicada de **D-26** deja de ser el reflejo de `RF-SP-032` para ser el único camino, y recibe dos datos más. `V38` renombra y no copia. `requirements/mv.md` v0.40.0, `modelo-datos.md` v0.68.0, `security.md` v0.72.0 —**el catálogo baja por primera vez**—, `architecture.md` v0.35.0 y la matriz de `requirements.md` v0.211.0. | Responsable del proyecto |
| 1.85.0 | 23-09-2026 | **El superadministrador nace con identidad documental y teléfono**, por decisión del responsable del proyecto: `CC`, `12345678910` y `3001234567`, escritos en la `V9` **en el sitio** y no en una migración de datos aparte — el dato se quiere en el alta, no en una corrección posterior. §10.8 se acota, y no se retira: lo que sostiene que el esquema admita la ausencia es que **inventar un documento sobre una persona es afirmar algo falso**, y el superadministrador **no es una persona** sino la cuenta con la que el sistema arranca; su documento es un marcador de instalación. Para la semilla de desarrollo y para toda alta por la API la regla no se mueve. **El coste está a la vista y se aceptó**: editar una migración ya aplicada rompe la suma de comprobación de Flyway en cualquier base existente, que hay que recrear; CI levanta una nueva en cada corrida. `DevelopmentSeedIT.documento` queda **invertida** en el mismo cambio — afirmaba lo contrario y era ella, no la migración, la que ponía el `verify` en rojo cada vez que alguien lo intentaba. | Responsable del proyecto |
