...
**La respuesta lleva además un RESUMEN de lo filtrado** (10-09-2026, por decisión del responsable del proyecto): **cuántos registros hay** y **cuántos están en `FIRST_DEPOSIT`**, cada uno con su **desglose por broker**. Va dentro del listado y no en un endpoint aparte — una sola llamada pinta la tabla y sus contadores, y así no pueden desincronizarse.
...
**El desglose por broker trae solo los brokers con al menos una cuenta.** Un broker que no aparece no tiene ninguna en ese filtro; no hay ceros que interpretar.
