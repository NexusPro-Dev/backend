...
| `user_memberships` | Historial de membresías de cada usuario consumidor — la abierta es la actual | `SP` |
...
**Retirar (`RF-SP-033`) cierra y no borra, y no toca `ends_at`.** El `DELETE` que hacía hasta el 05-09-2026 llevaba su motivo escrito —`RN-SP-015` dice que quien deja de ser consumidor **no tiene** membresía, no que tuviera una que terminó—, y ese motivo **se invierte con el historial**: la fila cerrada dice exactamente que la tuvo y se la quitaron, que es el hecho que hay que poder consultar. Es el mismo criterio con el que `endSupervisor` nunca fue un `DELETE` (`V21`).
