...
- Registrar y mantener el **catálogo de tasas por rol**, en porcentaje.
- **Asociar** una tasa de rol a un producto, que es lo único que la pone en vigor.
- Registrar la **tasa personalizada** de una persona, con su vigencia.
- Consultar unas y otras.
- **Resolver la comisión efectiva**: dada una persona, un producto y una **fecha**, qué porcentaje le corresponde y **por qué tasa**.

**No incluye**

- **El valor directo.** Declarar «este producto paga 5.000 fijos» queda para después, por decisión del responsable del proyecto: un importe fijo obliga a decidir su moneda y **no está acotado por nada** —tres valores fijos en una cadena pueden superar el importe de la venta—, mientras que un porcentaje lo acota `RN-CM-007`.
...
| Resolución | Qué porcentaje le corresponde a una persona por un producto **en una fecha** | Las tres |
...
| `RF-CM-003` | Corregir el porcentaje de una tasa | Tasas | `commissions:update` |
...
!!! warning "Los cinco primeros existen, y este documento cambia lo que hacen"

    `RF-CM-001` a `RF-CM-005` **están implementados** desde el 28-08-2026, con 45 pruebas. El modelo de este documento cambia la forma de `commission_rates` —le quita el producto, la persona y la vigencia—, de modo que **la implementación y su suite hay que rehacerlas**. No es construir algo nuevo: es rehacer algo que funciona.
...
| `RN-CM-007` | El porcentaje va de cero a cien | Al registrar y al corregir | Se admite el **cero**, que significa «esto no comisiona» y **no es lo mismo que no tener tasa**: es la forma de asociar un producto a un rol declarando que no paga nada | Alta |
...
**El tope de la cadena.** `RN-CM-011` reparte, `RN-CM-007` acota **cada** porcentaje a cien, y **nadie acota la suma**. Queda sin dueño hasta que exista quien aplique las tarifas.
...
|---|---|---|
| `ck_commission_rates_percentage` | `percentage >= 0 AND percentage <= 100` | `RN-CM-007`. El cero **se admite** |
...
| `ck_user_commission_rates_percentage` | `percentage >= 0 AND percentage <= 100` | `RN-CM-007` |
