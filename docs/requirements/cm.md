...
**La personalizada se asocia a productos EXACTAMENTE IGUAL que la de rol**, desde el 11-09-2026 y por decisión del responsable del proyecto. Hasta esa fecha **ignoraba el producto**: quien tenía una ganaba lo mismo vendiera lo que vendiera. Hoy se crea primero y **se asocia después** —a uno o a varios productos, con su operación de asociar y de desasociar— y **no paga nada hasta que se asocia** (`RN-CM-012`), igual que la de rol. Donde esté asociada gana sobre la de su rol; donde no, esa persona cobra por su rol como cualquier otra.

**Que el mecanismo sea el mismo es la decisión**, y no una coincidencia: había una segunda forma posible —el producto como columna de la propia tasa, fijado al crearla— y se descartó por pedir dos maneras distintas de decir lo mismo. Lo que cuesta está en `RN-CM-006`, que **deja de vivir en el motor**: el `EXCLUDE` cabía porque la persona, la vigencia y el producto vivían en una fila, y con la asociación fuera la regla cruza dos tablas, cosa que ningún índice hace.
...
- Consultar unas y otras, **y sus asociaciones en las tres direcciones**: los productos de una tasa de rol, los productos de una personalizada —desde el 12-09-2026— y los roles que cobran por un producto. «Quién tiene excepción en este producto» se responde **filtrando las personalizadas por producto**.
...
| `PM` | Consume | **Productos** (`RN-CM-002`): validar que el producto al que se asocia una tasa existe y no está retirado; y su **precio** (`RN-CM-019`), para convertir un valor fijo a su porcentaje equivalente al comprobar el tope |
...
| `RN-CM-002` | El producto debe existir | Al asociar una tasa de rol y al registrar una personalizada | El producto debe existir en `PM`. Se declara además como clave foránea, en las dos tablas. **Alcanza a la personalizada desde el 11-09-2026**, que hasta entonces no nombraba ninguno | Alta |
...
| `RN-CM-010` | No se configura lo que ya no se vende | Al asociar una tasa de rol y al registrar una personalizada | No se admite atar una tasa a un producto **retirado**: sería configurar algo que nadie puede vender. Lo que ya existía **permanece**, por `RN-CM-005`. **Alcanza a la personalizada desde el 11-09-2026** | Media |
...
| `RN-CM-012` | **Ninguna tasa rige hasta que se asocia** | Siempre | Existir en el catálogo no pone en vigor a nada. Sin asociación no se paga nada a nadie, y **no hay tarifa por omisión**: la ausencia de producto significa «ninguno», nunca «todos». **Desde el 11-09-2026 alcanza también a la personalizada**, que hasta entonces era la excepción a esta regla — pagaba desde el primer día sobre todo el catálogo. Lo que la regla cuesta se paga ahora por partida doble: **una tasa creada y no asociada parece configurada y no paga**, y eso no falla — se descubre liquidando | **Crítica** |
...
| `RN-CM-014` | **Las dos clases de tasa se asocian a productos, y con el mismo mecanismo** | Al asociar | **Invertida el 11-09-2026.** Hasta esa fecha decía que solo las de rol se asociaban y que una personalizada regía sobre todo el catálogo. Ahora las dos tienen su **tabla de asociación** —`product_commission_rates` la de rol, `user_commission_rate_products` la personalizada—, las dos admiten **varios productos por tasa**, y las dos tienen su operación de **desasociar**. Se descartó la alternativa de darle a la personalizada una **columna** `product_id` fijada al crearla: resolvía lo mismo y obligaba a aprender dos maneras de decir lo mismo | Alta |
| `RN-CM-015` | **Una tasa asociada no se retira** | Al retirar una tasa, de rol o personalizada | Si sigue asociada a algún producto, el retiro se **rechaza**: hay que desasociarla primero. La asociación **no tiene retiro lógico** y sobreviviría apuntando a una fila que la resolución ya no mira, de modo que **dejaría de pagarse sin que nada lo dijera** — en la de rol el producto deja de comisionar, y en la personalizada su titular vuelve en silencio a la tarifa de su rol. **Alcanza a la personalizada desde el 11-09-2026**, cuando ganó asociaciones. El coste es **dos operaciones donde había una**, y se paga a la vista | **Crítica** |
...
| `RN-CM-018` | **El valor fijo no está acotado por arriba mientras no está asociado** | Siempre que la tasa no está asociada a ningún producto | `RN-CM-007` acota el porcentaje a cien; el importe, por su cuenta, no tiene ese número — una tasa no conoce el precio de nada hasta que se le asocia un producto. **Desde el 11-09-2026 la regla es UNA para las dos clases**: sin asociar, ni la de rol ni la personalizada tienen tope; en cuanto se asocian, lo pone `RN-CM-019` contra el precio del producto al que se atan. Es la simetría que faltaba: hasta esa fecha la personalizada **nunca** llegaba a tener tope, porque nunca se ataba a nada | **Crítica** |
| `RN-CM-019` | **Un producto no puede configurarse para pagar más del 100 % de sí mismo** | Al asociar y al corregir, en las dos clases de tasa | La suma de lo que un producto paga a **todas** sus tasas de rol asociadas —cada porcentaje tal cual, cada valor fijo convertido a `fixed_amount ÷ precio × 100`, contra el precio de **ese** producto— no puede superar cien. Se comprueba **al asociar** (contando la nueva) y **al corregir** (revisando **todos** los productos donde la tasa corregida está asociada; si cualquiera se pasaría, la corrección se rechaza entera). **Desde el 11-09-2026 la tasa personalizada tiene su propio tope al asociarse**, y es **individual y no una suma**: ninguna puede pagar más del 100 % del precio del producto al que se ata. **No entra en la suma de las de rol**, y es deliberado: las personalizadas de personas distintas sobre el mismo producto son **alternativas entre sí**, no cosas que se paguen a la vez, y sumarlas rechazaría configuraciones legítimas. El tope se calcula contra el precio **de hoy**: si el producto cambia de precio después (`RF-PM-004`), nadie vuelve a comprobarlo. **Con precio CERO no aplica** (14-09-2026): un producto gratuito no tiene «cien por ciento» del que pasarse, y qué puede pagar lo dice `RN-CM-020`. **Entre el 08-09-2026 y el 14-09-2026 decía lo contrario** —que cualquier fijo mayor que cero sobre precio cero era «más del 100 %» y se rechazaba—, y se invirtió por decisión del responsable del proyecto (§5.2) | **Crítica** |
| `RN-CM-020` | **Un producto gratuito comisiona solo por importe fijo** | Al asociar y al corregir, en las dos clases de tasa | Sobre un producto de **precio cero** (`RN-PM-006`) se admite **cualquier** tasa de **valor fijo**, **sin tope** —no hay cien por ciento de cero—, y **se rechaza toda tasa de porcentaje**: un porcentaje de nada es nada, y asociarlo configura algo que no paga. Se comprueba **al asociar** (`RF-CM-006`, `RF-CM-007`) y **al corregir** (`RF-CM-003`): corregir hacia porcentaje una tasa asociada a un producto gratuito se rechaza entera, como el tope. **Se evalúa contra el precio de hoy y nadie vuelve a mirarlo** (§5.2): un porcentaje ya asociado a un producto que después baja a cero **pasa a pagar cero**, y un fijo asociado a un gratuito que después sube de precio **no se vuelve a acotar**. Es el mismo hueco temporal que `RN-CM-019` acepta, y se acepta por lo mismo | Alta |
