...
|---|---|---|---|
| **Tasa de rol** | «Un `AGENTE` gana el 10 % **por este producto**» | **No tiene** | **No hace falta**: nace con su producto y rige sobre él desde el alta (15-09-2026) |
| **Tasa personalizada** | «**esta persona** gana el 12 %» | **Sí**, y una sola vigente por persona **y producto** | **Sí**, y solo rige por esa asociación |

**La tasa de rol nace con su producto, y solo rige sobre él** (`RN-CM-021`, 15-09-2026). **Dejó de ser catálogo**: hasta esa fecha se registraba sin producto y regía únicamente donde se la asociaba (`RN-CM-012`), de modo que una tasa creada y sin asociar no pagaba nada. Hoy no existe una tasa de rol sin producto —`product_id` es obligatorio y no se cambia—, y **rige desde que existe**, sin paso de asociación. Lo que se pierde y lo que se gana está en §5.4.

**La personalizada se sigue asociando a productos**, desde el 11-09-2026 y por decisión del responsable del proyecto. Hasta esa fecha **ignoraba el producto**: quien tenía una ganaba lo mismo vendiera lo que vendiera. Hoy se crea primero y **se asocia después** —a uno o a varios productos, con su operación de asociar y de desasociar— y **no paga nada hasta que se asocia** (`RN-CM-012`). Donde esté asociada gana sobre la de su rol; donde no, esa persona cobra por su rol como cualquier otra. **Desde el 15-09-2026 es la única clase que se asocia**: la de rol ya no lo necesita.

**Que las dos clases usaran el mismo mecanismo fue la decisión del 11-09-2026, y el 15-09-2026 se deshizo a medias**: la segunda forma —el producto como columna de la propia tasa, fijado al crearla— que entonces se descartó «por pedir dos maneras de decir lo mismo» es exactamente la que la tasa de rol adopta hoy (§5.4). Las dos clases vuelven a decirlo de dos maneras, y se acepta a conciencia: la de rol es **configuración de un producto** —qué paga a cada rol— y la personalizada es **una excepción de una persona** que puede abarcar varios. Lo que cuesta en la personalizada sigue estando en `RN-CM-006`, que **no vive en el motor**: el `EXCLUDE` cabía porque la persona, la vigencia y el producto vivían en una fila, y con la asociación fuera la regla cruza dos tablas, cosa que ningún índice hace.
...
| **Valor fijo** | «gana 10.000 por venta» | **Nada por su cuenta** cuando no conoce ningún precio (`RN-CM-018`). Desde el 03-09-2026 una de rol **asociada** entra en la suma que `RN-CM-019` acota, convertida contra el precio de **ese** producto; desde el 11-09-2026 la **personalizada** tiene su propio tope contra el precio del producto al que se ata. Sin asociar, una de rol sigue sin tope — ver §5.3 |
...
- Registrar la **tasa personalizada** de una persona, con su vigencia, **también en cualquiera de las dos formas**, y **asociarla a los productos sobre los que rige**.
- Consultar unas y otras: **el listado de todas las tasas de rol**, con su producto y filtrable por él; los productos de una personalizada —desde el 12-09-2026—; y los roles que cobran por un producto. «Quién tiene excepción en este producto» se responde **filtrando las personalizadas por producto**.
...
|---|---|---|
| Tasas | Las tasas de rol de cada producto y las excepciones por persona | `commission_rates`, `user_commission_rates` |
| Asociación | Qué tasa **personalizada** rige sobre qué producto | `user_commission_rate_products` |
| Resolución | Qué le corresponde a una persona por un producto **en una fecha** | Las tres |

**Por qué la asociación sigue siendo un submódulo, y ya solo para la personalizada.** Hasta el 15-09-2026 la tasa de rol **regía sobre varios productos** y la asociación era lo que lo expresaba; hoy la tasa de rol **es de un producto** (`RN-CM-021`), y «un producto tiene una tasa por cada rol de la cadena» se declara en la propia tabla, con la unicidad `(producto, rol)`. Lo que aquel argumento temía —corregir un porcentaje en cincuenta filas y que una se quedara atrás— es ahora lo que el responsable del proyecto decidió aceptar: **cada producto se configura por su cuenta**, y una tasa que quiera repetirse en cincuenta productos son cincuenta tasas (§5.4). La personalizada conserva su asociación porque una excepción de una persona sí abarca varios productos.
...
| `PM` | Consume | **Productos** (`RN-CM-002`): validar que el producto de una tasa de rol —o al que se asocia una personalizada— existe y no está retirado; su **precio** (`RN-CM-019`), para convertir un valor fijo a su porcentaje equivalente al comprobar el tope; y su **moneda** (`RN-CM-017`), para acotar los decimales de un importe fijo de rol |
...
| `RF-CM-006` | Registrar la tasa personalizada de una persona, **y asociarla a productos** | Tasas | `commissions:create` |
...
| `RN-CM-002` | El producto debe existir | Al registrar una tasa de rol y al asociar una personalizada | El producto debe existir en `PM`. Se declara además como clave foránea: en `commission_rates` (desde el 15-09-2026) y en `user_commission_rate_products`. **Alcanza a la personalizada desde el 11-09-2026**, que hasta entonces no nombraba ninguno | Alta |
| `RN-CM-004` | **La personalizada gana siempre, donde está asociada** | Al resolver | Si la persona tiene una tasa personalizada **vigente en la fecha y asociada a ese producto**, es esa. Si no, la que su **rol vendedor** tenga asociada a ese producto. Si no hay ninguna de las dos, **no hay tarifa**. Siguen siendo dos niveles, no cuatro, y **desde el 11-09-2026 los dos exigen asociación**. Hasta esa fecha el primero no la exigía —ni siquiera existía— y la personalizada ganaba **sin mirar el producto**, de modo que tapaba el catálogo entero de su titular y el segundo nivel no llegaba a consultarse nunca | **Crítica** |
...
| `RN-CM-006` | Una sola tasa personalizada vigente por persona **y producto** | **Al asociar** y al corregir la vigencia de una personalizada | **Ningún día puede estar cubierto por dos tasas personalizadas vivas de la misma persona sobre el mismo producto.** Sí pueden existir varias consecutivas —son el historial—, varias simultáneas **sobre productos distintos**, y varias simultáneas **sin asociar**, que no rigen nada. **El 11-09-2026 esta regla SALE DEL MOTOR**, y es la única del módulo que lo hace en sentido contrario: el `EXCLUDE` cabía porque la persona, la vigencia y el producto vivían en la misma fila, y con la asociación en otra tabla la regla **cruza dos**, cosa que ningún índice hace. Pasa a comprobarse en el caso de uso **con un bloqueo consultivo por persona**, que es lo que impide que dos asociaciones simultáneas la burlen. **Se comprueba AL ASOCIAR y no al registrar**: sin producto no hay solapamiento posible | **Crítica** |
...
| `RN-CM-010` | No se configura lo que ya no se vende | Al registrar una tasa de rol y al asociar una personalizada | No se admite una tasa sobre un producto **retirado**: sería configurar algo que nadie puede vender. Lo que ya existía **permanece**, por `RN-CM-005`. **Alcanza a la personalizada desde el 11-09-2026** | Media |
...
| `RN-CM-012` | **Ninguna tasa personalizada rige hasta que se asocia; la de rol rige desde que nace** | Siempre | **Reescrita el 15-09-2026.** La personalizada existe primero y **no paga nada hasta que se asocia** a un producto: no hay tarifa por omisión, y la ausencia de producto significa «ninguno», nunca «todos». La de rol **ya no tiene ese paso**: nace con su producto (`RN-CM-021`) y rige sobre él en el acto. Lo que la regla costaba por partida doble —una tasa creada y no asociada parece configurada y no paga— queda hoy en la personalizada sola | **Crítica** |
...
| `RN-CM-014` | **Solo la personalizada se asocia a productos; la de rol nace con el suyo** | Al asociar | **Invertida por segunda vez el 15-09-2026.** Nació diciendo que solo las de rol se asociaban; el 11-09-2026 pasó a decir que las dos, con el mismo mecanismo; hoy dice que **solo la personalizada** tiene tabla de asociación —`user_commission_rate_products`—, admite varios productos por tasa y tiene su operación de desasociar. La de rol lleva el producto **como columna fijada al crearla** (`RN-CM-021`): la alternativa que el 11-09-2026 se descartó para la personalizada es la que el responsable del proyecto eligió para la de rol, y las dos clases dicen la misma cosa de dos maneras a conciencia (§5.4) | Alta |
| `RN-CM-015` | **Una tasa personalizada asociada no se retira** | Al retirar una personalizada | Si sigue asociada a algún producto, el retiro se **rechaza**: hay que desasociarla primero. La asociación **no tiene retiro lógico** y sobreviviría apuntando a una fila que la resolución ya no mira, de modo que su titular volvería **en silencio** a la tarifa de su rol. **La de rol queda fuera desde el 15-09-2026**: no tiene asociación que la sostenga, y retirarla es exactamente la forma de que el producto deje de pagar a ese rol — a la vista, con motivo e instantánea (`RN-CM-005`) | **Crítica** |
...
| `RN-CM-018` | **El valor fijo de una personalizada no está acotado por arriba mientras no está asociada; el de rol nace acotado** | Siempre que una personalizada no está asociada a ningún producto | `RN-CM-007` acota el porcentaje a cien; el importe, por su cuenta, no tiene ese número — una personalizada no conoce el precio de nada hasta que se le asocia un producto, y en cuanto se asocia lo pone `RN-CM-019` contra el precio de ese producto. **La de rol queda fuera desde el 15-09-2026**: conoce su producto desde el alta, y `RN-CM-019` y `RN-CM-020` la acotan **al registrarla**, no después | **Crítica** |
| `RN-CM-019` | **Un producto no puede configurarse para pagar más del 100 % de sí mismo** | Al registrar o corregir una tasa de rol, y al asociar o corregir una personalizada | La suma de lo que un producto paga a **todas** sus tasas de rol vivas —cada porcentaje tal cual, cada valor fijo convertido a `fixed_amount ÷ precio × 100`, contra el precio de **ese** producto— no puede superar cien. **Desde el 15-09-2026** se comprueba **al registrar** la tasa de rol (contando la nueva) y **al corregirla** (contra su único producto); hasta entonces se comprobaba al asociar y, al corregir, revisando todos los productos donde la tasa estaba asociada. **Desde el 11-09-2026 la tasa personalizada tiene su propio tope al asociarse**, y es **individual y no una suma**: ninguna puede pagar más del 100 % del precio del producto al que se ata. **No entra en la suma de las de rol**, y es deliberado: las personalizadas de personas distintas sobre el mismo producto son **alternativas entre sí**, no cosas que se paguen a la vez, y sumarlas rechazaría configuraciones legítimas. El tope se calcula contra el precio **de hoy**: si el producto cambia de precio después (`RF-PM-004`), nadie vuelve a comprobarlo. **Con precio CERO no aplica** (14-09-2026): un producto gratuito no tiene «cien por ciento» del que pasarse, y qué puede pagar lo dice `RN-CM-020`. **Entre el 08-09-2026 y el 14-09-2026 decía lo contrario** —que cualquier fijo mayor que cero sobre precio cero era «más del 100 %» y se rechazaba—, y se invirtió por decisión del responsable del proyecto (§5.2) | **Crítica** |
| `RN-CM-020` | **Un producto gratuito comisiona solo por importe fijo** | Al registrar o corregir una tasa de rol, y al asociar o corregir una personalizada | Sobre un producto de **precio cero** (`RN-PM-006`) se admite **cualquier** tasa de **valor fijo**, **sin tope** —no hay cien por ciento de cero—, y **se rechaza toda tasa de porcentaje**: un porcentaje de nada es nada, y asociarlo configura algo que no paga. Se comprueba **al registrar la de rol** (`RF-CM-001`, desde el 15-09-2026), **al asociar la personalizada** (`RF-CM-006`) y **al corregir** (`RF-CM-003`): corregir hacia porcentaje una tasa de un producto gratuito se rechaza entera, como el tope. **Se evalúa contra el precio de hoy y nadie vuelve a mirarlo** (§5.2): un porcentaje ya asociado a un producto que después baja a cero **pasa a pagar cero**, y un fijo asociado a un gratuito que después sube de precio **no se vuelve a acotar**. Es el mismo hueco temporal que `RN-CM-019` acepta, y se acepta por lo mismo | Alta |

| `RN-CM-021` | **La tasa de rol nace con su producto, y no lo cambia** | Al registrar y en toda corrección de una tasa de rol | **Nace el 15-09-2026** por decisión del responsable del proyecto. Toda tasa de rol declara **un producto**, obligatorio, y rige **solo sobre él** desde el alta. El producto **no se corrige**: cambiar de producto es retirar la tasa y registrar otra, porque lo que se pagó por el primero tiene que seguir resolviendo la misma fila. **Una tasa que se quiera repetir en varios productos son varias tasas** — es el precio de que cada producto se configure por su cuenta (§5.4) | **Crítica** |
...
**`RN-CM-006` — una sola vigente por persona y producto, y ya NO en el motor.** Es la que sostiene a `RN-CM-004`: con dos personalizadas de la misma persona cubriendo el mismo día **sobre el mismo producto**, la resolución deja de ser determinista. El 11-09-2026 **salió del motor**, y es el único movimiento de este módulo en esa dirección: el `EXCLUDE` cabía porque la persona, la vigencia y el producto vivían en la misma fila, y al pasar el producto a una tabla de asociación la regla **cruza dos tablas** — ningún índice hace eso. Pasa a comprobarse **al asociar**, con un **bloqueo consultivo por persona**, que es exactamente el patrón que `RN-CM-019` ya usaba por producto. **Lo que se pierde hay que decirlo**: era la única garantía del módulo que no dependía de que nadie se olvidara de comprobarla. Lo que se gana es que las dos clases de tasa se configuren igual.
...
**`RN-CM-021` — el producto es de la tasa, y la asociación de rol desaparece.** Lo que la hace crítica es lo que deja sin red: hasta el 15-09-2026 una tasa de rol podía existir sin pagar, y ponerla en vigor era un acto aparte que alguien podía olvidar; hoy **registrarla es ponerla en vigor**. El error posible cambia de signo —ya no es «configuré y no paga» sino «registré y paga desde ya»— y se acepta porque es el que se ve: una tasa de rol se lee junto a su producto en el mismo listado (`RF-CM-002`).
...
Lo que `RN-CM-019` **no** alcanza sigue siendo la suma de la cadena, aunque desde el 11-09-2026 por un motivo más estrecho. Hasta esa fecha una **tasa personalizada** no se ataba a ningún producto, de modo que quedaba fuera de cualquier cuenta que se pudiera hacer antes de la venta. Ahora se asocia como las demás y **tiene su propio tope**: ninguna fila de la cadena puede pasarse por su cuenta. Lo que falta es sumarlas, y para eso hay que saber **quiénes componen la cadena** — un dato que no existe al configurar, solo al vender. `60 + 30 + 20` sigue pagando el 110 % aunque los tres sean legítimos por separado.

Con el valor fijo el agujero **cambió de tamaño y de forma** al volver en v0.7.0, y desde el 11-09-2026 se cierra con **una sola frase para las dos clases**: una tasa **sin asociar** no tiene tope —no conoce el precio de nada—, y en cuanto se asocia lo tiene contra el precio de ese producto. Antes la personalizada era la excepción permanente a eso, porque nunca llegaba a atarse a nada.

**Y queda un hueco temporal que `RN-CM-019` acepta a conciencia, igual que el resto del módulo acepta los suyos**: el tope se calcula contra el precio del producto **en el instante de asociar o corregir**. Si el precio cambia después (`RF-PM-004`), nada vuelve a sumar — `PM` no conoce `CM`, y revisarlo en cada cambio de precio queda fuera de este alcance. La suma pudo ser válida el día que se configuró y dejar de serlo sin que nadie lo haya decidido.
...
| **¿La personalizada cambia también?** | **No**: sigue siendo de la persona, con vigencia, y **se asocia a productos como hoy** | *Que también naciera desde el producto* — es otro rediseño encima de este, y la excepción de una persona sí abarca varios productos |
...
|---|---|---|---|
| `commissions:read` | `commissions` | `read` | Consultar tasas —las de rol con su producto—, las asociaciones de la personalizada, y resolver la comisión efectiva |
| `commissions:create` | `commissions` | `create` | Registrar una tasa, de rol —para un producto— o personalizada |
| `commissions:update` | `commissions` | `update` | Corregir un valor, y **asociar o desasociar** productos a una personalizada |
...
### 7.3 ~~`product_commission_rates`~~ — retirada el 15-09-2026

Existió desde `V49` (02-09-2026) hasta `V94` (15-09-2026): `(product_id, commission_rate_id, role_id copiado, created_at)`, con la clave primaria `(product_id, role_id)` sosteniendo `RN-CM-013` y una clave foránea compuesta que impedía que el rol copiado divergiera del de la tasa. **Dejó de hacer falta cuando la tasa de rol pasó a llevar su producto como columna** (`RN-CM-021`, §5.4): lo que esta tabla decía —qué tasa rige sobre qué producto— lo dice hoy `commission_rates.product_id`, y la unicidad por rol y producto vive en la propia tabla. `V94` la **borra**, y con ella `uq_commission_rates_id_role`, que existía solo para que su clave foránea compuesta pudiera apuntar ahí. `user_commission_rate_products`, la asociación de la personalizada, **no se toca**.
