...
| **Tasa personalizada** | «**esta persona** gana el 12 % **en ESTE producto**» | **Sí**, y una sola vigente por persona **y producto** | **Sí, y es obligatorio** desde el 11-09-2026 |
...
**La personalizada también se acota a un producto**, desde el 11-09-2026 y por decisión del responsable del proyecto. Hasta esa fecha **ignoraba el producto**: quien tenía una ganaba lo mismo vendiera lo que vendiera. Hoy declara **a qué producto se refiere**, y sigue ganando sobre la de su rol **en ese producto y solo en ese** (`RN-CM-004`). Quien quiera una excepción en tres productos declara tres. Sigue siendo una excepción, no un grado más.
...
- Registrar la **tasa personalizada** de una persona **sobre un producto**, con su vigencia, **también en cualquiera de las dos formas**.
...
| `RN-CM-004` | **La personalizada gana siempre, en su producto** | Al resolver | Si la persona tiene una tasa personalizada **vigente en la fecha y para ese producto**, es esa. Si no la tiene, la que su **rol vendedor** tenga asociada a ese producto. Si no hay asociación, **no hay tarifa**. Siguen siendo dos niveles, no cuatro. **Enmendada el 11-09-2026**: hasta esa fecha la personalizada se resolvía **sin mirar el producto**, de modo que quien tuviera una ganaba lo mismo vendiera lo que vendiera. Ahora una excepción alcanza **solo al producto que declara**, y en el resto del catálogo esa persona vuelve a cobrar por su rol | **Crítica** |
...
| `RN-CM-006` | Una sola tasa personalizada vigente por persona **y producto** | Al registrar y al corregir una personalizada | **Ningún día puede estar cubierto por dos tasas personalizadas vivas de la misma persona sobre el mismo producto.** Sí pueden existir varias consecutivas —son el historial— y varias simultáneas **sobre productos distintos**, que es justamente lo que `RN-CM-004` habilita desde el 11-09-2026. Se declara **en el motor** con un `EXCLUDE`, porque es la única regla del módulo que dos peticiones simultáneas pueden burlar | **Crítica** |
...
| `RN-CM-014` | **Las dos clases de tasa se acotan a un producto, cada una a su manera** | Al asociar y al registrar una personalizada | **Invertida el 11-09-2026.** Hasta esa fecha decía que solo las de rol se acotaban y que una personalizada regía sobre todo el catálogo. Ahora las dos lo hacen, y la **forma** es distinta porque lo que son es distinto: la de **rol** es catálogo reutilizable y se acota con una **tabla de asociación** —una fila por producto, la misma tasa en muchos—; la **personalizada** ya es de una sola persona y se acota con una **columna propia**, `product_id`, porque no hay nada que reutilizar. Una personalizada **sin producto no se puede expresar**: la columna es `NOT NULL` | Alta |
...
| `RN-CM-018` | **El valor fijo no está acotado por arriba mientras no conoce ningún precio** | Siempre que la tasa no está atada a ningún producto | `RN-CM-007` acota el porcentaje a cien; el importe, por su cuenta, no tiene ese número — una tasa no conoce el precio de nada hasta que se le ata un producto. **Enmendada el 11-09-2026**: la **personalizada** ya no cae aquí, porque desde esa fecha nace atada a un producto y por tanto conoce su precio; su tope lo pone `RN-CM-019`. Lo que sigue sin tope es la tasa **de rol sin asociar**, y solo hasta que se le asocie el primer producto | **Crítica** |
| `RN-CM-019` | **Un producto no puede configurarse para pagar más del 100 % de sí mismo** | Al asociar y al corregir una tasa **de rol**, y al registrar y corregir una **personalizada** | La suma de lo que un producto paga a **todas** sus tasas de rol asociadas —cada porcentaje tal cual, cada valor fijo convertido a `fixed_amount ÷ precio × 100`, contra el precio de **ese** producto— no puede superar cien. Se comprueba **al asociar** (contando la nueva) y **al corregir** (revisando **todos** los productos donde la tasa corregida está asociada; si cualquiera se pasaría, la corrección se rechaza entera). **Desde el 11-09-2026 la tasa personalizada tiene su propio tope**, y es **individual y no una suma**: ninguna personalizada puede pagar más del 100 % del precio del producto al que se ata —un porcentaje ya lo acota `RN-CM-007`, y un valor fijo se convierte igual que arriba—. **No entra en la suma de las de rol**, y es deliberado: las personalizadas de personas distintas sobre el mismo producto son **alternativas entre sí**, no cosas que se paguen a la vez, y sumarlas rechazaría configuraciones legítimas. El tope se calcula contra el precio **de hoy**: si el producto cambia de precio después (`RF-PM-004`), nadie vuelve a comprobarlo. **Con precio CERO** (`RN-PM-006`) se resuelve sin excepción: un valor fijo **mayor que cero** sobre un producto gratuito paga más del 100 % de lo que ese producto cobra y **se rechaza**; uno de cero ocupa cero. La división `fixed_amount ÷ precio` **no llega a hacerse** en ese caso — ver §5.2 | **Crítica** |
...
**`RN-CM-006` — una sola vigente por persona y producto.** Es la que sostiene a `RN-CM-004`: con dos personalizadas de la misma persona cubriendo el mismo día **sobre el mismo producto**, la resolución deja de ser determinista. Desde el 11-09-2026 el alcance de la regla lleva el producto dentro, y eso **la afloja a propósito**: la misma persona puede tener varias vigentes a la vez, una por producto, que es justo lo que la enmienda habilita. **Y sigue siendo la única regla del módulo que vive en el motor**, porque es la única que dos peticiones simultáneas pueden burlar — es exactamente el defecto que `RN-SP-018` tuvo y que se corrigió el 26-08-2026.
...
Lo que `RN-CM-019` **no** alcanza sigue siendo la suma de la cadena, aunque desde el 11-09-2026 por un motivo más estrecho. Hasta esa fecha una **tasa personalizada** no se ataba a ningún producto, de modo que quedaba fuera de cualquier cuenta que se pudiera hacer antes de la venta. Ahora sí se ata, y **tiene su propio tope**: ninguna fila de la cadena puede pasarse por su cuenta. Lo que falta es sumarlas, y para eso hay que saber **quiénes componen la cadena** — un dato que no existe al configurar, solo al vender. `60 + 30 + 20` sigue pagando el 110 % aunque los tres sean legítimos por separado.

Con el valor fijo el agujero **cambió de tamaño y de forma** al volver en v0.7.0, y se ha ido cerrando por partes: una tasa fija **de rol asociada** entra en la suma de `RN-CM-019` convertida contra el precio de ese producto, y desde el 11-09-2026 una **personalizada** se convierte igual contra el precio del suyo. La única que sigue sin tope es la tasa fija **de rol sin asociar** — no conoce el precio de nada hasta que se le asocie el primer producto, y entonces se comprueba.
...
| 0.10.0 | 11-09-2026 | **La tasa personalizada deja de regir sobre todo el catálogo y pasa a declarar SU producto**, por decisión del responsable del proyecto. Hasta hoy quien tenía una ganaba lo mismo vendiera lo que vendiera: la resolución la elegía mirando **solo persona y fecha**. Ahora una excepción alcanza **al producto que nombra y a ninguno más**, y en el resto del catálogo esa persona vuelve a cobrar por su rol. **`user_commission_rates` gana `product_id` `NOT NULL`** \(`V84`\) — columna propia y no tabla de asociación, porque una personalizada ya es de una sola persona y no hay nada que reutilizar. **Cuatro reglas enmendadas y una invertida**: `RN-CM-004` resuelve por persona, producto y fecha; **`RN-CM-006` se AFLOJA a propósito** —una vigente por persona **y producto**, de modo que la misma persona puede tener varias simultáneas sobre productos distintos, y el `EXCLUDE` del motor lleva ahora el producto dentro—; `RN-CM-002` y `RN-CM-010` alcanzan también a la personalizada, que hasta hoy no nombraba ningún producto; y **`RN-CM-014` queda INVERTIDA**: decía que solo las de rol se acotaban a un producto, y ahora lo hacen las dos, cada una con la forma que le corresponde. **Se cierra `RN-CM-018` para las personalizadas**: su excusa escrita era que «nunca conoce el precio de nada», y atarla a un producto se la quita — un valor fijo personalizado que supere el precio de su producto se **rechaza**, con el mismo cálculo que ya hacen las de rol. Ese tope es **individual y no una suma** (`RN-CM-019`): las personalizadas de personas distintas sobre el mismo producto son **alternativas entre sí**, no cosas que se paguen a la vez, y sumarlas rechazaría configuraciones legítimas. **`RN-CM-011` se encoge pero NO se cierra**: ninguna fila de la cadena puede pasarse por su cuenta, pero sumar la cadena sigue exigiendo saber quiénes la componen, y eso no se sabe al configurar. **La migración ABORTA si encuentra alguna personalizada viva**: no se puede adivinar a qué producto pertenecía una tasa que valía para todos, y retirarlas en silencio dejaría a alguien sin cobrar su excepción hasta la siguiente liquidación. | Responsable del proyecto |
