...
- **V.13** Toda eliminación —lógica o física— DEBE registrar el **motivo** declarado por quien la ejecuta. El motivo es obligatorio en el esquema y en el contrato de la API: una eliminación sin motivo DEBE rechazarse antes de ejecutarse. NO DEBE suplirse con un valor automático, salvo en tres casos, que DEBEN declararse en la especificación: los procesos internos sin actor humano; la eliminación de **asociaciones** —filas de relación cuyo significado se agota en el par que vinculan, como un permiso asignado a un rol—; y la eliminación de **contenido propio** —lo que una persona escribió en nombre propio y retira ella misma, como una reseña—. En una asociación el «por qué» ya está contenido en el propio evento (qué se desvinculó, de qué, quién y cuándo), y exigir un texto libre solo produciría ruido. En el contenido propio ocurre lo mismo por otro camino: el único «por qué» posible es que su autor ya no lo quiere ahí, y el evento lo contiene entero cuando **quien elimina y quien escribió son la misma persona**; exigirle un texto a quien retira lo suyo produce «lo borro» en cada fila. Esa tercera excepción se agota en la coincidencia: la misma fila, retirada por **cualquier otra persona** —quien modera—, exige motivo como toda entidad de negocio. Ninguna de las tres excepciones alcanza a las demás entidades de negocio. La auditoría de eliminación DEBE además conservar el **estado del registro al momento de eliminarse**: saber que algo se borró no sirve si ya no puede saberse qué era.
