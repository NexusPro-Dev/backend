...
| `AC` | `course_categories`, `courses`, `course_category_items`, `course_recommendations`, `course_memberships`, `course_products`, `course_modules`, `lessons`, `academy_images` | **5 escritas** —`course_categories` (`V18`), `courses` (`V21`), `course_modules` (`V23`), `lessons` (`V24`) y `course_category_items` (`V42`, 25-09-2026)— **y cuatro diseñadas** (§4.2): las crearán los requerimientos que las estrenan, en el orden de [`requirements/ac.md`](requirements/ac.md) §6.1 |
...
| `course_products.product_id` | `products` | `AC` → `PM` — qué servicio abre el curso (25-09-2026, diseñada). **La primera de `AC` hacia `PM`**; que sea `BOT` no cabe en la clave y lo comprueba el dominio contra `ProductCatalog`. **Sin `ON DELETE`**: toda suite que borre `products` borra antes estas filas |
