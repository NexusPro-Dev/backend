...
| **D-21** | ~~Lista de proxies confiables por entorno~~ · **Reformulada el 27-08-2026, y a peor.** Se creía que era «qué IPs poner en `TRUSTED_PROXIES` en cada entorno». Al aterrizar sobre Railway ([`ADR-002`](architecture/ADR-002-plataforma-de-despliegue-railway.md)) resulta que **no hay ninguna que poner**: `ClientIpResolver` compara el par inmediato contra un conjunto de **coincidencia exacta**, sin rangos ni CIDR, y la dirección con la que el borde de la plataforma habla con el contenedor no es fija ni publicada. Con la lista vacía el resolvedor hace lo correcto —ignora `X-Forwarded-For` y registra la IP del socket—, de modo que **la auditoría apunta al proxy**: un dato incompleto pero cierto, en lugar de uno que el atacante elige escribiendo una cabecera. Lo que se pierde es el **Art. V.15 entero**: desde dónde se hizo cada operación no se responde hoy en un entorno desplegado. La salida **no es una lista de IPs** sino que `ClientIpResolver` admita rangos | El Art. V.15 en `testing` y `production`. No bloquea el despliegue: [`deployment.md` §11.2](deployment.md#112-lo-que-la-verificacion-no-puede-comprobar-hoy) |
