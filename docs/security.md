...
| **RN-SEG-015** | Autenticarse no autoriza nada | Al declarar el permiso de una operación de la API | **Toda operación que exija token exige además un permiso**, también las que actúan sobre el propio actor (`/users/me`, `/movements/mine`, la propia contraseña, la compra propia). Las únicas operaciones sin permiso son las **públicas** —las que se atienden sin token: entrar, renovar, salir, registrarse, recuperar la contraseña, los catálogos del formulario de registro, el hotlink y lo que su pantalla muestra— y la lista es cerrada y la vigila `EndpointPermissionsIT`. Decisión del responsable del proyecto (`RF-SP-062`, 21-09-2026): el frontend decide qué vista o consulta mostrar por el permiso, y «solo tener el token» no dice nada | **Crítica** |
...
Un permiso se identifica con el formato `<recurso>:<acción>`, en minúsculas, y **gobierna una operación o ninguna, nunca dos** (`RN-SEG-014`, desde el 19-09-2026). El catálogo, tal como queda con `V28` (`RF-SP-060`), `V29` (`users:read-sellers`, `RF-SP-059`), `V30` (`users:read-clients`, `RF-SP-061`), `V31` (los once de alcance propio de `RF-SP-062`, 21-09-2026), `V32` (`movements:list-sales`, `RF-MV-015`), `V34` (los ocho `teams:` de `RF-SP-063` a `RF-SP-070`), `V36` (`movements:assign-sellers`, `RF-MV-016`, 23-09-2026) y `V37` (`movements:list-sale-lines`, `RF-MV-017`, 23-09-2026), **menos los dos que `V38` retira** (`users:assign-membership` y `users:revoke-membership`, `RF-SP-032` y `RF-SP-033` descartados el 23-09-2026) — **ciento treinta y tres**; los cincuenta y uno que `V28` siembra se listan debajo, y los once de `V31` en la nota de `RN-SEG-015`:
...
users:read-own-profile  users:update-own-profile  users:change-own-password
users:read-own-sellers  users:read-own-clients
...
!!! danger "Autenticarse no autoriza nada — `RN-SEG-015`, desde el 21-09-2026"

    **Hasta el 21-09-2026 once operaciones se atendían con solo el token**: el propio perfil y su corrección, la propia contraseña, mis vendedores, mis clientes, las cuentas de broker de mi equipo y las de una persona a cargo, mis movimientos —listado y detalle—, mis productos comprados y la compra propia de un paquete. Cada una lo justificaba como «alcance sobre uno mismo: no hay nada que autorizar más allá de estar autenticado», y el argumento era correcto **para la seguridad** y equivocado **para el frontend**, que decide qué vista o consulta mostrar mirando los permisos del actor y con «tiene token» no puede decidir nada. Lo dijo el responsable del proyecto con sus palabras: «no basta con solo tener el token».

    **`RF-SP-062` lo cierra**: cada una recibe permiso propio, sembrado por `V31` —`users:read-own-profile`, `users:update-own-profile`, `users:change-own-password`, `users:read-own-sellers`, `users:read-own-clients`, `broker-accounts:read-own-team`, `broker-accounts:read-team-member`, `movements:list-own`, `movements:read-own`, `movements:read-own-products`, `packages:buy`—, con las convenciones de `RF-SP-060` y **`own` para el alcance sobre uno mismo**, como `products:read-own-comments` ya hacía. **Nadie pierde nada**: `V31` da cada permiso a **todo rol** —de sistema o creado a mano— **por su tipo**: los de `FUNCIONARIO` y `VENDEDOR` reciben los once; los de `CONSUMIDOR` reciben ocho —no `users:read-own-clients`, `broker-accounts:read-own-team` ni `broker-accounts:read-team-member`, que son de vendedor y el frontend no debe ofrecérselos a un cliente—. Es la misma lógica de `V28` (cada hijo a todo rol que portara el padre) con «estar autenticado» como padre. Y una guarda: ningún rol queda con un permiso que su padre no porte (`RN-SEG-003`).

    **Las públicas siguen sin permiso, y no es una excepción a la regla sino su límite**: una operación que se atiende sin token no tiene a quién pedirle uno. Son catorce, y la lista cerrada vive en `EndpointPermissionsIT` (`PUBLICAS`, antes `SIN_PERMISO_A_PROPOSITO`): `POST /auth/login`, `/auth/refresh`, `/auth/logout`, `/auth/registration`, `/auth/password-recovery` y su confirmación; `GET /countries`, `/document-types`, `/brokers`, `/payment-methods`; `GET /hotlinks/{username}/{code}`, `/hotlinks/{username}/packages/{code}`, `/products/{id}/comments` y `/product-images/{imageId}`. Una ruta autenticada sin `@PreAuthorize` falla en `mvn verify`.

    **Lo que cambia para quien administra roles**: un rol creado a mano después del 21-09-2026 nace, como siempre, sin permisos —y desde hoy eso significa que sus personas **no pueden ni ver su perfil ni cambiar su contraseña** hasta que se le concedan los de alcance propio por `RF-SP-005`. Es el precio de la regla y queda escrito; el frontend, que ya distingue vistas por permiso, lo enseña como lo que es.
...
**Nacen los diez permisos de `AC` el 17-09-2026 —los cuatro `course-categories:` sembrados por `V19` ese mismo día, los seis `courses:` sembrados por `V22` el 18-09-2026 (`RF-AC-008`), con lo que el catálogo llega a **sesenta**—**, por decisión del responsable del proyecto ([`requirements/ac.md`](requirements/ac.md) v0.1.0 §7): cuatro `course-categories:` y seis `courses:`. **Las categorías tienen recurso propio y los módulos y lecciones no**, por la misma diferencia que `PM` hizo entre `packages:` y la portada: una categoría existe sin cursos y la administra quien organiza el catálogo, que puede no ser quien arma un curso; un módulo y una lección no existen sin su curso, y `courses:update` los cubre —registrar, corregir, cambiar de estado, retirar y poner portada— junto con las tres relaciones del curso. **Dos de los seis no son `<recurso>:<acción>` sobre una escritura**, como `products:sale` y `products:hotlink` no lo son: **`courses:learn` gobierna la vista del alumno** —catálogo, detalle y contenido de lo que se ofrece— y **`courses:teach` no gobierna ninguna ruta**: habilita a **figurar como instructor** (`RN-AC-006`), y lo comprueba `AC` al asignar, contra una interfaz que `SP` publicará a petición de `RF-AC-008` —«¿esta persona porta este permiso?», la primera que responde sobre un permiso y no sobre un dato—. Se sembrarán con `RF-AC-001` los cuatro y con `RF-AC-008` los seis, **asociados a `SUPERADMIN` y a `ADMIN` sin reserva** —también `teach` y `learn`: que un administrador pueda ser instructor o recorrer el aula no es una operación que deba quedar exclusiva de la raíz—, y **a `CLIENTE` no**, por lo mismo que `products:sale`: quien administre roles concede `courses:learn` a los de tipo `CONSUMIDOR` por `RF-SP-006`, y `courses:teach` a un rol de instructor que este módulo **no siembra**, porque sembrar roles es de `SP` y qué roles existen es decisión del negocio.
...
!!! danger "La reserva del superadministrador — DOS desde el 24-09-2026, cuando la tercera se levantó"
    ...
    | ~~`movements:read`, `movements:create`, `movements:confirm`, `movements:void`~~ | `V51`, 02-09-2026 — **LEVANTADA el 24-09-2026** (`V40`) | **Decisión del responsable del proyecto.** `ADMIN` pasa a portar los cuatro, y con él la cadena comercial entera puede declararlos. Es exactamente lo que el párrafo de abajo anunciaba que haría falta: mientras la reserva siguiera en pie, **no había a quién delegar el trabajo diario de la fuerza comercial** |

    **La tercera se levantó, y este apartado se conserva porque explica por qué.** Lo que sigue se escribió cuando la reserva estaba en pie, y describe el coste que acabó decidiendo su retirada: no hay que leerlo como una descripción del sistema de hoy, sino como el razonamiento que llevó a `V40`.

    **Las dos primeras cuestan poco y la tercera costaba el módulo.** `audit:read-security` y `currencies:update` cubren operaciones que de verdad solo hace el superadministrador, de modo que la consecuencia —«`ADMIN` no puede delegar lo que no tiene»— es exactamente la que se busca.
...
| Imagen de portada de academia (`GET`, por la familia `/api/v1/academy-images/`) — **construida** el 25-09-2026, `RF-AC-032` | 120 | — | 1 minuto | — |
...
| `GET /api/v1/academy-images/{imageId}` | **La imagen de portada de una categoría, un curso o un módulo de Academia** (`RF-AC-032`, 17-09-2026, **construida** el 25-09-2026): la décima ruta pública del sistema y la primera de `AC`. Es `GET /api/v1/product-images/{imageId}` sobre **otra tabla, `academy_images`**, porque `product_images` es de `PM` y [`modules.md` §7](modules.md#7-reglas-de-dependencia) prohíbe que `AC` la escriba ([`requirements/ac.md` §5.2.3](requirements/ac.md)); **el detector de firma sí se comparte**, y pasa a `shared/`. Hereda **todo lo demás sin cambio**: bytes con su tipo real, solo `JPEG`, `PNG` y `WebP`, caché inmutable de un año, `nosniff`, `404` si no existe, y **no sabe de qué entidad es la imagen**. **Nada más de Academia es público**: el aula entera exige sesión y `courses:learn`, y la lección abierta a todos (`RN-AC-014`) es «todos los que tienen sesión», no «cualquiera» | Siempre, y solo en `GET`: la subida y el retiro viven bajo `course-categories:update` y `courses:update`. Cota propia por familia, con el mismo número que la de `PM` |
...
!!! note "El alcance de datos está pendiente de diseño (D-22)"
    ...
    **Actualización del 18-09-2026 — la excepción no se amplía, cambia de tabla para un caso.** El cliente sale de `user_supervisors` (`RN-SP-028` revertida, [`requirements/sp.md`](requirements/sp.md) v1.59.0) y su vendedor principal pasa a ser la fila `REGISTRO` de `client_sellers` (`RN-SP-049`). `RN-SP-046` sigue diciendo lo mismo —**el superior vigente ve las cuentas de su gente**—, pero «superior vigente» de un cliente se resuelve ahora en esa tabla y no en la de mando. **Sigue siendo la única lectura autorizada por estructura**, sigue siendo un solo nivel y sigue cerrada sobre las cuentas de broker: lo que cambia es una consulta, no el perímetro. Y **`RF-SP-059` no entra aquí**: `GET /users/me/sellers` es alcance sobre uno mismo, como `GET /users/me`, y `GET /users/{id}/sellers` lo autoriza `users:read` — ninguno de los dos se apoya en la estructura.

    **Actualización del 21-09-2026 — la segunda lectura por estructura, y la primera en profundidad, por decisión del responsable del proyecto.** `RF-MV-015` (`RN-MV-031`, [`requirements/mv.md`](requirements/mv.md) v0.32.0) publica **las ventas de mi alcance**: el consumidor ve las suyas, **el vendedor las suyas y las de toda su red hacia abajo** —`user_supervisors` vigente, recursivo, con la raíz incluida— y quien porta un rol `FUNCIONARIO` todas. Rompe **dos** de las cotas escritas arriba —«la única lectura autorizada por estructura» y «un solo nivel»— y no se disfraza: el responsable lo pidió así («yo como director puedo ver las ventas de mis agentes, y los managers de los directores y por ende también los agentes»), y sin la profundidad un manager no vería nada. Lo que se conserva es lo que hace que siga siendo **localizable**: **(1)** el permiso sigue abriendo la ruta (`movements:list-sales`, `RN-SEG-015`) y la estructura decide **qué** se ve, no **si** se entra; **(2)** el alcance **no lo calcula `MV`**: `SP` publica **`CommercialReach`** —«hasta dónde llega esta persona», por su tipo de rol y su subárbol— y `MV` lo aplica como predicado, de modo que hay **una** definición de «mi red» y vive con su dueño (`architecture.md` §15.2); **(3)** fuera del alcance se responde **vacío**, no `403` ni `404`: el filtro por persona no es un oráculo de la estructura; **(4)** se cierra sobre **las ventas**: los depósitos y las comisiones declararán el suyo. **Y responde las tres preguntas de negocio de [`ADR-005`](architecture/ADR-005-modelo-de-alcance-de-datos.md)**: el alcance se determina por `user_supervisors` y no por membresía; el actor **se ve a sí mismo**; quien no tiene superior ni subordinados alcanza **a sí mismo**. Lo que **no** cierra D-22 es el modelo técnico general —la comprobación de arquitectura que obligue a todo listado a declarar su alcance—: `CommercialReach` es el resolvedor de la opción B con **un** tipo de alcance, y la regla de ArchUnit sigue pendiente.

    **Actualización del 21-09-2026.** Nacen los **equipos** (`RF-SP-063` a `RF-SP-070`, `requirements/sp.md` §10.20 y §10.21): agrupan a los managers, la cúspide de la estructura. **Tampoco conceden alcance**: `teams:*` son ocho permisos de administración, sembrados a `SUPERADMIN` y `ADMIN`, y pertenecer a un equipo no abre ningún dato a nadie. Si algún día «mi equipo» tiene que significar algo para la autorización, se decide en D-22 y no en `team_members` — la advertencia de §10.21 de aquel documento es la misma que esta. Y la actualización de arriba lo deja más claro: el alcance de `RF-MV-015` recorre `user_supervisors` y **no** pasa por los equipos.
...
| 0.58.0 | 16-09-2026 | **`RN-SEG-012` deja de alcanzar a los permisos de un rol de sistema**, por decisión del responsable del proyecto ([`requirements/sp.md`](requirements/sp.md) v1.57.0). La regla nació como «los roles de sistema no se modifican ni eliminan» y la semilla `V8` la contradecía desde el primer día: `MANAGER`, `DIRECTOR`, `AGENTE` y `CLIENTE` se siembran **sin permisos a propósito**, «a la espera de `RF-SP-005`» — y `RF-SP-005` los rechazaba con `409`. El resultado era que **ningún vendedor ni ningún cliente podía tener nunca un permiso**, y se descubrió el 16-09-2026 al intentar darle `products:sale` a `CLIENTE` desde la pantalla de roles. Se resuelve **acotando la regla a lo que de verdad protege**: la identidad y la posición del rol —código, tipo, padre, estado y eliminación—, que son lo que sostiene la contención de privilegios (§4.3) y lo que una migración necesita encontrar donde lo dejó. **Los permisos se administran por la vía normal**, con las tres cotas de siempre: contenidos en el padre (`RN-SEG-003`), poseídos por quien los concede (`RN-SEG-010`) y nunca sobre un rol que el actor porta (`RN-SEG-011`). Se descartó sembrarlos por migración: cada ajuste de permisos exigiría otra, que es justo lo que `V8` decía no querer. Y se descartó colgar roles hijos no de sistema: `RN-SEG-003` los acota a un padre vacío. **`SUPERADMIN` y `ADMIN` quedan igual de protegidos que antes en lo que importa**: `RN-SEG-011` impide que quien los porta les toque los permisos, y `RN-SEG-010` que nadie conceda a `ADMIN` lo que no posee. | Responsable del proyecto |
...
| 0.72.0 | 23-09-2026 | **El catálogo de §4.4 BAJA por primera vez, de ciento treinta y cinco a ciento treinta y tres**: `V38` retira `users:assign-membership` y `users:revoke-membership` con los requerimientos que los pedían —`RF-SP-032` y `RF-SP-033`, descartados (`RN-SP-056`, [`requirements/sp.md`](requirements/sp.md) v1.84.0)—, por decisión del responsable del proyecto. **No es una operación que cambie de permiso, es una operación que deja de existir**: el nivel de una persona se compra (`RF-MV-003`) o se recibe al registrarse, y fijarlo a mano deja de ser una puerta del sistema. La migración borra las dos filas de `permissions`, sus asociaciones en `role_permissions` y la implicación que `V28` había sembrado entre ambas; ningún rol queda sin permisos por ello. **`RN-SEG-014` se cumple igual**: los dos códigos gobernaban una operación cada uno, y las dos se van con ellos. | Responsable del proyecto |
| 0.73.0 | 24-09-2026 | **La reserva del superadministrador baja de seis permisos a dos, y la cadena comercial recibe lo que le faltaba** (`V40`), por decisión del responsable del proyecto. `ADMIN` pasa a portar los cuatro `movements:` —`read`, `create`, `confirm` y `void`—, y con ello se puede delegar hacia abajo: `MANAGER` y `DIRECTOR` reciben `movements:read` y `movements:create`, y `CLIENTE` recibe `movements:create`. **No es una concesión nueva, es la que §4.5 ya anunciaba que haría falta**: el propio apartado de la reserva dejaba escrito que, por `RN-SEG-003`, con `ADMIN` fuera **ningún rol de esa cadena podría declarar `movements:create`** — «no es que `ADMIN` no pueda delegarlo: es que no hay a quién delegárselo». Quedan reservados `audit:read-security` y `currencies:update`, que son los dos que de verdad solo hace el superadministrador. **El reparto comercial también se ensancha**: `products:sale` y `products:hotlink` a `MANAGER`, `DIRECTOR` y `AGENTE` —`products:sale` también a `CLIENTE`—, las reseñas a `CLIENTE` (`products:comment`, `products:update-comment`), y `users:read-team`, `users:read-clients` y `users:read-sellers` a `MANAGER` y `DIRECTOR`. **Y dos retiros deliberados**: `CLIENTE` pierde `movements:list-sales` —no vende— y **`users:read-own-sellers` se retira de `CLIENTE` y de `AGENTE`**; `CLIENTE` conserva en su lugar `users:read-sellers`, de modo que consulta por identificador y no por `/me`. Lo segundo se preguntó y se confirmó como intencional: **no es el mismo permiso y no abre la misma ruta**, y queda anotado aquí porque a quien lo lea dentro de seis meses le parecerá un cruce. **La contención de `RN-SEG-003` se mantiene**, comprobada sobre el reparto entero antes de escribir la migración. | Responsable del proyecto |
| 0.74.0 | 24-09-2026 | **Nace `products:buy-by-hotlink`** (`RF-MV-011`, `V41`; [`requirements/mv.md`](requirements/mv.md) v0.43.0): comprar un producto **por el enlace de un vendedor**, que es la compra en la que la venta se le acredita a quien repartió el enlace y no a quien ya le vendía. **Propio y no `products:buy`, y la distinción es el motivo de que exista**: son dos puertas sobre el mismo producto con **atribuciones distintas**, y quien administra roles tiene que poder abrir una sin abrir la otra — con un solo permiso esa decisión no se puede expresar. Es el criterio de `RN-SEG-014`, el mismo que separó `users:revoke-membership` de `users:assign-membership`. **Se reparte por TIPO de rol** y no por lista, como `V31` con la familia de alcance propio: un rol de consumidor nuevo tiene que poder comprar sin que nadie toque una migración. | Responsable del proyecto |
| 0.75.0 | 25-09-2026 | **`GET /api/v1/academy-images/{imageId}` pasa de diseñada a construida** (`RF-AC-032`): pública solo en `GET` en `SecurityConfig`, y con su familia de cota propia en `RateLimitFilter`, sin compartir contador con `/product-images/`. | Responsable técnico |
| 0.76.0 | 26-09-2026 | **Siete permisos de `MV` declarados y sin sembrar** (§4.4), los de la etapa 6 —pagos, saldos y retiros— ([`requirements/mv.md`](requirements/mv.md) v0.44.0). El catálogo sembrado no cambia. `RF-MV-004` deja de compartir `movements:confirm` y estrena `movements:reject-payment`, por `RN-SEG-014`. | Responsable técnico |
| 0.77.0 | 26-09-2026 | **`movements:list-own-entries`**, el octavo permiso declarado y sin sembrar de la etapa 6 de `MV` (§4.4): el historial de los saldos propios (`RF-MV-022`, [`requirements/mv.md`](requirements/mv.md) v0.46.0). | Responsable técnico |
